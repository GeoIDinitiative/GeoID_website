...
# IAU named-feature outlines, one file per world and moon
# (bake-nomenclature.py): every one that has been baked.
NESTED_FILES += tuple(sorted(f"nomenclature/{p.name}"
                             for p in (GLOBAL / "nomenclature").glob("*.geojson")))
