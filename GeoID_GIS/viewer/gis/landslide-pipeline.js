...
import { samplerOver } from "./layer-query.js?v=20260920-f6aad18";
import { refreshPolygonOptions, resolvePolygonExtent, promptDrawTool } from "./extent-picker.js?v=20260920-f6aad18";
import { fetchWindow, fetchGfsNodes, rainfallFrames, interpolatorFor, dayHours, GFS_CREDIT, GFS_ARCHIVE_START } from "./gfs-rain.js?v=20260920-f6aad18";
import {
...
} from "./slope-hydrology.js?v=20260920-f6aad18";
import { fillSinks, mfdTopology, routeFlux } from "./hydrology.js?v=20260920-f6aad18";
import { makeRaster, slope as slopeOf } from "./raster-analysis.js?v=20260920-f6aad18";
import { buildRasterLayer } from "./geotiff-adapter.js?v=20260920-f6aad18";
import { loadRockProperties, parameterValue, resolveLithology } from "./rock-properties.js?v=20260920-f6aad18";
import { GEE_RAIN_SOURCES, coversBox, daysBetween, geeRainDates, fetchGeeRainParts, pixelIndex, isoDay as dayOf } from "./gee-rain.js?v=20260920-f6aad18";
import { mathsFor } from "./equations.js?v=20260920-f6aad18";
import { startPlayer, stopPlayer, seekPlayer } from "./timelapse-player.js?v=20260920-f6aad18";
import { upslopeWeights, stationStep, stationFlood, catchmentTopology, floodScratch, LANDSLIDE_PARAMS, LANDSLIDE_PLOTS, lowestCells } from "./landslide-stations.js?v=20260920-f6aad18";
import {
  makeStation, parseStationsCsv, stationsFromFeatures, uniqueName, seriesCsv, seriesFileName, MAX_STATIONS, colourAt,
} from "./station-series.js?v=20260920-f6aad18";
import { drawTimeSeries, yRangeOf } from "./time-series-plot.js?v=20260920-f6aad18";
import { planSeries, rendersOf, stepText, rampMaxFor, STEP_CHOICES, NATIVE_STEP, HOUR } from "./rain-steps.js?v=20260920-f6aad18";
import { mountStationMarkers } from "./station-markers.js?v=20260920-f6aad18";
import { equivalentMohrCoulomb, culmann, culmannAt, rockCell, localRelief, rockfallReach, velocityOf, criticalHeight } from "./rock-slope.js?v=20260920-f6aad18";
import {
...
} from "./flood-fos.js?v=20260920-f6aad18";
import { inundate, sourceFields, DEPTH_CLASSES, DEFAULTS as FLOOD_DEFAULTS, meanFlowFromWidth } from "./inundation.js?v=20260920-f6aad18";
import { burnRivers } from "./river-zones.js?v=20260920-f6aad18";
import { waterFeatures, waterMasks } from "./water-mask.js?v=20260920-f6aad18";
import { may, refusal } from "./membership.js?v=20260920-f6aad18";
...
function wire() {
  ...
  byId("lsp-run").addEventListener("click", () => {
    // The forecast IS one of the modelled maps, so it is asked here rather
    // than after the rain has been fetched: a GEE render is billed, and a
    // refusal after spending one is the wrong order.
    if (!may("landslides")) { say("run", refusal("landslides"), "warn"); return; }
    void run();
  });
...
if (typeof window !== "undefined" && typeof window.addEventListener === "function") {
  window.GeoIDLandslidePipeline = {
    ...
    /**
     * THE RUN AS AN EXPOSURE SOURCE: the model's grid, its area, the maps'
     * times, and the governing factor of safety of any map on demand.
     *
     * `fosAt` recomputes a map through `modelFrame`, which writes into the
     * ground's SHARED scratch buffers — the same arrays the map on the globe is
     * painted from. So `restore()` must be called when a caller is done, and
     * it puts the step the reader was looking at back.
     */
    exposureSource: () => {
      const g = state.ground; const r = state.run; const b = state.bounds;
      if (!g || !r || !b) return null;
      return {
        label: LAYER_NAME,
        grid: { width: g.grid.width, height: g.grid.height, bounds: g.grid.bounds },
        model: g.cells.model,
        ring: [[b.west, b.south], [b.east, b.south], [b.east, b.north], [b.west, b.north], [b.west, b.south]],
        times: r.frames.map((f) => f.time),
        step: () => state.step,
        minFos: r.minFos,
        fosAt: (k) => {
          const out = modelFrame(k);
          const arr = new Float32Array(g.n).fill(NaN);
          for (let i = 0; i < g.n; i += 1) if (g.cells.model[i]) arr[i] = governingFos(i, out.fos[i], out.rockFos[i]);
          return arr;
        },
        restore: () => { if (state.run && state.step >= 0) showStep(state.step); },
      };
    },
