...
} from "./gales-results.js?v=20260916-e94c613";
import { zipStore } from "./shapefile-writer.js?v=20260916-e94c613";
import { fieldArrays, pvdText, vtkCells, vtuParts } from "./vtk-export.js?v=20260916-e94c613";
import { parse as parseExpression, namesIn, variableTable, evaluate as evaluateExpression } from "./field-calculator.js?v=20260916-e94c613";
import { parseSolidProps } from "./strain-stress.js?v=20260916-e94c613";
import { vtkHead, readVtkGrid, parsePvd, vtkFieldName } from "./vtk-read.js?v=20260916-e94c613";
import { PLATFORMS, DEFAULT_GEOMETRY, losVector, losDisplacement, wrapFringes, fringeCount, fringesPerEdge, FRINGE_MAP } from "./insar.js?v=20260916-e94c613";
...
async function projectSource(runDir) {
  ...
  const walk = async (rel, depth) => {
    ...
    for (const item of list) {
      ...
      } else entries.push({ path, size: Number.isFinite(item.size) ? item.size : null });
    }
  };
  await walk(runDir, 0);
  let last = null;
...
async function valuesAt(fieldIndex, stepIndex) {
  ...
  // A step whose size the listing did not know is known now — written back
  // onto the PLAN's step, so a rebuild of the field list (a temporal summary,
  // a gradient, a calculator) keeps the description rather than dropping it
  // and answering "no such component". Measured on a run opened from a
  // project through a listing that carried no sizes.
  if (!Number.isFinite(step.size)) step.size = buffer.byteLength;
