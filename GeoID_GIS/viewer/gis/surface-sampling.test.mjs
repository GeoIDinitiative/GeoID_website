...
// ── The GUI is wired to this module, both pages ───────────────────────────────
{
  ...
  check("the studio's readout reads the surface, not the air, in degrees east", /if \(gisTerrain\?\.skin\?\.visible\) targets\.push\(gisTerrain\.skin\);/.test(studio) && /h\.object\.material\.opacity < 1\)\)/.test(studio) && /const lonEast = \(\(geo\.lon % 360\) \+ 360\) % 360;/.test(studio));
  {
    ...
    check("the ribbon and the deck fold", /class="studio-ribbon-fold"/.test(earth) && /data-fold="left"/.test(earth) && !/data-fold="right"/.test(earth));
    check("the planets' studio is the same markup as Earth's", earth === planets);
    check("the studio wires every [data-act] on the page, folds the decks and cards the sections", /#model-studio \[data-act\]/.test(studio) && /function wireRibbonAndFolds\(\)/.test(studio) && /function foldPaneSections\(\)/.test(studio) && /--studio-chrome-h/.test(studio) && /buildFromText,/.test(studio));
    ...
    const modes = readFileSync(new URL("./mode-manager.js", import.meta.url), "utf8");
    ...
    check("ONE deck, the GIS page's tab column: eight toolbox-groups in two bands, one open per band", ["add", "model", "label", "history"].every((g) => earth.includes(`data-group="${g}" data-deck="left" data-band="build"`)) && ["mesh", "structured", "refine", "log"].every((g) => earth.includes(`data-group="${g}" data-deck="left" data-band="mesh"`)) && !/studio-dock-right/.test(earth) && !/studio-deck-caption/.test(earth) && /class="control-section toolbox-group studio-group"/.test(earth) && !/studio-tabs|studio-tab"/.test(earth) && /function showGroup\(name\)/.test(studio) && /const band = group\.dataset\.band \|\| "build";/.test(studio));
    {
      const pipe = readFileSync(new URL("./model-pipeline.js", import.meta.url), "utf8");
      const hud = readFileSync(new URL("./draw-hud.js", import.meta.url), "utf8");
      check("the Model Builder raises the drawer through the rail's own button", /import \{ promptDrawTool \} from "\.\/extent-picker\.js/.test(pipe) && /Draw on the globe/.test(pipe) && /if \(promptDrawTool\(\)\)/.test(pipe));
      // A window listener hears nothing: the viewer dispatches a plain Event
      // on `document`, which does not bubble. Measured as zero on the page.
      check("and takes the shape from the seam the viewer really announces on", /document\.addEventListener\("geoid-study-area-edited", drawnWatch\);/.test(pipe) && !/window\.addEventListener\("geoid-study-area-edited"/.test(pipe) && /function adoptDrawnArea\(\)/.test(pipe) && /if \(picker && picker\.value !== "drawn"\) return;/.test(pipe));
      check("a dead export is borrowed off the rail and its SLOT hides", /slot\.hidden = exportDead\(actions\);/.test(hud) && /#gis-draw-export-slot\[hidden\] \{ display: none !important; \}/.test(hud) && /attributeFilter: \["hidden", "style", "class", "disabled"\]/.test(hud) && /subtree: true/.test(hud));
    }
    check("the frame is its contents, capped where the worlds strip is", /#model-studio \.studio-dock \{\n  top: calc\(var\(--studio-chrome-h, 5rem\) \+ 1\.4rem\);\n  bottom: auto;\n  max-height: calc\(100% - var\(--studio-chrome-h, 5rem\) - 1\.4rem - 3\.2rem\);/.test(css));
    ...
    check("the mode switch is IN the bar, and the tile it had is gone", /<span class="studio-brand">Meshing Studio<\/span>\n      <span id="studio-mode-slot"><\/span>/.test(earth) && !/studio-modebar/.test(earth) && !/studio-modebar/.test(css) && /\.brand-toprow\.is-parked \{[^}]*justify-content: flex-start;/.test(css) && !/is-in-studio/.test(css));
    ...
    check("the WHOLE top row is what gets parked, so the folder and the player travel with it", /function parkModeRow\(slotId\) \{/.test(modes) && /document\.querySelector\("\.brand-toprow"\)/.test(modes) && /row\.classList\.add\("is-parked"\)/.test(modes) && !/is-in-studio/.test(modes));
    check("and Model mode's hide of the floating player cannot take the parked one with it", /body\.studio-open \.brand-toprow\.is-parked #music-btn \{ display: inline-flex !important; \}/.test(css));
    ...
    check("the deck collapses into its margin and leaves the GIS page's own edge handle", /class="studio-edge-tab" data-edge="left"/.test(earth) && !/data-edge="right"/.test(earth) && /#model-studio \.studio-dock-left\.is-folded \{ transform: translateX\(calc\(-100% - 1\.4rem\)\); pointer-events: none; \}/.test(css) && /writing-mode: vertical-rl;/.test(css) && /if \(tab\) tab\.hidden = !folded;/.test(studio));
