...
// ── The GUI is wired to this module, both pages ───────────────────────────────
{
  ...
  check("the studio's readout reads the surface, not the air, in degrees east", /if \(gisTerrain\?\.skin\?\.visible\) targets\.push\(gisTerrain\.skin\);/.test(studio) && /h\.object\.material\.opacity < 1\)\)/.test(studio) && /const lonEast = \(\(geo\.lon % 360\) \+ 360\) % 360;/.test(studio));
  {
    ...
    check("a deck is the GIS page's tab column: four toolbox-groups, one open at a time", ["add", "model", "label", "history"].every((g) => earth.includes(`data-group="${g}" data-deck="left"`)) && ["mesh", "structured", "refine", "log"].every((g) => earth.includes(`data-group="${g}" data-deck="right"`)) && /class="control-section toolbox-group studio-group"/.test(earth) && !/studio-tabs|studio-tab"/.test(earth) && /function showGroup\(side, name\)/.test(studio) && /if \(other !== group\) other\.open = false;/.test(studio));
