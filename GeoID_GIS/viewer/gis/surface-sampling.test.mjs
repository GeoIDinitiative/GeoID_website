...
// ── The GUI is wired to this module, both pages ───────────────────────────────
{
  ...
  check("every domain is its flagged faces, each a part with a row and a card", /function renderPartsList/.test(studio) && /function showPartCard/.test(studio) && /\["sides", "wall", 0xa8703f, F\.sides_below/.test(studio) && /\["sky", "lid", 0x9fd8ff, F\.sky/.test(studio));
  ...
  check("the air shell is displayed without its floor", /\? \[\["top", "ground"/.test(studio) && /: \[\["sky", "lid"/.test(studio));
