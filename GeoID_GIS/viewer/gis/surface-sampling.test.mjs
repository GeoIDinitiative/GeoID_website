...
// ── The GUI is wired to this module, both pages ───────────────────────────────
{
  ...
  check("the studio's readout reads the surface, not the air, in degrees east", /if \(gisTerrain\?\.skin\?\.visible\) targets\.push\(gisTerrain\.skin\);/.test(studio) && /h\.object\.material\.opacity < 1\)\)/.test(studio) && /const lonEast = \(\(geo\.lon % 360\) \+ 360\) % 360;/.test(studio));
  {
    ...
    check("the mode switch is IN the bar, and the tile it had is gone", /<span class="studio-brand">Meshing Studio<\/span>\n      <span id="studio-mode-slot"><\/span>/.test(earth) && !/studio-modebar/.test(earth) && !/studio-modebar/.test(css) && /#model-studio \.view-mode-switch\.is-in-studio \{\n  display: flex;\n  flex-direction: row;/.test(css));
