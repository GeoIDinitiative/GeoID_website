...
// ── The GUI is wired to this module, both pages ───────────────────────────────
{
  ...
  check("the studio's readout reads the surface, not the air, in degrees east", /if \(gisTerrain\?\.skin\?\.visible\) targets\.push\(gisTerrain\.skin\);/.test(studio) && /h\.object\.material\.opacity < 1\)\)/.test(studio) && /const lonEast = \(\(geo\.lon % 360\) \+ 360\) % 360;/.test(studio));
  {
    ...
    check("the band is said by a hairline, not by a BUILD/MESH caption", !/studio-deck-caption/.test(css) && /#model-studio \.studio-group\[data-band="build"\] \+ \.studio-group\[data-band="mesh"\] \{/.test(css));
    check("every nested subtab arrives collapsed, and a remembered state still wins", /details\.open = Boolean\(folds\[key\]\);/.test(studio) && /details\.open = domainOpen\.get\(id\) \?\? false;/.test(studio) && !/air\.open = true;|pts\.open = true;/.test(studio));
