...
// ── The GUI is wired to this module, both pages ───────────────────────────────
{
  ...
  check("the studio's readout reads the surface, not the air, in degrees east", /if \(gisTerrain\?\.skin\?\.visible\) targets\.push\(gisTerrain\.skin\);/.test(studio) && /h\.object\.material\.opacity < 1\)\)/.test(studio) && /const lonEast = \(\(geo\.lon % 360\) \+ 360\) % 360;/.test(studio));
  {
    ...
    check("ONE deck, the GIS page's tab column: eight toolbox-groups in two bands, one open per band", ["add", "model", "label", "history"].every((g) => earth.includes(`data-group="${g}" data-deck="left" data-band="build"`)) && ["mesh", "structured", "refine", "log"].every((g) => earth.includes(`data-group="${g}" data-deck="left" data-band="mesh"`)) && !/studio-dock-right/.test(earth) && (earth.match(/studio-deck-caption/g) || []).length === 2 && /class="control-section toolbox-group studio-group"/.test(earth) && !/studio-tabs|studio-tab"/.test(earth) && /function showGroup\(name\)/.test(studio) && /const band = group\.dataset\.band \|\| "build";/.test(studio));
