...
import * as IN from "./interpolation.js?v=20260826-9a6f617";
import * as VAL from "./validation.js?v=20260826-9a6f617";
import * as EX from "./analysis-extra.js?v=20260826-9a6f617";
import * as HY from "./hydrology.js?v=20260826-9a6f617";
import * as KR from "./kriging.js?v=20260826-9a6f617";
...
export async function runToolAuto(toolId, inputs = {}, params = {}, opts = {}) {
  ...
  let why = "";
  try {
    const client = await import("./sidecar-client.js?v=20260826-9a6f617");
    await client.probe();
...
async function persistDerived(desc, layer, name, record) {
  ...
  try {
    ...
    if (desc.outputType === "raster" && layer.raster) {
      const { writeGeoTiff } = await import("./geotiff-writer.js?v=20260826-9a6f617");
      return await bridge.saveProcessed(`${name}.tif`, writeGeoTiff(layer.raster),
        { mime: "image/tiff", provenance });
    }
    if (layer.collection) {
      const { toGeoJson } = await import("./vector-formats.js?v=20260826-9a6f617");
