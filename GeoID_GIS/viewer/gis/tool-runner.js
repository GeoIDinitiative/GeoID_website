...
import { buildSurface, nativeStepM } from "./model-build.js?v=20260830-f494dda";
import { nativeGridOf } from "./extraction.js?v=20260830-f494dda";
import { CRS_OPTIONS } from "./projection.js?v=20260830-f494dda";
import * as IN from "./interpolation.js?v=20260830-f494dda";
import * as VAL from "./validation.js?v=20260830-f494dda";
import * as EX from "./analysis-extra.js?v=20260830-f494dda";
import * as HY from "./hydrology.js?v=20260830-f494dda";
import * as KR from "./kriging.js?v=20260830-f494dda";
...
async function runToolAutoInner(desc, toolId, inputs, params, opts) {
  ...
  let why = "";
  try {
    const client = await import("./sidecar-client.js?v=20260830-f494dda");
    await client.probe();
...
async function persistDerived(desc, layer, name, record) {
  ...
  try {
    ...
    if (desc.outputType === "raster" && layer.raster) {
      const { writeGeoTiff } = await import("./geotiff-writer.js?v=20260830-f494dda");
      return await bridge.saveProcessed(`${name}.tif`, writeGeoTiff(layer.raster),
        { mime: "image/tiff", provenance });
    }
    if (layer.collection) {
      const { toGeoJson } = await import("./vector-formats.js?v=20260830-f494dda");
