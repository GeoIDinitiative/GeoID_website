...
import * as IN from "./interpolation.js?v=20260821-c97c8db";
import * as VAL from "./validation.js?v=20260821-c97c8db";
import * as EX from "./analysis-extra.js?v=20260821-c97c8db";
import * as HY from "./hydrology.js?v=20260821-c97c8db";
import * as KR from "./kriging.js?v=20260821-c97c8db";
...
export async function runToolAuto(toolId, inputs = {}, params = {}, opts = {}) {
  ...
  let why = "";
  try {
    const client = await import("./sidecar-client.js?v=20260821-c97c8db");
    await client.probe();
...
async function persistDerived(desc, layer, name, record) {
  ...
  try {
    ...
    if (desc.outputType === "raster" && layer.raster) {
      const { writeGeoTiff } = await import("./geotiff-writer.js?v=20260821-c97c8db");
      return await bridge.saveProcessed(`${name}.tif`, writeGeoTiff(layer.raster),
        { mime: "image/tiff", provenance });
    }
    if (layer.collection) {
      const { toGeoJson } = await import("./vector-formats.js?v=20260821-c97c8db");
