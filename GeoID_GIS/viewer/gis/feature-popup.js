...
import { pointInPolygon, boundsOf, haversineMetres } from "./geometry.js?v=20260909-8ac5c77";
...
import { rockClass, crustalSetting, rockClassLabel } from "./rock-class.js?v=20260909-8ac5c77";
import { lithologyLabel } from "./lithology-label.js?v=20260909-8ac5c77";
...
import { isRiskFeature, riskCard } from "./cyclone-risk-card.js?v=20260909-8ac5c77";
import { isZoneFeature, zoneCard } from "./volcanic-zone-card.js?v=20260909-8ac5c77";
import {
  canEditRow, editableFields, applyRowChange,
} from "./table-editor.js?v=20260909-8ac5c77";
...
function showViewerCard(hits, at) {
  ...
  const feature = zone ? {
    ...
    rows: [["Note", risk.note],
      ...rows.filter(([key]) => !/^(i|deg|rate_yr|p_yr|rate_hur_yr|p_hur_yr|years_per|rate_large_yr|p_large_yr|rate_large_yr|p_large_yr)$/i.test(key))],
    stack: beneath.map(({ layer, feature: f }) => ({
      label: layer.name || "Layer",
      unit: titleOf(f.properties || {}) || featureKind(f, layer),
    })),
...
function install() {
  ...
  canvas.addEventListener("click", (event) => {
    ...
    if (!hits.length) {
      ...
      if (window.GeoIDVolcanicRiskRaster?.probeAt?.(at.lat, at.lon)) return;
