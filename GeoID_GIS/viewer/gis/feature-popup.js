...
import { pointInPolygon, boundsOf, haversineMetres } from "./geometry.js?v=20260909-07f3be8";
...
import { rockClass, crustalSetting, rockClassLabel } from "./rock-class.js?v=20260909-07f3be8";
import { lithologyLabel } from "./lithology-label.js?v=20260909-07f3be8";
...
import { isRiskFeature, riskCard } from "./cyclone-risk-card.js?v=20260909-07f3be8";
import { isVolcanicRiskFeature, volcanicRiskCard } from "./volcanic-risk-card.js?v=20260909-07f3be8";
import { isZoneFeature, zoneCard } from "./volcanic-zone-card.js?v=20260909-07f3be8";
import {
  canEditRow, editableFields, applyRowChange,
} from "./table-editor.js?v=20260909-07f3be8";
...
function showViewerCard(hits, at) {
  ...
  const risk = !ice && !soil && isVolcanicRiskFeature(props)
    ? volcanicRiskCard(props, (() => {
      const w = window.GeoIDVolcanicRisk?.viewOf?.(props);
      return { view: w?.view, full: w?.id === "volcanic-risk-holocene" };
    })())
  ...
  const feature = zone ? {
    ...
    rows: [["Note", risk.note],
      ...rows.filter(([key]) => !/^(i|deg|rate_yr|p_yr|rate_hur_yr|p_hur_yr|years_per|rate_large_yr|p_large_yr|vei_max|vei_mean|tephra_m3_yr|vents|top_volcano|top_gvp|top_eruptions|top_vei_max|top_rate_yr)$/i.test(key))],
    stack: beneath.map(({ layer, feature: f }) => ({
      label: layer.name || "Layer",
      unit: titleOf(f.properties || {}) || featureKind(f, layer),
    })),
