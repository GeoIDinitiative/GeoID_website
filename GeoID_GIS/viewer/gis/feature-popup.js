...
import { pointInPolygon, boundsOf, haversineMetres } from "./geometry.js?v=20260910-7d63d6c";
...
import { rockClass, crustalSetting, rockClassLabel } from "./rock-class.js?v=20260910-7d63d6c";
import { lithologyLabel } from "./lithology-label.js?v=20260910-7d63d6c";
...
import { isRiskFeature, riskCard } from "./cyclone-risk-card.js?v=20260910-7d63d6c";
import { isVolcanicRiskFeature, volcanicRiskCard } from "./volcanic-risk-card.js?v=20260910-7d63d6c";
import { isSeismicRiskFeature, seismicRiskCard } from "./seismic-risk-card.js?v=20260910-7d63d6c";
import { isEarthquakeFeature, earthquakeCard } from "./earthquake-card.js?v=20260910-7d63d6c";
import { isZoneFeature, zoneCard } from "./volcanic-zone-card.js?v=20260910-7d63d6c";
import {
  canEditRow, editableFields, applyRowChange,
} from "./table-editor.js?v=20260910-7d63d6c";
...
function showViewerCard(hits, at) {
   ...
   */
  /**
   * THE VOLCANIC GRID IS CHECKED FIRST. Its cells carry the three columns the
   * cyclone card recognises its own by, so tested second they would open as
   * a tropical cyclone; `vei_max` is the column only they have.
   */
  const risk = !ice && !soil && isSeismicRiskFeature(props)
    ? seismicRiskCard(props, { band: window.GeoIDRiskFrames?.bandOf?.(props)?.band || "any" })
    : !ice && !soil && isVolcanicRiskFeature(props)
      ? volcanicRiskCard(props, window.GeoIDVolcanicRisk?.bandOf?.(props) || {})
  ...
  const feature = quake ? {
    ...
    rows: [["Note", risk.note],
      ...rows.filter(([key]) => !/^(i|deg|rate_yr|p_yr|rate_hur_yr|p_hur_yr|years_per|rate_large_yr|p_large_yr|vei_max|vents|prior_only|vei0|vei1|vei2|vei3|vei4|vei5|vei6|vei7|vei8|none|m5|m6|m7|m8|mag_max|quakes)$/i.test(key))],
    stack: beneath.map(({ layer, feature: f }) => ({
      label: layer.name || "Layer",
      unit: titleOf(f.properties || {}) || featureKind(f, layer),
    })),
