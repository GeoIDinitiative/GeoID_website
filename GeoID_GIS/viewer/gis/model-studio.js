...
import { currentBody, getBody, currentBodyId } from "./bodies.js?v=20260919-6aa21d5";
import { PRIMITIVES, buildSurface, buildInside, boundingBoxOf } from "./mesh-primitives.js?v=20260919-6aa21d5";
import {
  latticeTetMesh, tetBoundarySurface, qualityStats, elementCounts, toGmsh22,
} from "./mesh-volume.js?v=20260919-6aa21d5";
import { MODEL_MODE_RADIUS } from "./geo-utils.js?v=20260919-6aa21d5";
import { downloadText } from "./extraction.js?v=20260919-6aa21d5";
import { shellPositions, surfacePositions, tinHeightAt, tinToGrid, gridAsTin, tinValueAt } from "./surface-sampling.js?v=20260919-6aa21d5";
import { rampColour } from "./symbology.js?v=20260919-6aa21d5";
import { layeredVolumes, facetPositions, tinWith, LAYER_FLAGS } from "./layered-model.js?v=20260919-6aa21d5";
import { sectionPolygons, sectionPositions, profileHeightAt } from "./section-model.js?v=20260919-6aa21d5";
import { faceParts, partPositions, studioGmshScript, DEFAULT_FACE_FLAGS } from "./studio-gmsh.js?v=20260919-6aa21d5";
import { describeField, FIELD_TYPES } from "./mesh-size-fields.js?v=20260919-6aa21d5";
import { femSpec } from "./model-build.js?v=20260919-6aa21d5";
...
function log(line) {
  ...
  // The same line as a toast, where the eye is (studio-toast.js decides which are news).
  try { document.dispatchEvent(new CustomEvent("geoid-studio:notice", { detail: { text: String(line), level: /\b(fail|error|cannot|could not|refused)\b/i.test(line) ? "error" : "", source: "studio" } })); } catch (e) { /* a page with no CustomEvent */ }
}
...
/** Opening stretch and gap for a layered model: each thin layer to ~4% of the model's height. */
function layerViewDefaults(L, heightM) {
  const H = Math.max(1, heightM);
  let soilSum = 0; let soilN = 0; let waterSum = 0; let waterN = 0;
  for (let i = 0; i < L.solid.length; i += 1) {
    const t = L.solid[i] - L.bedrock[i];
    if (t > 0.01) { soilSum += t; soilN += 1; }
    const d = L.water[i] - L.solid[i];
    if (d > 0.01) { waterSum += d; waterN += 1; }
  }
  const stretch = (sum, n) => (n ? Math.max(1, Math.min(100, Math.round((0.04 * H) / (sum / n)))) : 1);
  return {
