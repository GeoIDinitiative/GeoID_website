...
check("a cell with nothing is not drawn, and is not the app's grey", fp.colourFor({ properties: { p_yr: 0 } }), "transparent");
check("and the renderer honours that by drawing nothing", /colourFor\(feature\) === "transparent"\) \{\s*return;/.test(readFileSync(new URL("./vector-render.js", import.meta.url), "utf8")), true);
check("while the key still names the class", NONE_LABEL, "no eruption's ash on record — not drawn");
