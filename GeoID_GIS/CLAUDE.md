...
`gis/research/qt-layout.json`: 62 pages, 2571 nodes. `qt-render.js` walks that,
...
What is left out is genuinely built on demand and cannot be in a static tree:
MapPage's per-layer rows and dialogs, CSV Plotter's dataset cards, Signal
Processing's `_series_box` factory.
