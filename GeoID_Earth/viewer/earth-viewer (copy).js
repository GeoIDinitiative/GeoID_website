import * as THREE from "./vendor/three.module.js";
    ...
    async function init() {
      ...
      const geologyFeatureCatalog = await loadJsonSafe(manifest.geology_interactive ? manifest.geology_interactive.feature_path : "");
      ...
      const SC_LAYERS = [
        ...
        { id: "sc-radiation",    label: "UV Index",                 path: "../assets/earth_sc_radiation.png",          scGroup: "Surface Conditions — Atmospheric", scParamKey: "radiation" },
        ...
        { id: "sc-co2_frost",    label: "Sea Ice Probability",       path: "../assets/earth_sc_co2_frost.png",          scGroup: "Surface Conditions — Atmospheric", scParamKey: "co2_frost" },
        ...
        { id: "sc-dust_devil",   label: "Convective Instability",   path: "../assets/earth_sc_dust_devil.png",         scGroup: "Surface Conditions — Habitability", scParamKey: "dust_devil" },
        { id: "sc-landing",      label: "Habitability Score",       path: "../assets/earth_sc_landing_score.png",      scGroup: "Surface Conditions — Habitability", scParamKey: "landing_score" },
        ...
        { id: "sc-brine",        label: "Surface Water Potential",  path: "../assets/earth_sc_brine_stability.png",    scGroup: "Surface Conditions — Habitability", scParamKey: "brine_stability" },
      ...
      const scBaseTextureResults = await Promise.all(
        SC_LAYERS.map((layer) => loadTextureSafe(textureLoader, layer.path)),
      );
      for (let i = 0; i < SC_LAYERS.length; i++) {
        const tex = scBaseTextureResults[i];
        if (tex) {
        ...
        layerTextures.set(SC_LAYERS[i].id, tex || null);
      }
    ...
    function isObjectActuallyVisible(object) {
      ...
      function createUserPinFromInspectPoint(inspectPoint = gisInspectPoint) {
        ...
        openNewPinEditor({
	      ...
	      function shouldRunEmbeddedGeoSelector(event) {
	        if (window.self === window.top || coreToggle.checked || activeMoonViewerFeature) return false;
	        ...
	        if (hoveredFeature || activeGeoPopupFeature) return false;
      ...
      renderer.domElement.addEventListener("pointerup", (event) => {
        ...
        if (shouldRunEmbeddedGeoSelector(event)) {
          const selectorHit = intersectMeasurementSurface(event.clientX, event.clientY);
