import * as THREE from "./vendor/three.module.js";
    ...
    console.log("[ctxPatchDebug] venus-viewer.js loaded", { manifest: window.__marsViewerManifest, verbose: window.__ctxPatchDebug.verbose });

    const PLANET_NAME = "Venus";
    const PLANET_SLUG = "venus";
    ...
    const seaSection = seaToggle ? seaToggle.closest("details") : null;
    const mineralSelectRow = mineralSelect ? mineralSelect.closest(".row") : null;
    const mineralOpacityRow = mineralOpacity ? mineralOpacity.closest(".row") : null;
    const surfaceConditionsPanel = document.getElementById("surface-conditions");

    if (!labelData.length) {
      if (tourModeToggle) {
        tourModeToggle.checked = false;
        tourModeToggle.disabled = true;
      }
      if (tourModeSection) tourModeSection.style.display = "none";
      if (featureSearch) featureSearch.placeholder = "No named features available in current assets";
      if (featureSearchGo) featureSearchGo.disabled = true;
    }
    if (!moonData.length) {
      if (moonToggle) {
        moonToggle.checked = false;
        moonToggle.disabled = true;
        const row = moonToggle.closest(".row");
        if (row) row.style.display = "none";
      }
      if (moonViewerSection) moonViewerSection.style.display = "none";
      if (moonViewerToggle) moonViewerToggle.checked = false;
    }
    if (!Array.isArray(manifest.mineral_layers) || !manifest.mineral_layers.length) {
      if (mineralSelectRow) mineralSelectRow.style.display = "none";
      if (mineralOpacityRow) mineralOpacityRow.style.display = "none";
    }
    ...
    if (surfaceConditionsPanel) surfaceConditionsPanel.hidden = true;
    if (geologyContactsToggle) {
      geologyContactsToggle.checked = false;
      geologyContactsToggle.disabled = true;
      const row = geologyContactsToggle.closest(".row");
      if (row) row.style.display = "none";
    }
    if (geologyStructuresToggle) {
      geologyStructuresToggle.checked = false;
      geologyStructuresToggle.disabled = true;
      const row = geologyStructuresToggle.closest(".row");
      if (row) row.style.display = "none";
    }
    if (geologyTectonicToggle) { const row = geologyTectonicToggle.closest(".row"); if (row) row.style.display = "none"; geologyTectonicToggle.checked = false; geologyTectonicToggle.disabled = true; }
    if (geologyGrabenToggle) { const row = geologyGrabenToggle.closest(".row"); if (row) row.style.display = "none"; geologyGrabenToggle.checked = false; geologyGrabenToggle.disabled = true; }
    if (geologyFluvialToggle) { const row = geologyFluvialToggle.closest(".row"); if (row) row.style.display = "none"; geologyFluvialToggle.checked = false; geologyFluvialToggle.disabled = true; }
    if (geologyImpactToggle) { const row = geologyImpactToggle.closest(".row"); if (row) row.style.display = "none"; geologyImpactToggle.checked = false; geologyImpactToggle.disabled = true; }
    if (geologyVolcanicToggle) { const row = geologyVolcanicToggle.closest(".row"); if (row) row.style.display = "none"; geologyVolcanicToggle.checked = false; geologyVolcanicToggle.disabled = true; }
    if (geologyErosionalToggle) { const row = geologyErosionalToggle.closest(".row"); if (row) row.style.display = "none"; geologyErosionalToggle.checked = false; geologyErosionalToggle.disabled = true; }
    if (geologyEolianToggle) { const row = geologyEolianToggle.closest(".row"); if (row) row.style.display = "none"; geologyEolianToggle.checked = false; geologyEolianToggle.disabled = true; }
    ...
    async function init() {
      ...
      setStatus(`Loading ${PLANET_NAME} textures...`);
      ...
      const baseLayers = manifest.layers || [{
        ...
        description: "Global Venus basemap from the local viewer manifest.",
      }];
      ...
      const geologyLayers = manifest.geology_layers || [{
        id: "venus-geology",
        label: "Venus geology context",
        path: manifest.geology.path,
        description: "Topographic context overlay derived from Magellan global products.",
        default: true,
      }];
      ...
      const SC_LAYERS = manifest.surface_condition_layers || [];
      ...
      const initialLayer = baseLayers.find((layer) => layer.default) || baseLayers[0];
...
uniform float uViewportWidth;`,
      ...
      const GIS_BOOKMARK_STORAGE_KEY = `${PLANET_SLUG}-gis-bookmarks-v1`;
      const GIS_STUDY_AREA_STORAGE_KEY = `${PLANET_SLUG}-gis-study-areas-v1`;
      const GIS_PIN_STORAGE_KEY = `${PLANET_SLUG}-gis-pins-v1`;
      const GIS_BUFFER_STORAGE_KEY = `${PLANET_SLUG}-gis-buffers-v1`;
      const GIS_BASE_STORAGE_KEY = `${PLANET_SLUG}-gis-bases-v1`;
    ...
    function isObjectActuallyVisible(object) {
      ...
      function createUserPinFromInspectPoint(inspectPoint = gisInspectPoint) {
        if (!inspectPoint || inspectPoint.bodyName !== "Mars") {
          return;
        }
        openNewPinEditor({
      ...
      function exportGisPinsGeoJSON() {
        ...
        downloadJson("mars_user_pins.geojson", {
      ...
      function exportGisPinsCsv() {
        ...
        downloadCsv("mars_user_pins.csv", [
      ...
      function currentMeasurementGeoJSON() {
        ...
        if ((measureMode === "distance" || measureMode === "profile" || measureMode === "route") && measurePoints.length >= 2) {
          ...
          return {
            ...
            properties: {
              tool: measureMode,
              body: measurePoints[0].bodyName || PLANET_NAME,
      ...
      function syncGisInspectSection() {
        ...
        if (gisPinPlaceButton) gisPinPlaceButton.disabled = !gisInspectPoint || gisInspectPoint.bodyName !== "Mars";
        ...
        if (gisInspectPoint) {
          ...
          gisMetric.innerHTML = [
            `<strong>${gisInspectPoint.bodyName || PLANET_NAME} Inspect</strong>`,
        ...
        if (measureMode === "distance" && measurePoints.length >= 2) {
          ...
          if (routeMetric) {
            routeMetric.innerHTML = [
              `<strong>${measurePoints[0].bodyName || PLANET_NAME} route</strong>: ${routeProfile.totalDistanceKm.toFixed(1)} km`,
          ...
          profileCanvas.hidden = true;
          showProfileModal(
            `${measurePoints[0].bodyName || PLANET_NAME} Route Profile`,
          ...
          profileCanvas.hidden = true;
          showProfileModal(
            `${measurePoints[0].bodyName || PLANET_NAME} Elevation Profile`,
