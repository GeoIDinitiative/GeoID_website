import * as THREE from "./vendor/three.module.js";
    ...
    console.log("[ctxPatchDebug] mercury-viewer.js loaded", { manifest: window.__mercuryViewerManifest, verbose: window.__ctxPatchDebug.verbose });

    const manifest = window.__mercuryViewerManifest;
    ...
    // Mercury sidereal rotation period: 58.646 Earth days.
    const _MERCURY_DISPLAY_PERIOD_MS = 600000; // 10 min visual rotation
    const _MERCURY_ROT_REAL_MS = 58.6462 * 24 * 3600000; // rotation period in ms
    const _MERCURY_MOON_SPEED_FACTOR = _MERCURY_ROT_REAL_MS / _MERCURY_DISPLAY_PERIOD_MS; // scaled
    ...
    const TOUR_MODE_FACETS = [
      ...
      },
      {
        ...
        description: "Historic Mercury landers and rover landing sites.",
    ...
    if (brandLogo) {
      brandLogo.src = "../../../assets/mercury_icon.png?v=20260329d";
    }
    ...
    const locatorViewState = { camera: null, mercuryGroup: null, globe: null };
    ...
    function sampleLocatorLatLon() {
      if (!locatorViewState.globe || !locatorViewState.camera || !locatorViewState.mercuryGroup || activeMoonViewerFeature) {
        return null;
      }
      locatorCameraLocal.copy(locatorViewState.camera.position);
      locatorViewState.mercuryGroup.worldToLocal(locatorCameraLocal);
    ...
    // ── Mercury interior model (depth-based) ────────────────────────────────────
    ...
    const MERCURY_INTERIOR_T_PTS = [[0.0, 1800], [0.6, 1600], [0.83, 1300], [0.998, 200], [1.0, 170]];
    const MERCURY_INTERIOR_P_PTS = [[0.0, 36.0], [0.6, 30.0], [0.83, 11.0], [0.998, 0.2], [1.0, 0.0]];
    ...
    function estimateMercuryInteriorTemperature(rFrac) {
      return Math.round(_interiorInterp(MERCURY_INTERIOR_T_PTS, rFrac));
    }

    function estimateMercuryInteriorPressure(rFrac) {
      return Math.round(_interiorInterp(MERCURY_INTERIOR_P_PTS, rFrac) * 10) / 10;
    }

    function mercuryInteriorLayerName(rFrac) {
    ...
    function mercuryInteriorLayerColor(layerName) {
      return MERCURY_INTERIOR_LAYER_COLORS[layerName] ?? "#f6dcc8";
    }

    function mercuryInteriorTempColor(tempC) {
    ...
    function mercuryInteriorPressureColor(gpa) {
    ...
    function estimateMercuryWindSpeed(latDeg, elevMeters) {
      const latRad = latDeg * Math.PI / 180;
    ...
    function estimateMercuryIrradiance(latDeg, elevMeters) {
      const latRad = latDeg * Math.PI / 180;
    ...
    function estimateMercuryRadiation(elevMeters) {
      return Math.round((0.5 * Math.exp(elevMeters / 14000) + 0.2) * 100) / 100;
    }

    function estimateMercuryDiurnalRange(latDeg, elevMeters) {
      const latRad = latDeg * Math.PI / 180;
    ...
    function estimateSpaceTemperature(cameraDistanceFromMercury) {
      // Near Mercury: ~−63 °C effective equilibrium. Far from Mercury: bias to deep space ~−247 °C.
      // Interpolate across camera distance to avoid sudden jumps.
      const t = Math.max(0, Math.min(1, (cameraDistanceFromMercury - 3.5) / 30));
    ...
    const SC_PARAMS = {
      temperature: {
        ...
        compute: (lat, elev) => estimateMercuryTemperature(lat, elev),
      ...
      pressure: {
        ...
        compute: (_lat, elev) => estimateMercuryPressure(elev),
      ...
      wind: {
        ...
        compute: (lat, elev) => estimateMercuryWindSpeed(lat, elev),
      ...
      irradiance: {
        ...
        compute: (lat, elev) => estimateMercuryIrradiance(lat, elev),
      ...
      radiation: {
        ...
        compute: (_lat, elev) => estimateMercuryRadiation(elev),
      ...
      diurnal: {
        ...
        compute: (lat, elev) => estimateMercuryDiurnalRange(lat, elev),
    ...
    let activeGeoPopupLocalPos = null;   // THREE.Vector3 in Mercury body-local space (unspun)
    ...
    let mercurySceneGroup = null;
    let mercuryGlobeRef = null;   // set by init(); bridged so module-level openGeoPopup can access globe rotation
    let moonLayer = null;
    let gisBases = [];
    const saturnViewModeSelect = null; // Mercury has no tilted/untilted toggle
    ...
    const MERCURY_MEAN_RADIUS_KM = 2439.7;
    ...
    const CORE_LAYER_DATA = [
      ...
      },
      {
        ...
        name: "Mantle",
        type: "Silicate shell",
        description: "A thick shell of iron-rich silicate rock. Mercury lacks active plate tectonics, so heat escapes slowly through a thick, immobile lithosphere rather than at spreading centres. Much of the easily melted mantle source appears to be depleted relative to early Mercury, helping explain why large-scale magma production is now limited and volcanism is far less vigorous than in the past.",
      ...
      },
      {
        ...
        description: "Whether Mercury has a solid inner core is unconfirmed. InSight seismic data is ambiguous; low seismic activity limits deep-interior resolution.",
    ...
    const CTX_SERVICE_CANDIDATES = [
      {
        name: "CTX",
        serviceUrl: "https://astro.arcgis.com/arcgis/rest/services/OnMercury/CTX/MapServer",
      },
      {
        name: "CTX1",
        serviceUrl: "https://astro.arcgis.com/arcgis/rest/services/OnMercury/CTX1/MapServer",
      },
    ];
    ...
    function createContourLineLayer(THREERef, mercuryGroup, elevationSampler, getTerrainRelief, radius = 3.2) {
      const group = new THREERef.Group();
      group.visible = false;
      mercuryGroup.add(group);
    ...
    function greatCircleDistanceKm(a, b) {
      const radiusKm = a.radiusKm || b.radiusKm || MERCURY_MEAN_RADIUS_KM;
    ...
    function sphericalPolygonAreaKm2(points) {
      ...
      const radiusKm = points[0]?.radiusKm || MERCURY_MEAN_RADIUS_KM;
    ...
    function estimateSurfaceSlopeDegrees(elevationSampler, latDegrees, lonDegrees, radiusKm = MERCURY_MEAN_RADIUS_KM) {
      if (!elevationSampler) {
        return null;
      }
      const latStep = 0.08;
    ...
    function destinationPoint(latDegrees, lonDegrees, bearingDegrees, distanceKm, radiusKm = MERCURY_MEAN_RADIUS_KM) {
    ...
    function projectLatLonToLocalKm(point, origin, radiusKm = MERCURY_MEAN_RADIUS_KM) {
    ...
    function buildBufferSourceState() {
      ...
      if (sourceType === "inspect") {
        if (!(gisInspectPoint && gisInspectPoint.bodyName === "Mercury")) {
          return null;
        }
        return {
          type: "inspect",
    ...
    function buildMercurySurfacePolyline(vertices, closed = false, lift = 0.014) {
    ...
    function buildMercuryPolygonFillMesh(vertices) {
      if (!Array.isArray(vertices) || vertices.length < 3) {
        return null;
      }
      const boundary = buildMercurySurfacePolyline(vertices, true, 0.012);
    ...
    function exportCurrentProfilePng() {
      ...
      const baseName = String(currentProfilePlotState.title || "mercury_elevation_profile")
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "_")
        .replace(/^_+|_+$/g, "") || "mercury_elevation_profile";
    ...
    function buildLegendEntries(baseLayers, geologyLayers, mineralLayers, geologyInteractiveState = null, geologyStructureLayers = []) {
      ...
      if (geologyToggle.checked && selectedGeologyLayer) {
        ...
        entries.push({
          title: "Solid Geology",
          copy: "USGS, Geologic Map of Mercury simplified into rock-type classes for the active solid geology overlay.",
          tags: ["geology", "units"],
      ...
      if (
        ...
        if (activeStructureFacets.length) {
          entries.push({
            ...
            copy: "USGS, Geologic Map of Mercury structural traces split into individually toggleable structure families.",
    ...
    function buildMetadataState(baseLayers, geologyLayers, mineralLayers, geologyInteractiveState = null, geologyStructureLayers = []) {
      ...
      if (geologyContactsToggle.checked && (geologyInteractiveState?.contacts || []).length) {
        pushActiveLayer(
          ...
          "USGS, Geologic Map of Mercury geologic contacts draped from the vector shapefiles.",
        );
      }
      const activeStructureFacets = getActiveGeologyStructureFacets(geologyStructureLayers);
      if (activeStructureFacets.length) {
        pushActiveLayer(
          ...
          "geology",
          selectedGeologyLayer || selectedBaseLayer,
          `USGS, Geologic Map of Mercury structural traces. Active facets: ${activeStructureFacets.map((facet) => facet.label).join(", ")}.`,
        );
      }
      if (selectedMineralLayer) {
      ...
      if (coreEnabled) {
        pushActiveLayer(
          ...
          "core",
          selectedBaseLayer,
          "Interior layers are a schematic Mercury model inferred from InSight seismic data, gravity measurements, and geochemical analysis.",
        );
      }
      ...
      if (
        ...
        sourceEntries.push({
          ...
          copy: "Use the source pages below for the USGS, Geologic Map of Mercury layer and related Mercury geology references.",
      ...
      sourceEntries.push({
        title: "Mercury ambient sound",
        copy: "NASA InSight recording of ambient sound on the Martian surface.",
        links: [makeMetadataLink("NASA InSight sounds", "https://mercury.nasa.gov/news/8564/nasa-insight-lander-captures-stunning-sounds-of-mercury/")],
      });
      sourceEntries.push({
    ...
    function activateMoonViewer(feature, camera, controls) {
      if (!isMoonFeature(feature) || !mercurySceneGroup) {
        return;
      }
      cancelCameraFlight();
      deactivateTourMode();
      ...
      const target = mercurySceneGroup.localToWorld(localTarget.clone());
      const direction = target.clone().normalize();
      if (direction.lengthSq() < 0.0001) {
        direction.set(0.55, 0.18, 1).normalize();
      }
      // Offset the entry angle so Mercury isn't dead-centre behind the moon.
      ...
      app.innerHTML = `
        <div class="webgl-fallback">
          <div class="webgl-fallback-card">
            ...
            <h2 style="margin:0 0 0.6rem;">Mercury viewer could not start.</h2>
    ...
    function moveCameraToFeature(feature, camera, controls, options = {}) {
      ...
      if (parentMoon) {
        const lat = feature.lat !== undefined ? feature.lat : feature.anchor_lat;
        const lon = feature.lon !== undefined ? feature.lon : feature.anchor_lon;
        if (!parentMoon || !mercurySceneGroup || !Array.isArray(parentMoon.moon_anchor) || lat === undefined || lon === undefined) {
          return;
        }
        ...
        const target = mercurySceneGroup.localToWorld(targetLocal.clone());
        const moonCenter = mercurySceneGroup.localToWorld(moonAnchor.clone());
      ...
      if (Array.isArray(feature.ring_anchor)) {
        ...
        if (mercurySceneGroup) {
          target = mercurySceneGroup.localToWorld(target.clone());
        }
        ...
        const _spinDelta = getSpinTime() * (2 * Math.PI / _MERCURY_DISPLAY_PERIOD_MS);
    ...
    function openGeoPopup(feature, worldPos, clickSpinDelta) {
      ...
      if (worldPos) {
        const localPoint = mercurySceneGroup.worldToLocal(worldPos.clone());
        const spinDelta = (clickSpinDelta !== undefined) ? clickSpinDelta : (mercuryGlobeRef ? mercuryGlobeRef.rotation.y - Math.PI : 0);
    ...
    function buildCutawayInterior(radius, layerData, elevationMap, terrainRelief) {
      ...
      const MERCURY_INNER_CORE_RADIUS = 0.600;  // solid iron inner core (~1450 km, MESSENGER/BepiColombo estimates)
      const MERCURY_OUTER_CORE_RADIUS = 0.830;  // liquid iron-sulfide outer core (~2020 km outer boundary)
      const MERCURY_MANTLE_RADIUS = 0.998;  // silicate mantle, near-crust boundary
    ...
    function buildMercurySolidInterior(radius) {
      // Unused for rocky planet — Mercury interior shown via cutaway only.
    ...
    function latLonToVector3(latDegrees, lonDegrees, radius) {
      ...
      const lon = THREE.MathUtils.degToRad(mercuryLonToSceneLon(lonDegrees));
    ...
    function isPointOccludedByPlanet(pointLocal, mercuryGroup, camera, bodyRadius = 3.2) {
      const inverseWorld = mercuryGroup.matrixWorld.clone().invert();
    ...
    function getMoonOccluders() {
      if (!mercurySceneGroup || !Array.isArray(moonData) || moonData.length === 0) {
        return [];
      }
      const scale = mercurySceneGroup.scale;
      ...
      return moonData
        ...
        .map((item) => ({
          name: item.name,
          center: mercurySceneGroup.localToWorld(new THREE.Vector3(
            item.moon_anchor[0],
            item.moon_anchor[1],
            item.moon_anchor[2],
          )),
    ...
    function updateMoonFeatureLabelVisibility(entries, mercuryGroup, camera, renderer, activeMoonFeature, volcanicEnabled = true) {
      ...
      for (const entry of entries) {
        ...
        moonCenterWorld.copy(entry.moonAnchor).applyMatrix4(mercuryGroup.matrixWorld);
        surfaceWorldPosition.copy(entry.surfacePoint).applyMatrix4(mercuryGroup.matrixWorld);
        ...
        if (projected.z < -1 || projected.z > 1) {
          ...
          const localRepositioned = mercuryGroup.worldToLocal(repositioned.clone());
    ...
    function updateMoonVisibility(entries, mercuryGroup, camera, renderer, moonsEnabled, labelsEnabled) {
      const projected = new THREE.Vector3();
      ...
      for (const entry of entries) {
        ...
        if (!isMoonViewerFocus && isPointOccludedByPlanet(entry.moonMesh.position, mercuryGroup, camera)) {
          entry.moonMesh.visible = false;
          continue;
        }

        moonWorldPosition.copy(entry.moonMesh.position).applyMatrix4(mercuryGroup.matrixWorld);
        ...
        spriteWorldPosition.copy(entry.sprite.position).applyMatrix4(mercuryGroup.matrixWorld);
    ...
    function updateLabelVisibility(
      ...
      const occupiedRects = [];
      const candidates = [];
      mercuryGroup.getWorldPosition(groupWorldPosition);
      ...
      const placeMosaicLabel = (entry, metrics, anchorProjected, occupiedRectsLocal) => {
        const spriteParent = entry.sprite.parent || mercuryGroup;
      ...
      for (const entry of entries) {
        ...
        if (baseScale) {
          ...
          // In moon viewer mode Mercury is small and distant — scale labels down so they
          ...
          if (entry.markerBaseScale) {
            ...
            if (useMosaicCloseLayout && scaleBarMeters <= 2000) {
              ...
              const targetMarkerRadiusWorld = (targetMarkerDiameterMeters * 0.5) * (3.2 / MERCURY_RADIUS_METERS);
        ...
        if (useMosaicCloseLayout) {
          ...
          // wide of the (now-distant) Mercury globe and visually dominate the scene.
    ...
    class CTXCanvasLayer {
      ...
      constructor(baseTexture, serviceConfig = {}, canvasWidth = 8192, canvasHeight = 4096) {
        ...
        this.TILE_BASE = serviceConfig.tileBase || "https://astro.arcgis.com/arcgis/rest/services/OnMercury/CTX/MapServer/tile";
      ...
      update(camera) {
        ...
        if (this.focusOverlayEnabled) {
          ...
          const focusBbox = (focusStage && focusTarget)
            ...
            : null;
          const scaleEstimate = estimateBodyMapScale(camera, this._globe, MERCURY_RADIUS_METERS, this.GLOBE_R);
          ...
          if (baseLayerSelect.value === "ctx-mosaic" || baseLayerSelect.value === "ctx-mosaic-color") {
            const settleMs = 350;
            const scaleGate = estimateBodyMapScale(camera, this._globe, MERCURY_RADIUS_METERS, this.GLOBE_R);
            ...
            if (scaleGate && focusTarget) {
              ...
              const screenWidth = Math.max(window.innerWidth * dpr, 1);
              const screenHeight = Math.max(window.innerHeight * dpr, 1);
              const metersPerDeg = (2 * Math.PI * MERCURY_RADIUS_METERS) / 360;
    ...
    class CTXDetailPatchStreamer {
      constructor(mercuryGroup, tileBase, getTilePayload, getTileLevelCap, getViewBbox, getFocusTarget, elevationSampler, getTerrainRelief, anisotropy = 1) {
        this.group = new THREE.Group();
        this.group.visible = false;
        mercuryGroup.add(this.group);
    ...
    async function init() {
      const startup = window.__mercuryViewerStartup;
      ...
      function getActiveZoomContext() {
        ...
        if (activeMoonViewerFeature) {
          ...
          const centerWorld = mercuryGroup.localToWorld(moonContext.centerLocal.clone());
          return {
            centerWorld,
        ...
        mercuryGroup.getWorldPosition(wheelZoomBodyCenter);
      ...
      function estimateScaleBarMetersForCameraPosition(position, bodyMesh = globe, bodyRadiusMeters = MERCURY_RADIUS_METERS, bodyRadiusScene = 3.2) {
      ...
      mercurySceneGroup = mercuryGroup;
      ...
      setStatus("Loading Mercury textures...");
      ...
      const baseLayers = manifest.layers || [{
        id: "viking-color",
        label: "Mercury Color Map - Viking",
      ...
      const geologyLayers = manifest.geology_layers || [{
        id: "sim3292-units",
        label: "USGS, Geologic Map of Mercury",
        path: manifest.geology.path,
        description: "USGS, Geologic Map of Mercury geology overlay sourced from Scientific Investigations Map 3292.",
        default: true,
      }];
      ...
      const SC_LAYERS = [
        { id: "sc-temperature",  label: "Temperature",              path: "assets/mercury_sc_temperature.png",        scGroup: "Surface Conditions — Atmospheric", scParamKey: "temperature" },
        { id: "sc-pressure",     label: "Pressure",                 path: "assets/mercury_sc_pressure.png",           scGroup: "Surface Conditions — Atmospheric", scParamKey: "pressure" },
        { id: "sc-wind",         label: "Wind Speed",               path: "assets/mercury_sc_wind.png",               scGroup: "Surface Conditions — Atmospheric", scParamKey: "wind" },
        { id: "sc-irradiance",   label: "Solar Irradiance",         path: "assets/mercury_sc_irradiance.png",         scGroup: "Surface Conditions — Atmospheric", scParamKey: "irradiance" },
        { id: "sc-radiation",    label: "Radiation Dose",           path: "assets/mercury_sc_radiation.png",          scGroup: "Surface Conditions — Atmospheric", scParamKey: "radiation" },
        { id: "sc-diurnal",      label: "Diurnal Temp Range",       path: "assets/mercury_sc_diurnal.png",            scGroup: "Surface Conditions — Atmospheric", scParamKey: "diurnal" },
        { id: "sc-atm_density",  label: "Atmospheric Density",      path: "assets/mercury_sc_atm_density.png",        scGroup: "Surface Conditions — Atmospheric", scParamKey: "atm_density" },
        { id: "sc-sound_speed",  label: "Speed of Sound",           path: "assets/mercury_sc_sound_speed.png",        scGroup: "Surface Conditions — Atmospheric", scParamKey: "sound_speed" },
        { id: "sc-co2_frost",    label: "CO₂ Frost Probability",    path: "assets/mercury_sc_co2_frost.png",          scGroup: "Surface Conditions — Atmospheric", scParamKey: "co2_frost" },
        { id: "sc-slope",        label: "Slope Gradient",           path: "assets/mercury_sc_slope.png",              scGroup: "Surface Conditions — Terrain",      scParamKey: "slope" },
        { id: "sc-roughness",    label: "Terrain Roughness",        path: "assets/mercury_sc_roughness.png",          scGroup: "Surface Conditions — Terrain",      scParamKey: "roughness" },
        { id: "sc-ice_depth",    label: "Permafrost Thickness",     path: "assets/mercury_sc_ice_depth.png",          scGroup: "Surface Conditions — Habitability", scParamKey: "ice_depth" },
        { id: "sc-dust_devil",   label: "Dust Devil Susceptibility",path: "assets/mercury_sc_dust_devil.png",         scGroup: "Surface Conditions — Habitability", scParamKey: "dust_devil" },
        { id: "sc-landing",      label: "Landing Zone Score",       path: "assets/mercury_sc_landing_score.png",      scGroup: "Surface Conditions — Habitability", scParamKey: "landing_score" },
        { id: "sc-solar",        label: "Solar Panel Output",       path: "assets/mercury_sc_solar_output.png",       scGroup: "Surface Conditions — Habitability", scParamKey: "solar_output" },
        { id: "sc-brine",        label: "Brine Stability",          path: "assets/mercury_sc_brine_stability.png",    scGroup: "Surface Conditions — Habitability", scParamKey: "brine_stability" },
        { id: "sc-magnetic",     label: "Magnetic Shielding",       path: "assets/mercury_sc_magnetic_shielding.png", scGroup: "Surface Conditions — Habitability", scParamKey: "magnetic_shielding" },
        { id: "sc-habitability", label: "Human Habitability",       path: "assets/mercury_sc_habitability.png",       scGroup: "Surface Conditions — Habitability", scParamKey: "habitability" },
      ];
      ...
      if (elevationMap) {
        ...
        const ELEVATION_DEM_LAYER = {
          id: "elevation-dem",
          label: "Elevation DEM",
          description: "Mercury global elevation raster (DEM) shown directly as the basemap.",
        };
      ...
      mercuryGroup.add(globe);
      mercuryGlobeRef = globe;
      locatorViewState.camera = camera;
      locatorViewState.mercuryGroup = mercuryGroup;
...
uniform float uViewportWidth;`,
      ...
      mercuryGroup.add(ctxFocusGlobe);
      ...
      const mercurySolidInterior = buildMercurySolidInterior(3.2);
      const mercurySolidInteriorGroup = mercurySolidInterior.group;
      mercuryGroup.add(mercurySolidInteriorGroup);
      const mercuryInteriorSphereMaterials = [
        mercurySolidInterior.metallicHydrogenMaterial,
        mercurySolidInterior.heavyElementCoreMaterial,
      ].filter(Boolean);
      const mercuryInteriorCapMaterials = [
        mercurySolidInterior.metallicHydrogenCapMaterial,
        mercurySolidInterior.heavyElementCoreCapMaterial,
      ].filter(Boolean);
      ...
      mercuryGroup.scale.set(1, planetBodyScaleY, 1);
      applyPlanetViewMode = function applyPlanetViewModeImpl(mode = "tilted", resetCamera = false) {
        ...
        mercuryGroup.rotation.z = tiltRad;
      ...
      const GIS_BOOKMARK_STORAGE_KEY = "mercury-gis-bookmarks-v1";
      const GIS_STUDY_AREA_STORAGE_KEY = "mercury-gis-study-areas-v1";
      const GIS_BUFFER_STORAGE_KEY = "mercury-gis-buffers-v1";
      const GIS_BASE_STORAGE_KEY = "mercury-gis-bases-v1";
      ...
      mercuryGroup.add(labelLayer.group);
      ...
      mercuryGroup.add(baseSiteLayer.group);
      ...
      mercuryGroup.add(geologyContactLayer.group);
      ...
      mercuryGroup.add(geologyStructureLayer.group);
      ...
      if (geologyOutlineState) {
        ...
        mercuryGroup.add(geologyOutlineMesh);
    ...
    function isObjectActuallyVisible(object) {
      ...
      function projectBaseLocal(point, origin) {
        ...
        const x = THREE.MathUtils.degToRad(normalizeLongitudeDegrees(lon - lon0)) * MERCURY_MEAN_RADIUS_KM * 1000 * Math.cos(THREE.MathUtils.degToRad(lat0));
        const y = THREE.MathUtils.degToRad(lat - lat0) * MERCURY_MEAN_RADIUS_KM * 1000;
        return { x, y };
      }
      ...
      function unprojectBaseLocal(x, y, origin) {
        const lat0 = Number(origin.lat);
        const lon0 = normalizeLongitudeDegrees(Number(origin.lon));
        const lat = lat0 + THREE.MathUtils.radToDeg(y / (MERCURY_MEAN_RADIUS_KM * 1000));
        const lon = lon0 + THREE.MathUtils.radToDeg(x / ((MERCURY_MEAN_RADIUS_KM * 1000) * Math.max(Math.cos(THREE.MathUtils.degToRad(lat0)), 0.0001)));
      ...
      function baseLabelItem(base) {
        return {
          ...
          name: base.name,
          type: "Base site",
          description: `Mercury base boundary covering ${Number(base.areaKm2 || 0).toFixed(1)} km².`,
      ...
      function rebuildBaseSiteLayerInstance() {
        if (!baseSiteLayer || !mercuryGroup) {
          return;
        }
        mercuryGroup.remove(baseSiteLayer.group);
      ...
      function renderActiveBaseOverlay() {
        ...
        if (vertices.length >= 3) {
          const fillMesh = buildMercuryPolygonFillMesh(vertices);
          if (fillMesh) {
          ...
          const boundaryPoints = buildMercurySurfacePolyline(vertices, true, 0.02);
        ...
        for (const building of activeBase.buildings || []) {
          ...
          const mesh = buildMercuryPolygonFillMesh(footprintVertices);
          if (mesh) {
          ...
          const linePoints = buildMercurySurfacePolyline(footprintVertices, true, 0.024);
      ...
      function passesQueryFilters(candidate, mineralContext) {
        ...
        let distanceKm = null;
        if (nearbyKm !== null && gisInspectPoint?.bodyName === "Mercury") {
      ...
      function currentMeasurementGeoJSON() {
        ...
        if ((measureMode === "distance" || measureMode === "profile" || measureMode === "route") && measurePoints.length >= 2) {
          ...
          return {
            ...
            properties: {
              tool: measureMode,
              body: measurePoints[0].bodyName || "Mercury",
      ...
      function sampleExtractionRecord(lat, lon, columnSet) {
        ...
        if (columnSet.has("temperature")) record.temperature_c = estimateMercuryTemperature(lat, elevation || 0);
        if (columnSet.has("pressure")) record.pressure_pa = estimateMercuryPressure(elevation || 0);
        if (columnSet.has("wind")) record.wind_m_s = estimateMercuryWindSpeed(lat, elevation || 0);
        if (columnSet.has("irradiance")) record.irradiance_w_m2 = estimateMercuryIrradiance(lat, elevation || 0);
        if (columnSet.has("radiation")) record.radiation_msv_day = estimateMercuryRadiation(elevation || 0);
      ...
      function exportPolygonSampleCsv(sourceType, stepKm, selectedColumns) {
        ...
        downloadCsv(`mercury_${geometry.name}_extract.csv`, [
          headers,
      ...
      function renderGisBufferOverlay() {
        ...
        const fillMesh = buildMercuryPolygonFillMesh(gisBufferState.vertices);
        if (fillMesh) {
          gisBufferGroup.add(fillMesh);
        }
        const boundaryPoints = buildMercurySurfacePolyline(gisBufferState.vertices, true, 0.018);
        if (boundaryPoints.length >= 2) {
          const boundaryLine = new THREE.Line(
            new THREE.BufferGeometry().setFromPoints(boundaryPoints),
            new THREE.LineBasicMaterial({
      ...
      function generateGisBuffer() {
        ...
        const bufferPoints = vertices.map((point) => ({
          lat: point.lat,
          lon: point.lon,
          radiusKm: MERCURY_MEAN_RADIUS_KM,
        }));
      ...
      function focusActiveBase() {
        ...
        moveCameraToFeature({
          ...
          lat: base.center.lat,
          lon: base.center.lon,
          description: `Mercury base boundary covering ${base.areaKm2.toFixed(1)} km².`,
          theme: "mission",
      ...
      function syncGisBufferSection() {
        const canInspect = Boolean(gisInspectPoint && gisInspectPoint.bodyName === "Mercury");
      ...
      function syncGisQuerySection() {
        if (gisQueryNearbyKm) gisQueryNearbyKm.disabled = !(gisInspectPoint && gisInspectPoint.bodyName === "Mercury");
      ...
      function syncGisBaseSection() {
        ...
        if (gisBaseCreatePreset) gisBaseCreatePreset.disabled = !(gisInspectPoint && gisInspectPoint.bodyName === "Mercury");
      ...
      function captureGisInspect(surfaceHit) {
        ...
        if (surfaceHit.context?.kind === "moon") {
          ...
          const localPoint = mercuryGroup.worldToLocal(surfaceHit.point.clone());
      ...
      function exportCurrentMeasurementCsv() {
        ...
        if (measureMode === "distance" && measurePoints.length >= 2) {
          ...
          downloadCsv("mercury_distance_measurement.csv", [
        ...
        if (measureMode === "area" && measurePoints.length >= 3) {
          ...
          downloadCsv("mercury_area_measurement.csv", rows);
          return;
        }
        if (measureMode === "profile" && measureProfileSamples.length >= 2) {
          ...
          downloadCsv("mercury_profile_measurement.csv", rows);
          return;
        }
        if (measureMode === "route" && measurePoints.length >= 2) {
          ...
          downloadCsv("mercury_route_measurement.csv", rows);
        }
      }
      if (gisInspectButton) {
      ...
      if (gisStudyExportGeoJsonButton) {
        gisStudyExportGeoJsonButton.addEventListener("click", () => {
          ...
          downloadJson("mercury_study_area.geojson", { type: "FeatureCollection", features: [feature] });
        });
      }
      ...
      if (gisBufferExport) {
        gisBufferExport.addEventListener("click", () => {
          ...
          downloadJson("mercury_buffer_zone.geojson", { type: "FeatureCollection", features: [feature] });
        });
      }
      if (gisBufferClear) {
      ...
      if (gisExportPointButton) {
        gisExportPointButton.addEventListener("click", () => {
          ...
          downloadCsv("mercury_inspect_point.csv", [
      ...
      if (gisExportStudyButton) {
        gisExportStudyButton.addEventListener("click", () => {
          ...
          downloadJson("mercury_study_geometry.geojson", {
      ...
      function parseMoonRadiusKm(feature) {
        ...
        return Number.isFinite(value) && value > 0 ? value : MERCURY_MEAN_RADIUS_KM;
      }
      ...
      function normalizeMeasureHitLocalPoint(localPoint, context) {
        ...
        // measurement overlay rotate with Mercury so markers stay surface-locked.
      ...
      function refineMeasureHitLocalPoint(localPoint, context) {
        if (context.kind === "moon") {
          // Convert mercuryGroup-local hit to moon body frame so markers stay surface-locked
      ...
      function intersectAnySurface(clientX, clientY) {
        ...
        for (const entry of moonLayer.entries) {
          ...
          if (moonHit) {
            ...
            if (ctx) {
              const localPoint = mercuryGroup.worldToLocal(moonHit.point.clone());
      ...
      function intersectMercurySurface(clientX, clientY) {
        const rect = renderer.domElement.getBoundingClientRect();
        pointer.x = ((clientX - rect.left) / rect.width) * 2 - 1;
        pointer.y = -(((clientY - rect.top) / rect.height) * 2 - 1);
        raycaster.setFromCamera(pointer, camera);
      ...
      function intersectMeasurementSurface(clientX, clientY) {
        ...
        // land on the Mercury globe visible in the background.
        if (activeMoonViewerFeature && hit.context?.kind !== "moon") {
          return null;
        }
        const context = hit.context || getActiveMeasureContext();
        const hitLocalPoint = hit.localPoint ? hit.localPoint.clone() : mercuryGroup.worldToLocal(hit.point.clone());
        ...
        if (measureMode === "distance" && measurePoints.length >= 2) {
          ...
          if (routeMetric) {
            routeMetric.innerHTML = [
              `<strong>${measurePoints[0].bodyName || "Mercury"} route</strong>: ${routeProfile.totalDistanceKm.toFixed(1)} km`,
          ...
          profileCanvas.hidden = true;
          showProfileModal(
            `${measurePoints[0].bodyName || "Mercury"} Route Profile`,
          ...
          profileCanvas.hidden = true;
          showProfileModal(
            `${measurePoints[0].bodyName || "Mercury"} Elevation Profile`,
      ...
      if (elevationSampler) {
        ...
        mercuryGroup.add(regionMaskGlobe);
      } else {
      ...
      function applyPlanetDisplayState() {
        ...
        const mercuryLabelsEnabled = labelsToggle.checked;
        globe.visible = true;
        ...
        mercurySolidInteriorGroup.visible = false; // never shown for rocky planets
        if (mercurySolidInterior.metallicHydrogenCapMesh) {
          mercurySolidInterior.metallicHydrogenCapMesh.visible = false;
        }
        if (mercurySolidInterior.heavyElementCoreCapMesh) {
          mercurySolidInterior.heavyElementCoreCapMesh.visible = false;
        }
        for (const material of mercuryInteriorSphereMaterials) {
          material.clippingPlanes = [];
          material.needsUpdate = true;
        }
        for (const material of mercuryInteriorCapMaterials) {
          material.clippingPlanes = [];
          material.needsUpdate = true;
        }
        if (baseMaterial) {
      ...
      if (csvPlotterExportPng) {
        csvPlotterExportPng.addEventListener("click", () => {
          if (csvPlotterCanvas) exportCanvasPng(csvPlotterCanvas, "mercury_csv_plot.png");
        });
      }
      ...
      renderer.domElement.addEventListener("pointermove", (event) => {
        const _rawSurfaceHit = measureMode
          ...
          : (() => {
              ...
              // In moon-viewer mode ignore background Mercury globe hits for cursor readout
        ...
        if (coreToggle.checked) {
          ...
          if (activeCutClipPlane && icDepth) {
            ...
            if (clipIntersected) {
              const localHit = mercuryGroup.worldToLocal(coreHit.clone());
              const rScene = localHit.length();
              const GLOBE_RADIUS_SCENE = 3.2;
              const MERCURY_RADIUS_KM = 3389.5;
              if (rScene <= GLOBE_RADIUS_SCENE) {
                const rFrac = rScene / GLOBE_RADIUS_SCENE;
                const depthKm = Math.round((1.0 - rFrac) * MERCURY_RADIUS_KM);
                const layerName = mercuryInteriorLayerName(rFrac);
                const tempC = estimateMercuryInteriorTemperature(rFrac);
                const pressureGPa = estimateMercuryInteriorPressure(rFrac);
                icDepth.textContent = `${depthKm.toLocaleString()} km`;
                icDepth.style.color = "";
                icLayer.textContent = layerName;
                icLayer.style.color = mercuryInteriorLayerColor(layerName);
                icTemp.textContent = `${tempC > 0 ? "+" : ""}${tempC} °C`;
                icTemp.style.color = mercuryInteriorTempColor(tempC);
                icPressure.textContent = `${pressureGPa} GPa`;
                icPressure.style.color = mercuryInteriorPressureColor(pressureGPa);
              } else {
        ...
        if (priorityHit) {
          ...
          const geologySurfaceHit = geologyToggle.checked ? intersectMercurySurface(event.clientX, event.clientY) : null;
          if (geologySurfaceHit) {
            // worldToLocal undoes mercuryGroup.rotation.z (axial tilt); then un-spin globe.rotation.y
            const hoverLocalHit = mercuryGroup.worldToLocal(geologySurfaceHit.point.clone());
      ...
      renderer.domElement.addEventListener("pointerup", (event) => {
        ...
        if (gisBasePlacementMode) {
          const baseSurfaceHit = intersectMercurySurface(event.clientX, event.clientY);
          if (baseSurfaceHit) {
            const localPoint = mercuryGroup.worldToLocal(baseSurfaceHit.point.clone());
        ...
        const surfaceHit = geologyToggle.checked ? intersectMercurySurface(event.clientX, event.clientY) : null;
        if (surfaceHit) {
          // worldToLocal undoes mercuryGroup.rotation.z (axial tilt); then un-spin globe.rotation.y
          const clickLocalHit = mercuryGroup.worldToLocal(surfaceHit.point.clone());
      ...
      function sampleFeatureAtViewport(nx, ny) {
        ...
        const surfaceHit = intersectMercurySurface(clientX, clientY);
        let geologyFeature = null;
        if (surfaceHit) {
          const sampleLocalHit = mercuryGroup.worldToLocal(surfaceHit.point.clone());
      ...
      async function runEmbeddedSmokeTest() {
        ...
        try {
          let state = window.__mercuryViewerDebug.getState();
          assert(state.solidGeology === true, "Solid geology should load enabled by default.");

          const feature = window.__mercuryViewerDebug.openFeatureAtViewport(0.5, 0.5);
          ...
          state = window.__mercuryViewerDebug.getState();
          assert(state.selectedOutlineVisible === true, "Selected geology outline should be visible.");

          window.__mercuryViewerDebug.setToggle("geology-toggle", false);
          window.__mercuryViewerDebug.setToggle("geology-contacts-toggle", true);
          window.__mercuryViewerDebug.setToggle("geology-structures-toggle", true);
          state = window.__mercuryViewerDebug.getState();
          ...
          const radiusBefore = window.__mercuryViewerDebug.getFirstLineRadius("contacts");
          assert(radiusBefore !== null, "Expected a visible contact line sample radius.");
          window.__mercuryViewerDebug.setTerrainRelief(0.2);
          await new Promise((resolve) => setTimeout(resolve, 250));
          const radiusAfter = window.__mercuryViewerDebug.getFirstLineRadius("contacts");
          assert(radiusAfter !== null && Math.abs(radiusAfter - radiusBefore) > 1e-4, "Contact linework should move with terrain relief.");

          window.__mercuryViewerDebug.setToggle("core-toggle", true);
          state = window.__mercuryViewerDebug.getState();
...
${error && error.message ? error.message : error}`;
      ...
      function updateMoonOrbits(moonEntries, featureEntries, t) {
        ...
        for (const entry of moonEntries) {
          ...
          const omega = _periodDays
            ? (2 * Math.PI * _MERCURY_MOON_SPEED_FACTOR) / (_periodDays * 86400000)
      ...
      function render() {
        ...
        if (!activeMoonViewerFeature) {
          ...
          if (moonControlContext) {
            const moonCenterWorld = mercuryGroup.localToWorld(moonControlContext.centerLocal.clone());
        ...
        if (!coreToggle.checked) {
          ...
          globe.rotation.y = Math.PI + _spinT * (2 * Math.PI / _MERCURY_DISPLAY_PERIOD_MS);
        ...
        if (activeGeoPopupLocalPos && activeGeoPopupFeature) {
          try {
            const worldPos = mercuryGroup.localToWorld(
              activeGeoPopupLocalPos.clone().applyEuler(new THREE.Euler(0, globe.rotation.y - Math.PI, 0)),
            );
            const planetCenterWorld = mercuryGroup.localToWorld(new THREE.Vector3(0, 0, 0));
        ...
        if (_heavyFrame) {
          ...
          let scaleBodyRadiusMeters = MERCURY_RADIUS_METERS;
    ...
    (function () {
      const audioEl = document.getElementById("mercury-audio-el");
