import * as THREE from "./vendor/three.module.js";
    ...
    console.log("[ctxPatchDebug] planet-viewer.js loaded", { manifest: window.__planetViewerManifest, verbose: window.__ctxPatchDebug.verbose });

    const manifest = window.__planetViewerManifest;
    ...
    const _PLANET_DISPLAY_PERIOD_MS = 600000; // 10 min visual rotation
    const _PLANET_ROT_REAL_MS = 24.6228 * 3600000; // 88,642,080 ms
    const _PLANET_MOON_SPEED_FACTOR = _PLANET_ROT_REAL_MS / _PLANET_DISPLAY_PERIOD_MS; // ≈ 147.74×
    ...
    if (brandLogo) {
      brandLogo.src = "../../../assets/planet_icon.png?v=20260329d";
    }
    ...
    function estimatePlanetTemperature(latDeg, elevMeters) {
      const latRad = latDeg * Math.PI / 180;
      const baseTempC = -20 - 80 * Math.sin(latRad) ** 2;
      return Math.round(baseTempC - 2.5 * (elevMeters / 1000));
    }

    function estimatePlanetPressure(elevMeters) {
    ...
    const PLANET_INTERIOR_T_PTS = [[0.000, 2100], [0.320, 1900], [0.540, 1500], [0.985, 700], [1.000, -50]];
    const PLANET_INTERIOR_P_PTS = [[0.000, 40.0], [0.320, 37.0], [0.540, 23.0], [0.985,  1.2], [1.000,   0]];
    ...
    function estimatePlanetInteriorTemperature(rFrac) {
      return Math.round(_interiorInterp(PLANET_INTERIOR_T_PTS, rFrac));
    }

    function estimatePlanetInteriorPressure(rFrac) {
      return Math.round(_interiorInterp(PLANET_INTERIOR_P_PTS, rFrac) * 10) / 10;
    }

    function planetInteriorLayerName(rFrac) {
      for (const layer of PLANET_INTERIOR_LAYERS) {
        if (rFrac >= layer.rMin && rFrac <= layer.rMax) return layer.name;
      }
      return "Unknown";
    }

    const PLANET_INTERIOR_LAYER_COLORS = {
      "Crust":             "#d4b896",
      "Mantle":            "#c07848",
    ...
    function planetInteriorLayerColor(layerName) {
      return PLANET_INTERIOR_LAYER_COLORS[layerName] ?? "#f6dcc8";
    }

    function planetInteriorTempColor(tempC) {
    ...
    function planetInteriorPressureColor(gpa) {
    ...
    function estimatePlanetWindSpeed(latDeg, elevMeters) {
      const latRad = latDeg * Math.PI / 180;
    ...
    function estimatePlanetIrradiance(latDeg, elevMeters) {
      const latRad = latDeg * Math.PI / 180;
    ...
    function estimatePlanetRadiation(elevMeters) {
      return Math.round((0.5 * Math.exp(elevMeters / 14000) + 0.2) * 100) / 100;
    }

    function estimatePlanetDiurnalRange(latDeg, elevMeters) {
      const latRad = latDeg * Math.PI / 180;
    ...
    function estimateSpaceTemperature(cameraDistanceFromPlanet) {
      ...
      const t = Math.max(0, Math.min(1, (cameraDistanceFromPlanet - 3.5) / 30));
    ...
    const SC_PARAMS = {
      temperature: {
        ...
        compute: (lat, elev) => estimatePlanetTemperature(lat, elev),
      ...
      pressure: {
        ...
        compute: (_lat, elev) => estimatePlanetPressure(elev),
      ...
      wind: {
        ...
        compute: (lat, elev) => estimatePlanetWindSpeed(lat, elev),
      ...
      irradiance: {
        ...
        compute: (lat, elev) => estimatePlanetIrradiance(lat, elev),
      ...
      radiation: {
        ...
        compute: (_lat, elev) => estimatePlanetRadiation(elev),
      ...
      diurnal: {
        ...
        compute: (lat, elev) => estimatePlanetDiurnalRange(lat, elev),
    ...
    let planetSceneGroup = null;
    let planetGlobeRef = null;   // set by init(); bridged so module-level openGeoPopup can access globe rotation
    let moonLayer = null;
    let gisBases = [];
    ...
    const PLANET_MEAN_RADIUS_KM = 3389.5;
    ...
    const CTX_SERVICE_CANDIDATES = [
      {
        name: "CTX",
        serviceUrl: "https://astro.arcgis.com/arcgis/rest/services/OnPlanet/CTX/MapServer",
      },
      {
        name: "CTX1",
        serviceUrl: "https://astro.arcgis.com/arcgis/rest/services/OnPlanet/CTX1/MapServer",
      },
    ];
    ...
    function createContourLineLayer(THREERef, planetGroup, elevationSampler, getTerrainRelief, radius = 3.2) {
    ...
    function greatCircleDistanceKm(a, b) {
      const radiusKm = a.radiusKm || b.radiusKm || PLANET_MEAN_RADIUS_KM;
    ...
    function sphericalPolygonAreaKm2(points) {
      ...
      const radiusKm = points[0]?.radiusKm || PLANET_MEAN_RADIUS_KM;
    ...
    function estimateSurfaceSlopeDegrees(elevationSampler, latDegrees, lonDegrees, radiusKm = PLANET_MEAN_RADIUS_KM) {
      if (!elevationSampler) {
        return null;
      }
      const latStep = 0.08;
    ...
    const PLANET_RADIUS_METERS = 3396190;
    ...
    function destinationPoint(latDegrees, lonDegrees, bearingDegrees, distanceKm, radiusKm = PLANET_MEAN_RADIUS_KM) {
    ...
    function projectLatLonToLocalKm(point, origin, radiusKm = PLANET_MEAN_RADIUS_KM) {
    ...
    function buildPlanetSurfacePolyline(vertices, closed = false, lift = 0.014) {
    ...
    function buildPlanetPolygonFillMesh(vertices) {
      if (!Array.isArray(vertices) || vertices.length < 3) {
        return null;
      }
      const boundary = buildPlanetSurfacePolyline(vertices, true, 0.012);
    ...
    function exportCurrentProfilePng() {
      ...
      const baseName = String(currentProfilePlotState.title || "planet_elevation_profile")
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "_")
        .replace(/^_+|_+$/g, "") || "planet_elevation_profile";
    ...
    function activateMoonViewer(feature, camera, controls) {
      if (!isMoonFeature(feature) || !planetSceneGroup) {
        return;
      }
      cancelCameraFlight();
      deactivateTourMode();
      ...
      const target = planetSceneGroup.localToWorld(localTarget.clone());
    ...
    function moveCameraToFeature(feature, camera, controls, options = {}) {
      ...
      if (parentMoon) {
        const lat = feature.lat !== undefined ? feature.lat : feature.anchor_lat;
        const lon = feature.lon !== undefined ? feature.lon : feature.anchor_lon;
        if (!parentMoon || !planetSceneGroup || !Array.isArray(parentMoon.moon_anchor) || lat === undefined || lon === undefined) {
          return;
        }
        ...
        const target = planetSceneGroup.localToWorld(targetLocal.clone());
        const moonCenter = planetSceneGroup.localToWorld(moonAnchor.clone());
      ...
      if (Array.isArray(feature.ring_anchor)) {
        ...
        if (planetSceneGroup) {
          target = planetSceneGroup.localToWorld(target.clone());
        }
    ...
    function openGeoPopup(feature, worldPos, clickSpinDelta) {
      ...
      if (worldPos) {
        const localPoint = planetSceneGroup.worldToLocal(worldPos.clone());
        const spinDelta = (clickSpinDelta !== undefined) ? clickSpinDelta : (planetGlobeRef ? planetGlobeRef.rotation.y - Math.PI : 0);
    ...
    function buildCutawayInterior(radius, layerData, elevationMap, terrainRelief) {
      ...
      const PLANET_INNER_CORE_RADIUS = 0.32;    // hypothetical dense inner core
      const PLANET_OUTER_CORE_RADIUS = 0.54;    // liquid iron-sulfide core (InSight)
      const PLANET_MANTLE_RADIUS = 0.985;       // mantle, near-crust boundary
      
      // ── Inner core: dense crystalline metallic interior ───────────────────
      const innerCoreMesh = new THREE.Mesh(
        new THREE.SphereGeometry(radius * PLANET_INNER_CORE_RADIUS, 96, 96, phiStart, phiLength),
      ...
      const outerCoreMesh = new THREE.Mesh(
        new THREE.SphereGeometry(radius * PLANET_OUTER_CORE_RADIUS, 128, 128, phiStart, phiLength),
        new THREE.ShaderMaterial({
          uniforms: {},
          vertexShader: LAYER_VERT,
      ...
      const mantleMesh = new THREE.Mesh(
        new THREE.SphereGeometry(radius * PLANET_MANTLE_RADIUS, 128, 128, phiStart, phiLength),
        new THREE.ShaderMaterial({
          uniforms: {},
          vertexShader: LAYER_VERT,
      ...
      const mantleInnerBoundaryMesh = new THREE.Mesh(
        new THREE.SphereGeometry(radius * (PLANET_OUTER_CORE_RADIUS + 0.002), 128, 128, phiStart, phiLength),
        new THREE.ShaderMaterial({
          uniforms: {},
          vertexShader: LAYER_VERT,
          fragmentShader: MANTLE_FRAG,
      ...
      const CAP_X = -0.012;
      const crustInnerRadius = radius * PLANET_MANTLE_RADIUS;
      ...
      const capDefs = [
        ...
        { outer: crustInnerRadius, inner: radius * PLANET_OUTER_CORE_RADIUS,  fragmentShader: MANTLE_SECTION_FRAG },  // molecular envelope ring
        { outer: radius * PLANET_OUTER_CORE_RADIUS,  inner: radius * PLANET_INNER_CORE_RADIUS,  fragmentShader: CORE_SECTION_FRAG },  // metallic hydrogen ring
      ];
      let crustRing = null;
      ...
      const fluidCapDisk = new THREE.Mesh(
        new THREE.CircleGeometry(radius * PLANET_OUTER_CORE_RADIUS, 128),
        fluidCapMat,
      );
      ...
      const innerCapDisk = new THREE.Mesh(
        new THREE.CircleGeometry(radius * PLANET_INNER_CORE_RADIUS, 96),
        innerCapMat,
      );
    ...
    function buildPlanetSolidInterior(radius) {
    ...
    function planetLonToSceneLon(lonDegrees) {
    ...
    function latLonToVector3(latDegrees, lonDegrees, radius) {
      ...
      const lon = THREE.MathUtils.degToRad(planetLonToSceneLon(lonDegrees));
    ...
    function isPointOccludedByPlanet(pointLocal, planetGroup, camera, bodyRadius = 3.2) {
      const inverseWorld = planetGroup.matrixWorld.clone().invert();
    ...
    function getMoonOccluders() {
      if (!planetSceneGroup || !Array.isArray(moonData) || moonData.length === 0) {
        return [];
      }
      const scale = planetSceneGroup.scale;
      ...
      return moonData
        ...
        .map((item) => ({
          name: item.name,
          center: planetSceneGroup.localToWorld(new THREE.Vector3(
            item.moon_anchor[0],
            item.moon_anchor[1],
            item.moon_anchor[2],
          )),
    ...
    function updateMoonFeatureLabelVisibility(entries, planetGroup, camera, renderer, activeMoonFeature, volcanicEnabled = true) {
      ...
      for (const entry of entries) {
        ...
        moonCenterWorld.copy(entry.moonAnchor).applyMatrix4(planetGroup.matrixWorld);
        surfaceWorldPosition.copy(entry.surfacePoint).applyMatrix4(planetGroup.matrixWorld);
        ...
        if (projected.z < -1 || projected.z > 1) {
          ...
          const localRepositioned = planetGroup.worldToLocal(repositioned.clone());
    ...
    function updateMoonVisibility(entries, planetGroup, camera, renderer, moonsEnabled, labelsEnabled) {
      const projected = new THREE.Vector3();
      ...
      for (const entry of entries) {
        ...
        if (!isMoonViewerFocus && isPointOccludedByPlanet(entry.moonMesh.position, planetGroup, camera)) {
          entry.moonMesh.visible = false;
          continue;
        }

        moonWorldPosition.copy(entry.moonMesh.position).applyMatrix4(planetGroup.matrixWorld);
        ...
        spriteWorldPosition.copy(entry.sprite.position).applyMatrix4(planetGroup.matrixWorld);
    ...
    function updateLabelVisibility(
      ...
      const occupiedRects = [];
      const candidates = [];
      planetGroup.getWorldPosition(groupWorldPosition);
      ...
      const placeMosaicLabel = (entry, metrics, anchorProjected, occupiedRectsLocal) => {
        const spriteParent = entry.sprite.parent || planetGroup;
        ...
        if (baseScale) {
          ...
          if (entry.markerBaseScale) {
            ...
            if (useMosaicCloseLayout && scaleBarMeters <= 2000) {
              ...
              const targetMarkerRadiusWorld = (targetMarkerDiameterMeters * 0.5) * (3.2 / PLANET_RADIUS_METERS);
    ...
    class CTXCanvasLayer {
      ...
      constructor(baseTexture, serviceConfig = {}, canvasWidth = 8192, canvasHeight = 4096) {
        ...
        this.TILE_BASE = serviceConfig.tileBase || "https://astro.arcgis.com/arcgis/rest/services/OnPlanet/CTX/MapServer/tile";
      ...
      update(camera) {
        ...
        if (this.focusOverlayEnabled) {
          ...
          const focusBbox = (focusStage && focusTarget)
            ...
            : null;
          const scaleEstimate = estimateBodyMapScale(camera, this._globe, PLANET_RADIUS_METERS, this.GLOBE_R);
          ...
          if (baseLayerSelect.value === "ctx-mosaic" || baseLayerSelect.value === "ctx-mosaic-color") {
            const settleMs = 350;
            const scaleGate = estimateBodyMapScale(camera, this._globe, PLANET_RADIUS_METERS, this.GLOBE_R);
            ...
            if (scaleGate && focusTarget) {
              ...
              const screenWidth = Math.max(window.innerWidth * dpr, 1);
              const screenHeight = Math.max(window.innerHeight * dpr, 1);
              const metersPerDeg = (2 * Math.PI * PLANET_RADIUS_METERS) / 360;
    ...
    class CTXDetailPatchStreamer {
      constructor(planetGroup, tileBase, getTilePayload, getTileLevelCap, getViewBbox, getFocusTarget, elevationSampler, getTerrainRelief, anisotropy = 1) {
    ...
    async function init() {
      const startup = window.__planetViewerStartup;
      ...
      function getActiveZoomContext() {
        ...
        if (activeMoonViewerFeature) {
          ...
          const centerWorld = planetGroup.localToWorld(moonContext.centerLocal.clone());
          return {
            centerWorld,
        ...
        planetGroup.getWorldPosition(wheelZoomBodyCenter);
      ...
      function estimateScaleBarMetersForCameraPosition(position, bodyMesh = globe, bodyRadiusMeters = PLANET_RADIUS_METERS, bodyRadiusScene = 3.2) {
      ...
      const SC_LAYERS = [
        { id: "sc-temperature",  label: "Temperature",              path: "assets/planet_sc_temperature.png",        scGroup: "Surface Conditions — Atmospheric", scParamKey: "temperature" },
        { id: "sc-pressure",     label: "Pressure",                 path: "assets/planet_sc_pressure.png",           scGroup: "Surface Conditions — Atmospheric", scParamKey: "pressure" },
        { id: "sc-wind",         label: "Wind Speed",               path: "assets/planet_sc_wind.png",               scGroup: "Surface Conditions — Atmospheric", scParamKey: "wind" },
        { id: "sc-irradiance",   label: "Solar Irradiance",         path: "assets/planet_sc_irradiance.png",         scGroup: "Surface Conditions — Atmospheric", scParamKey: "irradiance" },
        { id: "sc-radiation",    label: "Radiation Dose",           path: "assets/planet_sc_radiation.png",          scGroup: "Surface Conditions — Atmospheric", scParamKey: "radiation" },
        { id: "sc-diurnal",      label: "Diurnal Temp Range",       path: "assets/planet_sc_diurnal.png",            scGroup: "Surface Conditions — Atmospheric", scParamKey: "diurnal" },
        { id: "sc-atm_density",  label: "Atmospheric Density",      path: "assets/planet_sc_atm_density.png",        scGroup: "Surface Conditions — Atmospheric", scParamKey: "atm_density" },
        { id: "sc-sound_speed",  label: "Speed of Sound",           path: "assets/planet_sc_sound_speed.png",        scGroup: "Surface Conditions — Atmospheric", scParamKey: "sound_speed" },
        { id: "sc-co2_frost",    label: "CO₂ Frost Probability",    path: "assets/planet_sc_co2_frost.png",          scGroup: "Surface Conditions — Atmospheric", scParamKey: "co2_frost" },
        { id: "sc-slope",        label: "Slope Gradient",           path: "assets/planet_sc_slope.png",              scGroup: "Surface Conditions — Terrain",      scParamKey: "slope" },
        { id: "sc-roughness",    label: "Terrain Roughness",        path: "assets/planet_sc_roughness.png",          scGroup: "Surface Conditions — Terrain",      scParamKey: "roughness" },
        { id: "sc-ice_depth",    label: "Permafrost Thickness",     path: "assets/planet_sc_ice_depth.png",          scGroup: "Surface Conditions — Habitability", scParamKey: "ice_depth" },
        { id: "sc-dust_devil",   label: "Dust Devil Susceptibility",path: "assets/planet_sc_dust_devil.png",         scGroup: "Surface Conditions — Habitability", scParamKey: "dust_devil" },
        { id: "sc-landing",      label: "Landing Zone Score",       path: "assets/planet_sc_landing_score.png",      scGroup: "Surface Conditions — Habitability", scParamKey: "landing_score" },
        { id: "sc-solar",        label: "Solar Panel Output",       path: "assets/planet_sc_solar_output.png",       scGroup: "Surface Conditions — Habitability", scParamKey: "solar_output" },
        { id: "sc-brine",        label: "Brine Stability",          path: "assets/planet_sc_brine_stability.png",    scGroup: "Surface Conditions — Habitability", scParamKey: "brine_stability" },
        { id: "sc-magnetic",     label: "Magnetic Shielding",       path: "assets/planet_sc_magnetic_shielding.png", scGroup: "Surface Conditions — Habitability", scParamKey: "magnetic_shielding" },
        { id: "sc-habitability", label: "Human Habitability",       path: "assets/planet_sc_habitability.png",       scGroup: "Surface Conditions — Habitability", scParamKey: "habitability" },
      ];
      ...
      planetGlobeRef = globe;
      locatorViewState.camera = camera;
      locatorViewState.planetGroup = planetGroup;
...
uniform float uViewportWidth;`,
      ...
      const PLANET_UPPER_FACE_MAT = new THREE.ShaderMaterial({
        uniforms: {
      ...
      if (cutawayResult.crustRing) cutawayResult.crustRing.material = PLANET_UPPER_FACE_MAT;
      const planetSolidInterior = buildPlanetSolidInterior(3.2);
      const planetSolidInteriorGroup = planetSolidInterior.group;
      planetGroup.add(planetSolidInteriorGroup);
      const planetInteriorSphereMaterials = [
        planetSolidInterior.metallicHydrogenMaterial,
        planetSolidInterior.heavyElementCoreMaterial,
      ].filter(Boolean);
      const planetInteriorCapMaterials = [
        planetSolidInterior.metallicHydrogenCapMaterial,
        planetSolidInterior.heavyElementCoreCapMaterial,
      ].filter(Boolean);
      ...
      planetGroup.scale.set(1, planetBodyScaleY, 1);
      ...
      planetGroup.add(labelLayer.group);
      ...
      planetGroup.add(baseSiteLayer.group);
      ...
      planetGroup.add(geologyContactLayer.group);
      ...
      planetGroup.add(geologyStructureLayer.group);
      ...
      if (geologyOutlineState) {
        ...
        planetGroup.add(geologyOutlineMesh);
    ...
    function isObjectActuallyVisible(object) {
      ...
      function projectBaseLocal(point, origin) {
        ...
        const x = THREE.MathUtils.degToRad(normalizeLongitudeDegrees(lon - lon0)) * PLANET_MEAN_RADIUS_KM * 1000 * Math.cos(THREE.MathUtils.degToRad(lat0));
        const y = THREE.MathUtils.degToRad(lat - lat0) * PLANET_MEAN_RADIUS_KM * 1000;
        return { x, y };
      }
      ...
      function unprojectBaseLocal(x, y, origin) {
        const lat0 = Number(origin.lat);
        const lon0 = normalizeLongitudeDegrees(Number(origin.lon));
        const lat = lat0 + THREE.MathUtils.radToDeg(y / (PLANET_MEAN_RADIUS_KM * 1000));
        const lon = lon0 + THREE.MathUtils.radToDeg(x / ((PLANET_MEAN_RADIUS_KM * 1000) * Math.max(Math.cos(THREE.MathUtils.degToRad(lat0)), 0.0001)));
      ...
      function rebuildBaseSiteLayerInstance() {
        if (!baseSiteLayer || !planetGroup) {
          return;
        }
        planetGroup.remove(baseSiteLayer.group);
      ...
      function renderActiveBaseOverlay() {
        ...
        if (vertices.length >= 3) {
          const fillMesh = buildPlanetPolygonFillMesh(vertices);
          if (fillMesh) {
          ...
          const boundaryPoints = buildPlanetSurfacePolyline(vertices, true, 0.02);
        ...
        for (const building of activeBase.buildings || []) {
          ...
          const mesh = buildPlanetPolygonFillMesh(footprintVertices);
          if (mesh) {
          ...
          const linePoints = buildPlanetSurfacePolyline(footprintVertices, true, 0.024);
      ...
      function sampleExtractionRecord(lat, lon, columnSet) {
        ...
        if (columnSet.has("temperature")) record.temperature_c = estimatePlanetTemperature(lat, elevation || 0);
        if (columnSet.has("pressure")) record.pressure_pa = estimatePlanetPressure(elevation || 0);
        if (columnSet.has("wind")) record.wind_m_s = estimatePlanetWindSpeed(lat, elevation || 0);
        if (columnSet.has("irradiance")) record.irradiance_w_m2 = estimatePlanetIrradiance(lat, elevation || 0);
        if (columnSet.has("radiation")) record.radiation_msv_day = estimatePlanetRadiation(elevation || 0);
      ...
      function renderGisBufferOverlay() {
        ...
        const fillMesh = buildPlanetPolygonFillMesh(gisBufferState.vertices);
        if (fillMesh) {
          gisBufferGroup.add(fillMesh);
        }
        const boundaryPoints = buildPlanetSurfacePolyline(gisBufferState.vertices, true, 0.018);
        if (boundaryPoints.length >= 2) {
          const boundaryLine = new THREE.Line(
            new THREE.BufferGeometry().setFromPoints(boundaryPoints),
            new THREE.LineBasicMaterial({
      ...
      function generateGisBuffer() {
        ...
        const bufferPoints = vertices.map((point) => ({
          lat: point.lat,
          lon: point.lon,
          radiusKm: PLANET_MEAN_RADIUS_KM,
        }));
      ...
      function captureGisInspect(surfaceHit) {
        ...
        if (surfaceHit.context?.kind === "moon") {
          ...
          const localPoint = planetGroup.worldToLocal(surfaceHit.point.clone());
      ...
      function exportCurrentMeasurementCsv() {
        ...
        if (measureMode === "distance" && measurePoints.length >= 2) {
          ...
          downloadCsv("planet_distance_measurement.csv", [
        ...
        if (measureMode === "area" && measurePoints.length >= 3) {
          ...
          downloadCsv("planet_area_measurement.csv", rows);
          return;
        }
        if (measureMode === "profile" && measureProfileSamples.length >= 2) {
          ...
          downloadCsv("planet_profile_measurement.csv", rows);
          return;
        }
        if (measureMode === "route" && measurePoints.length >= 2) {
          ...
          downloadCsv("planet_route_measurement.csv", rows);
        }
      }
      if (gisInspectButton) {
      ...
      if (gisStudyExportGeoJsonButton) {
        gisStudyExportGeoJsonButton.addEventListener("click", () => {
          ...
          downloadJson("planet_study_area.geojson", { type: "FeatureCollection", features: [feature] });
        });
      }
      ...
      if (gisBufferExport) {
        gisBufferExport.addEventListener("click", () => {
          ...
          downloadJson("planet_buffer_zone.geojson", { type: "FeatureCollection", features: [feature] });
        });
      }
      if (gisBufferClear) {
      ...
      if (gisExportPointButton) {
        gisExportPointButton.addEventListener("click", () => {
          ...
          downloadCsv("planet_inspect_point.csv", [
      ...
      if (gisExportStudyButton) {
        gisExportStudyButton.addEventListener("click", () => {
          ...
          downloadJson("planet_study_geometry.geojson", {
      ...
      function parseMoonRadiusKm(feature) {
        ...
        return Number.isFinite(value) && value > 0 ? value : PLANET_MEAN_RADIUS_KM;
      }
      ...
      function refineMeasureHitLocalPoint(localPoint, context) {
        if (context.kind === "moon") {
          // Convert planetGroup-local hit to moon body frame so markers stay surface-locked
      ...
      function intersectAnySurface(clientX, clientY) {
        ...
        for (const entry of moonLayer.entries) {
          ...
          if (moonHit) {
            ...
            if (ctx) {
              const localPoint = planetGroup.worldToLocal(moonHit.point.clone());
      ...
      function intersectPlanetSurface(clientX, clientY) {
        const rect = renderer.domElement.getBoundingClientRect();
        pointer.x = ((clientX - rect.left) / rect.width) * 2 - 1;
        pointer.y = -(((clientY - rect.top) / rect.height) * 2 - 1);
        raycaster.setFromCamera(pointer, camera);
      ...
      function intersectMeasurementSurface(clientX, clientY) {
        ...
        const hitLocalPoint = hit.localPoint ? hit.localPoint.clone() : planetGroup.worldToLocal(hit.point.clone());
      ...
      if (elevationSampler) {
        ...
        planetGroup.add(regionMaskGlobe);
      } else {
      ...
      function applyPlanetDisplayState() {
        ...
        const planetLabelsEnabled = labelsToggle.checked;
        globe.visible = true;
        ...
        planetSolidInteriorGroup.visible = false; // never shown for rocky planets
        if (planetSolidInterior.metallicHydrogenCapMesh) {
          planetSolidInterior.metallicHydrogenCapMesh.visible = false;
        }
        if (planetSolidInterior.heavyElementCoreCapMesh) {
          planetSolidInterior.heavyElementCoreCapMesh.visible = false;
        }
        for (const material of planetInteriorSphereMaterials) {
          material.clippingPlanes = [];
          material.needsUpdate = true;
        }
        for (const material of planetInteriorCapMaterials) {
          material.clippingPlanes = [];
          material.needsUpdate = true;
        }
        if (baseMaterial) {
      ...
      if (csvPlotterExportPng) {
        csvPlotterExportPng.addEventListener("click", () => {
          if (csvPlotterCanvas) exportCanvasPng(csvPlotterCanvas, "planet_csv_plot.png");
        });
      }
      ...
      renderer.domElement.addEventListener("pointermove", (event) => {
        ...
        if (surfaceHit) {
          ...
          if (surfaceHit.context?.kind === "moon") {
            ...
            const tempC = estimatePlanetTemperature(latLon.lat, elevationMeters);
            const pressurePa = estimatePlanetPressure(elevationMeters);
            scTemp.textContent = `${tempC > 0 ? "+" : ""}${tempC} °C`;
        ...
        if (coreToggle.checked) {
          ...
          if (activeCutClipPlane && icDepth) {
            ...
            if (clipIntersected) {
              const localHit = planetGroup.worldToLocal(coreHit.clone());
              const rScene = localHit.length();
              const GLOBE_RADIUS_SCENE = 3.2;
              const PLANET_RADIUS_KM = 3389.5;
              if (rScene <= GLOBE_RADIUS_SCENE) {
                const rFrac = rScene / GLOBE_RADIUS_SCENE;
                const depthKm = Math.round((1.0 - rFrac) * PLANET_RADIUS_KM);
                const layerName = planetInteriorLayerName(rFrac);
                const tempC = estimatePlanetInteriorTemperature(rFrac);
                const pressureGPa = estimatePlanetInteriorPressure(rFrac);
                icDepth.textContent = `${depthKm.toLocaleString()} km`;
                icDepth.style.color = "";
                icLayer.textContent = layerName;
                icLayer.style.color = planetInteriorLayerColor(layerName);
                icTemp.textContent = `${tempC > 0 ? "+" : ""}${tempC} °C`;
                icTemp.style.color = planetInteriorTempColor(tempC);
                icPressure.textContent = `${pressureGPa} GPa`;
                icPressure.style.color = planetInteriorPressureColor(pressureGPa);
              } else {
        ...
        if (priorityHit) {
          ...
          const geologySurfaceHit = geologyToggle.checked ? intersectPlanetSurface(event.clientX, event.clientY) : null;
          if (geologySurfaceHit) {
            // worldToLocal undoes planetGroup.rotation.z (axial tilt); then un-spin globe.rotation.y
            const hoverLocalHit = planetGroup.worldToLocal(geologySurfaceHit.point.clone());
      ...
      renderer.domElement.addEventListener("pointerup", (event) => {
        ...
        if (gisBasePlacementMode) {
          const baseSurfaceHit = intersectPlanetSurface(event.clientX, event.clientY);
          if (baseSurfaceHit) {
            const localPoint = planetGroup.worldToLocal(baseSurfaceHit.point.clone());
        ...
        const surfaceHit = geologyToggle.checked ? intersectPlanetSurface(event.clientX, event.clientY) : null;
        if (surfaceHit) {
          // worldToLocal undoes planetGroup.rotation.z (axial tilt); then un-spin globe.rotation.y
          const clickLocalHit = planetGroup.worldToLocal(surfaceHit.point.clone());
      ...
      function sampleFeatureAtViewport(nx, ny) {
        ...
        const surfaceHit = intersectPlanetSurface(clientX, clientY);
        let geologyFeature = null;
        if (surfaceHit) {
          const sampleLocalHit = planetGroup.worldToLocal(surfaceHit.point.clone());
      ...
      async function runEmbeddedSmokeTest() {
        ...
        try {
          let state = window.__planetViewerDebug.getState();
          assert(state.solidGeology === true, "Solid geology should load enabled by default.");

          const feature = window.__planetViewerDebug.openFeatureAtViewport(0.5, 0.5);
          ...
          state = window.__planetViewerDebug.getState();
          assert(state.selectedOutlineVisible === true, "Selected geology outline should be visible.");

          window.__planetViewerDebug.setToggle("geology-toggle", false);
          window.__planetViewerDebug.setToggle("geology-contacts-toggle", true);
          window.__planetViewerDebug.setToggle("geology-structures-toggle", true);
          state = window.__planetViewerDebug.getState();
          ...
          const radiusBefore = window.__planetViewerDebug.getFirstLineRadius("contacts");
          assert(radiusBefore !== null, "Expected a visible contact line sample radius.");
          window.__planetViewerDebug.setTerrainRelief(0.2);
          await new Promise((resolve) => setTimeout(resolve, 250));
          const radiusAfter = window.__planetViewerDebug.getFirstLineRadius("contacts");
          assert(radiusAfter !== null && Math.abs(radiusAfter - radiusBefore) > 1e-4, "Contact linework should move with terrain relief.");

          window.__planetViewerDebug.setToggle("core-toggle", true);
          state = window.__planetViewerDebug.getState();
...
${error && error.message ? error.message : error}`;
      ...
      function updateMoonOrbits(moonEntries, featureEntries, t) {
        ...
        for (const entry of moonEntries) {
          ...
          const omega = _periodDays
            ? (2 * Math.PI * _PLANET_MOON_SPEED_FACTOR) / (_periodDays * 86400000)
      ...
      function render() {
        ...
        if (!activeMoonViewerFeature) {
          ...
          if (moonControlContext) {
            const moonCenterWorld = planetGroup.localToWorld(moonControlContext.centerLocal.clone());
        ...
        if (activeGeoPopupLocalPos && activeGeoPopupFeature) {
          try {
            const worldPos = planetGroup.localToWorld(
              activeGeoPopupLocalPos.clone().applyEuler(new THREE.Euler(0, globe.rotation.y - Math.PI, 0)),
            );
            const planetCenterWorld = planetGroup.localToWorld(new THREE.Vector3(0, 0, 0));
        ...
        if (_heavyFrame) {
          ...
          let scaleBodyRadiusMeters = PLANET_RADIUS_METERS;
    ...
    (function () {
      const audioEl = document.getElementById("planet-audio-el");
