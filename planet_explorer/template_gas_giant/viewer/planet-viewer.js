...
{"name": "Voyager 2 Flyby Track", "type": "Spacecraft flyby corridor", "lat": -3.0, "lon": 172.0, "theme": "landing", "description": "Representative corridor for Voyager 2's Saturn flyby on 26 August 1981. Passing at ~2.69 Saturn radii, Voyager 2 extended Voyager 1's ring and moon discoveries, obtained high-resolution views of the Cassini Division and A Ring structure, and continued on to Uranus and Neptune — the only spacecraft to visit all four outer planets."}];
    ...
    function georeferenceRingAndInnerMoonAnchors() {
      ...
      for (const entry of ringLabelData) {
        ...
        const radiusScene = radiusKm * PLANET_KM_TO_SCENE;
        entry.ring_anchor = [
      ...
      for (const moon of moonData) {
        ...
        const theta = Math.atan2(z, x);
        const radiusScene = orbitKm * PLANET_KM_TO_SCENE;
        moon.moon_anchor = [
          Number((Math.cos(theta) * radiusScene).toFixed(4)),
          y,
          Number((Math.sin(theta) * radiusScene).toFixed(4)),
    ...
    let viewerApplyPlanetViewMode = null;
    ...
    const PLANET_MEAN_RADIUS_KM = 58232.0;
    let activeCameraFlight = null;
    const planetViewModeSelect = document.getElementById("planet-view-mode");
    ...
    function updateScaleReadout(camera, renderer, controls) {
      ...
      let kmPerWorldUnit = PLANET_MEAN_RADIUS_KM / PLANET_SCENE_RADIUS;
    ...
    function configurePlanetUi() {
      if (brandLogo) {
        brandLogo.src = "../../../assets/planet_icon.png";
      }
      ...
      if (planetViewModeSelect) planetViewModeSelect.value = "tilted";
    }
    configurePlanetUi();
    ...
    function createCalibratedPlanetRingTexture(baseTexture = null, outerRadiusKm = PLANET_RING_REFERENCE_KM.mainOuter) {
      const size = 4096;
      ...
      for (let y = 0; y < size; y += 1) {
        ...
        for (let x = 0; x < size; x += 1) {
          ...
          if (r >= kmToUnit(PLANET_RING_REFERENCE_KM.dInner) && r <= 1) {
            const noise =
              0.5
              + 0.25 * Math.sin(r * 980)
            ...
            if (r < kmToUnit(PLANET_RING_REFERENCE_KM.cInner)) {
              alpha = 0.16 * smoothBand(r, PLANET_RING_REFERENCE_KM.dInner, PLANET_RING_REFERENCE_KM.cInner, 260);
              shade = 72 + 28 * noise;
            } else if (r < kmToUnit(PLANET_RING_REFERENCE_KM.bInner)) {
              alpha = 0.36 * smoothBand(r, PLANET_RING_REFERENCE_KM.cInner, PLANET_RING_REFERENCE_KM.bInner, 220);
              shade = 82 + 42 * noise;
            } else if (r < kmToUnit(PLANET_RING_REFERENCE_KM.cassiniInner)) {
              alpha = 0.9 * smoothBand(r, PLANET_RING_REFERENCE_KM.bInner, PLANET_RING_REFERENCE_KM.cassiniInner, 190);
              shade = 150 + 74 * noise;
            } else if (r < kmToUnit(PLANET_RING_REFERENCE_KM.aInner)) {
              alpha = 0.18 * smoothBand(r, PLANET_RING_REFERENCE_KM.cassiniInner, PLANET_RING_REFERENCE_KM.aInner, 120);
              shade = 35 + 28 * noise;
            } else if (r < kmToUnit(PLANET_RING_REFERENCE_KM.aOuter)) {
              alpha = 0.68 * smoothBand(r, PLANET_RING_REFERENCE_KM.aInner, PLANET_RING_REFERENCE_KM.aOuter, 160);
              shade = 126 + 62 * noise;
              alpha *= narrowGap(r, PLANET_RING_REFERENCE_KM.enckeCenter, 325, 0.025);
              shade *= narrowGap(r, PLANET_RING_REFERENCE_KM.enckeCenter, 325, 0.18);
              alpha *= narrowGap(r, PLANET_RING_REFERENCE_KM.keelerCenter, 90, 0.04);
              shade *= narrowGap(r, PLANET_RING_REFERENCE_KM.keelerCenter, 90, 0.22);
            } else if (r < kmToUnit(PLANET_RING_REFERENCE_KM.fRing - 450)) {
              alpha = 0.12 * smoothBand(r, PLANET_RING_REFERENCE_KM.aOuter, PLANET_RING_REFERENCE_KM.fRing - 450, 140);
              shade = 40 + 28 * noise;
            } else if (r < kmToUnit(PLANET_RING_REFERENCE_KM.fRing + 650)) {
              alpha = 0.62 * smoothBand(r, PLANET_RING_REFERENCE_KM.fRing - 450, PLANET_RING_REFERENCE_KM.fRing + 650, 90);
              shade = 158 + 48 * noise;
            } else {
              alpha = 0.04 * smoothBand(r, PLANET_RING_REFERENCE_KM.fRing + 650, PLANET_RING_REFERENCE_KM.mainOuter, 190);
              shade = 50 + 24 * noise;
            }
          }
    ...
    function estimatePlanetCloudTopTemp(latDeg) {
    ...
    function estimatePlanetCloudTopPressure() {
    ...
    const PLANET_INTERIOR_T_PTS = [[0.000, 25000], [0.160, 20000], [0.420, 12000], [0.850, 2000], [1.000, -134]];
    const PLANET_INTERIOR_P_PTS = [[0.000, 20000], [0.160, 1500],  [0.420, 200],   [0.850, 1.0],  [1.000, 0.0001]];
    function _planetInteriorInterp(pts, rFrac) {
    ...
    function planetInteriorTempC(rFrac) { return Math.round(_planetInteriorInterp(PLANET_INTERIOR_T_PTS, rFrac)); }
    function planetInteriorPressureGPa(rFrac) { return Math.round(_planetInteriorInterp(PLANET_INTERIOR_P_PTS, rFrac) * 10) / 10; }
    function planetInteriorLayerColor(name) {
    ...
    function moveCameraToFeature(feature, camera, controls, options = {}) {
      ...
      if (options.animate) {
        if (options.isTour && !Array.isArray(feature.ring_anchor)) {
          ...
          animateTourFlight(camera, controls, cameraPosition, planetCenter, 2800, options.onComplete || null);
        } else {
        ...
        camera.up.set(0, 1, 0);
        controls.object.position.copy(camera.position);
        controls.target.copy(options.isTour ? planetCenter : target);
        controls.update();
      }
    }
    ...
    function reloadToDefaultGlobalView(camera, controls) {
      ...
      viewerApplyPlanetViewMode?.(planetViewModeSelect ? planetViewModeSelect.value : "tilted");
      controls.saveState();
    }
    ...
    function buildCutawayInterior(radius, layerData, elevationMap, terrainRelief) {
      ...
      const PLANET_HEAVY_CORE_RADIUS = 0.22;
      const PLANET_METALLIC_HYDROGEN_RADIUS = 0.58;
      const PLANET_MOLECULAR_ENVELOPE_RADIUS = 0.96;
      
      // ── Inner core: dense crystalline metallic interior ───────────────────
      const innerCoreMesh = new THREE.Mesh(
        new THREE.SphereGeometry(radius * PLANET_HEAVY_CORE_RADIUS, 96, 96, phiStart, phiLength),
      ...
      const outerCoreMesh = new THREE.Mesh(
        new THREE.SphereGeometry(radius * PLANET_METALLIC_HYDROGEN_RADIUS, 128, 128, phiStart, phiLength),
        new THREE.ShaderMaterial({
          uniforms: {},
          vertexShader: LAYER_VERT,
      ...
      const mantleMesh = new THREE.Mesh(
        new THREE.SphereGeometry(radius * PLANET_MOLECULAR_ENVELOPE_RADIUS, 128, 128, phiStart, phiLength),
        new THREE.ShaderMaterial({
          uniforms: {},
          vertexShader: LAYER_VERT,
      ...
      const mantleInnerBoundaryMesh = new THREE.Mesh(
        new THREE.SphereGeometry(radius * (PLANET_METALLIC_HYDROGEN_RADIUS + 0.002), 128, 128, phiStart, phiLength),
        new THREE.ShaderMaterial({
          uniforms: {},
          vertexShader: LAYER_VERT,
          fragmentShader: MANTLE_FRAG,
      ...
      const CAP_X = -0.012;
      const crustInnerRadius = radius * PLANET_MOLECULAR_ENVELOPE_RADIUS;
      ...
      const capDefs = [
        ...
        { outer: crustInnerRadius, inner: radius * PLANET_METALLIC_HYDROGEN_RADIUS,  fragmentShader: MANTLE_SECTION_FRAG },  // molecular envelope ring
        { outer: radius * PLANET_METALLIC_HYDROGEN_RADIUS,  inner: radius * PLANET_HEAVY_CORE_RADIUS,  fragmentShader: CORE_SECTION_FRAG },  // metallic hydrogen ring
      ];
      let crustRing = null;
      ...
      const fluidCapDisk = new THREE.Mesh(
        new THREE.CircleGeometry(radius * PLANET_METALLIC_HYDROGEN_RADIUS, 128),
        fluidCapMat,
      );
      ...
      const innerCapDisk = new THREE.Mesh(
        new THREE.CircleGeometry(radius * PLANET_HEAVY_CORE_RADIUS, 96),
        innerCapMat,
      );
    ...
    async function init() {
      ...
      function handleSurfaceWheelZoom(event) {
        ...
        const zoomContext = getPlanetZoomContext();
      ...
      const ringsConfig = planetConfig.rings || null;
      ...
      if (ringsConfig && ringsConfig.path) {
        const ringOuterKm = Number.isFinite(Number(ringsConfig.outer_km))
          ...
          : PLANET_RING_REFERENCE_KM.mainOuter;
        const baseRingTexture = await loadTextureSafe(textureLoader, ringsConfig.path);
        const ringTexture = createCalibratedPlanetRingTexture(baseRingTexture, ringOuterKm);
        if (baseRingTexture) {
      ...
      const PLANET_UPPER_FACE_MAT = new THREE.ShaderMaterial({
        ...
        fragmentShader: `
          ...
          void main(){
            ...
            vec3 planetCream = vec3(0.90, 0.82, 0.68);
            vec3 planetPale = vec3(0.97, 0.92, 0.82);
            vec3 haze = vec3(0.98, 0.95, 0.88);
            float cloudMask = smoothstep(0.26, 0.86, texCol.r * 0.95 + texCol.g * 0.75 - texCol.b * 0.22);
            vec3 toned = mix(texCol, planetCream, 0.58);
            toned = pow(max(toned, vec3(0.0)), vec3(0.82));
            toned = mix(toned, planetPale, cloudMask * 0.24);
      ...
      const PLANET_MH_MAT  = new THREE.MeshStandardMaterial({ color: 0xa29378, roughness: 0.58, metalness: 0.44, side: THREE.BackSide });
      const PLANET_MH_OUTER = new THREE.MeshStandardMaterial({ color: 0xa29378, roughness: 0.58, metalness: 0.44, side: THREE.FrontSide });
      const PLANET_HC_MAT  = new THREE.MeshStandardMaterial({ color: 0x8a7a65, roughness: 0.82, metalness: 0.12, side: THREE.BackSide });
      // Flat cap disks and rings: DoubleSide so they're visible from both directions.
      const PLANET_MH_FLAT = new THREE.MeshStandardMaterial({ color: 0xa29378, roughness: 0.58, metalness: 0.44, side: THREE.DoubleSide });
      const PLANET_HC_FLAT = new THREE.MeshStandardMaterial({ color: 0x8a7a65, roughness: 0.82, metalness: 0.12, side: THREE.DoubleSide });
      if (cutawayResult.crustRing) cutawayResult.crustRing.material = PLANET_UPPER_FACE_MAT;
      if (cutawayResult.metallicHydrogenMesh) cutawayResult.metallicHydrogenMesh.material = PLANET_MH_MAT;
      if (cutawayResult.molecularBoundaryMesh) cutawayResult.molecularBoundaryMesh.material = PLANET_MH_OUTER;
      if (cutawayResult.metallicHydrogenCapMesh) cutawayResult.metallicHydrogenCapMesh.material = PLANET_MH_FLAT;
      if (cutawayResult.metallicHydrogenRing) cutawayResult.metallicHydrogenRing.material = PLANET_MH_FLAT;
      if (cutawayResult.heavyElementCoreMesh) cutawayResult.heavyElementCoreMesh.material = PLANET_HC_MAT;
      if (cutawayResult.heavyElementCoreCapMesh) cutawayResult.heavyElementCoreCapMesh.material = PLANET_HC_FLAT;
      const planetSolidInterior = buildPlanetSolidInterior(3.2);
      const planetSolidInteriorGroup = planetSolidInterior.group;
      marsGroup.add(planetSolidInteriorGroup);
      ...
      const planetBodyScaleY = Number(planetConfig.body_scale_y || 0.902);
      const planetAxialTiltDeg = Number(planetConfig.axial_tilt_deg || 26.7);
      marsGroup.scale.set(1, planetBodyScaleY, 1);
      const applyPlanetViewMode = (mode = "tilted", resetCamera = false) => {
      ...
      viewerApplyPlanetViewMode = applyPlanetViewMode;
      applyPlanetViewMode(planetViewModeSelect ? planetViewModeSelect.value : "tilted");
    ...
    function isObjectActuallyVisible(object) {
      ...
      function intersectExposedPlanetInteriorSurface(clientX, clientY) {
        const rect = renderer.domElement.getBoundingClientRect();
        pointer.x = ((clientX - rect.left) / rect.width) * 2 - 1;
        pointer.y = -(((clientY - rect.top) / rect.height) * 2 - 1);
        raycaster.setFromCamera(pointer, camera);
        const exposedTargets = [
          planetSolidInterior?.metallicHydrogenMesh,
          planetSolidInterior?.heavyElementCoreMesh,
        ...
        return { ...hit, localPoint, lat: latLon.lat, lon: latLon.lon, context: { kind: "planet-interior-surface" } };
      }
      ...
      function applyPlanetAtmosphereRemoval() {
        const removeAtmosphere = Boolean(geologyToggle && geologyToggle.checked);
        const coreEnabled = Boolean(coreToggle && coreToggle.checked);
        const planetLabelsEnabled = !removeAtmosphere && labelsToggle.checked;
        ...
        planetSolidInteriorGroup.visible = removeAtmosphere && !coreEnabled;
      ...
      if (planetViewModeSelect) {
        planetViewModeSelect.addEventListener("change", () => {
          resetExploreView(camera, controls);
          applyPlanetViewMode(planetViewModeSelect.value);
        });
      }
      ...
      renderer.domElement.addEventListener("pointermove", (event) => {
        ...
        if (!coreToggle.checked) {
          ...
          if (!surfaceHit && removeAtmosphereActive) {
            surfaceHit = intersectExposedPlanetInteriorSurface(event.clientX, event.clientY);
          }
          if (surfaceHit) {
            ...
            if (scTemp && scPressure) {
              ...
              } else if (surfaceHit.context?.kind === "planet-interior-surface") {
                const local = surfaceHit.localPoint || marsGroup.worldToLocal(surfaceHit.point.clone());
                const rFrac = Math.max(0, Math.min(1, local.length() / PLANET_SCENE_RADIUS));
                const layerName = planetInteriorLayerName(rFrac);
                const tempC = planetInteriorTempC(rFrac);
                const pGPa = planetInteriorPressureGPa(rFrac);
                scTemp.textContent = `${tempC > 0 ? "+" : ""}${tempC.toLocaleString()} °C`;
                scTemp.style.color = planetInteriorTempColor(tempC);
                scPressure.textContent = formatPressureGPa(pGPa);
                scPressure.style.color = planetInteriorPressureColor(pGPa);
                if (scContext) scContext.textContent = `LAYER: ${layerName.toUpperCase()} (SURFACE)`;
              } else {
                const tempC = estimatePlanetCloudTopTemp(latLon.lat);
                const pressurePa = estimatePlanetCloudTopPressure();
          ...
          if (activeCutClipPlane && icDepth) {
            ...
            if (coreGlobeHit) {
              ...
              if (signedDistance >= 0) {
                ...
                const tempC = estimatePlanetCloudTopTemp(latLon.lat);
                const pPa = estimatePlanetCloudTopPressure();
                const pGPa = pPa / 1e9;
                icDepth.textContent = "0 km";
                icLayer.textContent = "Upper Atmosphere (surface)";
                icLayer.style.color = planetInteriorLayerColor("Upper Atmosphere");
                ...
                icPressure.style.color = planetInteriorPressureColor(pGPa);
                return;
              }
            }
            ...
            if (raycaster.ray.intersectPlane(activeCutClipPlane, coreHit)) {
              ...
              const rScene = localHit.length();
              const GLOBE_R = 3.2;
              const PLANET_R_KM = 60268;
              if (rScene <= GLOBE_R) {
                const rFrac = rScene / GLOBE_R;
                const depthKm = Math.round((1.0 - rFrac) * PLANET_R_KM);
                const layerName = planetInteriorLayerName(rFrac);
                const tempC = planetInteriorTempC(rFrac);
                const pGPa = planetInteriorPressureGPa(rFrac);
                icDepth.textContent = `${depthKm.toLocaleString()} km`;
              ...
              if (coreSurfaceHit) {
                ...
                if (removeAtmosphereActive) {
                  const rFrac = PLANET_EXPOSED_INTERIOR_RFRAC;
                  const depthKm = Math.round((1.0 - rFrac) * 60268);
                  const layerName = planetInteriorLayerName(rFrac);
                  const tempC = planetInteriorTempC(rFrac);
                  const pGPa = planetInteriorPressureGPa(rFrac);
...
${error && error.message ? error.message : error}`;
      ...
      function render() {
        ...
        const removeAtmosphere = Boolean(geologyToggle && geologyToggle.checked);
        const planetLabelsEnabled = labelsToggle.checked && !removeAtmosphere;
        const planetSeismicEnabled = seismicToggle.checked && !removeAtmosphere;
        ...
        if (!coreToggle.checked) {
          ...
          updateLabelVisibility(
            ...
            renderer,
            planetLabelsEnabled,
            planetLabelsEnabled && volcanicLabelsToggle.checked,
            planetLabelsEnabled && landingLabelsToggle.checked,
            planetLabelsEnabled && habitationLabelsToggle.checked,
            true,
            stormLabelsToggle ? (planetLabelsEnabled && stormLabelsToggle.checked) : true,
          );
          updateRingLabelVisibility(
            ringLabelLayer.entries,
            marsGroup,
