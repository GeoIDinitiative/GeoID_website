...
import { moonLatLonToVector3, makeLabelTexture, isVolcanicMoonFeature, isCraterMoonFeature, getMoonFeatureConnectorStart, buildLabelLayer, buildMoonLayer, buildMoonFeatureLabelLayer, getEntryConnectorStart, updateLabelAnchors, rebuildLabelTextures, updateMoonFeatureLabelVisibility, updateMoonVisibility, updateLabelVisibility } from "./label-layer.js";
    ...
    // Earth sidereal rotation period: 23.9345 h.
    const _EARTH_DISPLAY_PERIOD_MS = 600000; // 10 min visual rotation
    ...
    const locatorViewState = { camera: null, earthGroup: null, globe: null };
    ...
    function sampleLocatorLatLon() {
      if (!locatorViewState.globe || !locatorViewState.camera || !locatorViewState.earthGroup || activeMoonViewerFeature) {
        return null;
      }
      locatorCameraLocal.copy(locatorViewState.camera.position);
      locatorViewState.earthGroup.worldToLocal(locatorCameraLocal);
    ...
    const EARTH_INTERIOR_LAYERS = [
      { name: "Inner Core",        rMin: 0.000, rMax: 0.191 },
      { name: "Liquid Outer Core", rMin: 0.191, rMax: 0.546 },
      { name: "Mantle",            rMin: 0.546, rMax: 0.985 },
      { name: "Crust",             rMin: 0.985, rMax: 1.000 },
    ];
    // Piecewise-linear T (°C) and P (GPa) profiles keyed on rFrac (sorted low→high)
    const EARTH_INTERIOR_T_PTS = [[0.000, 5700], [0.191, 5000], [0.546, 3500], [0.985, 600], [1.000, 15]];
    const EARTH_INTERIOR_P_PTS = [[0.000, 364.0], [0.191, 330.0], [0.546, 135.0], [0.985, 3.5], [1.000, 0.0001]];
    ...
    const MOON_VIEWER_TEXTURES = {
      "Moon": "assets/moon_texture_1024.jpg",
    };
    ...
    const CORE_LAYER_DATA = [
      {
        ...
        description: "Earth's crust is divided into rigid tectonic plates drifting over the convecting mantle. Oceanic crust is 5–10 km thick, dense and basaltic, and is continuously recycled at subduction zones — no oceanic crust older than ~200 million years survives. Continental crust is 25–70 km thick, less dense and dominated by granitic silicate rocks, and can persist for billions of years. The base of the crust — the Mohorovičić discontinuity (Moho) — marks a sharp jump in seismic-wave velocity.",
        depth: "0 – ~10 km (oceanic); 0 – ~70 km (continental)",
        composition: "Oceanic: basalt and gabbro. Continental: granite, andesite, and sedimentary rocks. Both are silicate-dominated.",
        temperature: "~15°C at surface → ~400–700°C at base",
      ...
      },
      {
        id: "mantle",
        name: "Mantle",
        type: "Silicate mantle",
        description: "The mantle is Earth's thickest layer, spanning from just below the crust to the core–mantle boundary at ~2,890 km depth. Solid but plastic over geological timescales, it convects heat upward and drives plate tectonics. The upper mantle includes the partially molten asthenosphere on which tectonic plates ride. The lower mantle is denser due to high-pressure mineral phase transitions. Hotspot plumes rising from the core–mantle boundary feed volcanic chains such as Hawaiʻi and Iceland.",
        depth: "~10–70 km to ~2,890 km depth",
        composition: "Upper mantle: peridotite (olivine, pyroxene, garnet). Lower mantle: bridgmanite and ferropericlase at high-pressure phases.",
        temperature: "~600°C near crust → ~3,500°C at core–mantle boundary",
        labelX: -2.2, labelY: 2.45,
        anchorY: 2.45,
      },
      {
        id: "outer-core",
        name: "Liquid Outer Core",
        type: "Iron-nickel fluid",
        description: "Earth's outer core is a 2,260 km-thick shell of liquid iron-nickel with ~10% lighter alloying elements (sulfur, oxygen, silicon). Differential rotation and Coriolis forces drive vigorous convection within this conducting fluid, sustaining the geodynamo that generates Earth's global magnetic field. Without the outer core's dynamo, the magnetosphere would collapse, exposing the surface to harmful solar-wind radiation.",
        depth: "~2,890 km to ~5,150 km depth",
        composition: "~80% liquid iron, ~5% nickel, ~10–15% lighter elements (S, O, Si, C, H).",
        temperature: "~3,500°C at top → ~5,000°C at base",
        labelX: -1.6, labelY: 1.179,
        anchorY: 1.179,
      },
      {
        id: "inner-core",
        ...
        description: "Earth's solid inner core was discovered in 1936 by seismologist Inge Lehmann from anomalous P-wave reflections. Despite temperatures rivalling the Sun's surface (~5,000–5,700°C), extreme pressure (~330–364 GPa) keeps the iron-nickel alloy solid. The inner core rotates slightly faster than the rest of Earth (super-rotation) and may have an innermost inner core with a distinct crystal alignment, suggesting a complex thermal history.",
        depth: "~5,150 km depth to centre (~1,220 km radius)",
        composition: "Solid iron-nickel alloy with a small fraction of lighter elements. Crystal structure is likely hexagonal close-packed (HCP) iron.",
        temperature: "~5,000°C at boundary → ~5,700°C at centre",
        labelX: -1.0, labelY: 0,
        anchorY: 0,
      },
    ];
    ...
    function createContourLineLayer(THREERef, earthGroup, elevationSampler, getTerrainRelief, radius = 3.2) {
    ...
    function buildLegendEntries(baseLayers, geologyLayers, mineralLayers, geologyInteractiveState = null, geologyStructureLayers = []) {
      ...
      if (geologyToggle.checked && selectedGeologyLayer) {
        ...
        entries.push({
          title: "Solid Geology",
          copy: "USGS, Geologic Map of Earth simplified into rock-type classes for the active solid geology overlay.",
          tags: ["geology", "units"],
      ...
      if (
        ...
        if (activeStructureFacets.length) {
          entries.push({
            ...
            copy: "USGS, Geologic Map of Earth structural traces split into individually toggleable structure families.",
    ...
    function buildMetadataState(baseLayers, geologyLayers, mineralLayers, geologyInteractiveState = null, geologyStructureLayers = []) {
      ...
      if (geologyContactsToggle.checked && (geologyInteractiveState?.contacts || []).length) {
        pushActiveLayer(
          ...
          "USGS, Geologic Map of Earth geologic contacts draped from the vector shapefiles.",
        );
      }
      const activeStructureFacets = getActiveGeologyStructureFacets(geologyStructureLayers);
      if (activeStructureFacets.length) {
        pushActiveLayer(
          ...
          "geology",
          selectedGeologyLayer || selectedBaseLayer,
          `USGS, Geologic Map of Earth structural traces. Active facets: ${activeStructureFacets.map((facet) => facet.label).join(", ")}.`,
        );
      }
      if (selectedMineralLayer) {
      ...
      if (
        ...
        sourceEntries.push({
          ...
          copy: "Use the source pages below for the USGS, Geologic Map of Earth layer and related Earth geology references.",
      ...
      app.innerHTML = `
        <div class="webgl-fallback">
          <div class="webgl-fallback-card">
            ...
            <h2 style="margin:0 0 0.6rem;">Earth viewer could not start.</h2>
    ...
    function moveCameraToFeature(feature, camera, controls, options = {}) {
      ...
      if (Array.isArray(feature.ring_anchor)) {
        ...
        const _spinDelta = getSpinTime() * (2 * Math.PI / _EARTH_DISPLAY_PERIOD_MS);
    ...
    function buildCutawayInterior(radius, layerData, elevationMap, terrainRelief) {
      ...
      const EARTH_INNER_CORE_RADIUS = 0.191;   // solid inner core boundary (~1,220 km)
      const EARTH_OUTER_CORE_RADIUS = 0.546;   // core–mantle boundary (~3,480 km)
      const EARTH_MANTLE_RADIUS = 0.985;       // base of crust (~100 km depth)
    ...
    function earthLonToSceneLon(lonDegrees) {
    ...
    function latLonToVector3(latDegrees, lonDegrees, radius) {
      ...
      const lon = THREE.MathUtils.degToRad(earthLonToSceneLon(lonDegrees));
    ...
    class CTXDetailPatchStreamer {
      constructor(earthGroup, tileBase, getTilePayload, getTileLevelCap, getViewBbox, getFocusTarget, elevationSampler, getTerrainRelief, anisotropy = 1) {
    ...
    async function init() {
      ...
      function getActiveZoomContext() {
        ...
        if (activeMoonViewerFeature) {
          ...
          const centerWorld = earthGroup.localToWorld(moonContext.centerLocal.clone());
          return {
            centerWorld,
        ...
        earthGroup.getWorldPosition(wheelZoomBodyCenter);
      ...
      const geologyLayers = manifest.geology_layers || [{
        id: "sim3292-units",
        label: "USGS, Geologic Map of Earth",
        path: manifest.geology.path,
        description: "USGS, Geologic Map of Earth geology overlay sourced from Scientific Investigations Map 3292.",
        default: true,
      }];
      ...
      locatorViewState.earthGroup = earthGroup;
...
uniform float uViewportWidth;`,
      ...
      const earthInteriorSphereMaterials = [
        earthSolidInterior.metallicHydrogenMaterial,
        earthSolidInterior.heavyElementCoreMaterial,
      ].filter(Boolean);
      const earthInteriorCapMaterials = [
        earthSolidInterior.metallicHydrogenCapMaterial,
        earthSolidInterior.heavyElementCoreCapMaterial,
      ].filter(Boolean);
      ...
      earthGroup.scale.set(1, planetBodyScaleY, 1);
      ...
      earthGroup.add(baseSiteLayer.group);
      ...
      earthGroup.add(geologyContactLayer.group);
      ...
      earthGroup.add(geologyStructureLayer.group);
      ...
      if (geologyOutlineState) {
        ...
        earthGroup.add(geologyOutlineMesh);
    ...
    function isObjectActuallyVisible(object) {
      ...
      function baseLabelItem(base) {
        return {
          ...
          name: base.name,
          type: "Base site",
          description: `Earth base boundary covering ${Number(base.areaKm2 || 0).toFixed(1)} km².`,
      ...
      function rebuildBaseSiteLayerInstance() {
        if (!baseSiteLayer || !earthGroup) {
          return;
        }
        earthGroup.remove(baseSiteLayer.group);
      ...
      function focusActiveBase() {
        ...
        moveCameraToFeature({
          ...
          lat: base.center.lat,
          lon: base.center.lon,
          description: `Earth base boundary covering ${base.areaKm2.toFixed(1)} km².`,
          theme: "mission",
      ...
      function captureGisInspect(surfaceHit) {
        ...
        if (surfaceHit.context?.kind === "moon") {
          ...
          const localPoint = earthGroup.worldToLocal(surfaceHit.point.clone());
      ...
      function refineMeasureHitLocalPoint(localPoint, context) {
        if (context.kind === "moon") {
          // Convert earthGroup-local hit to moon body frame so markers stay surface-locked
      ...
      function intersectAnySurface(clientX, clientY) {
        ...
        for (const entry of moonLayer.entries) {
          ...
          if (moonHit) {
            ...
            if (ctx) {
              const localPoint = earthGroup.worldToLocal(moonHit.point.clone());
      ...
      function intersectMeasurementSurface(clientX, clientY) {
        ...
        const hitLocalPoint = hit.localPoint ? hit.localPoint.clone() : earthGroup.worldToLocal(hit.point.clone());
      ...
      if (elevationSampler) {
        ...
        earthGroup.add(regionMaskGlobe);
      } else {
      ...
      function applyPlanetDisplayState() {
        ...
        earthSolidInteriorGroup.visible = false; // never shown for rocky planets
        if (earthSolidInterior.metallicHydrogenCapMesh) {
          earthSolidInterior.metallicHydrogenCapMesh.visible = false;
        }
        if (earthSolidInterior.heavyElementCoreCapMesh) {
          earthSolidInterior.heavyElementCoreCapMesh.visible = false;
        }
      ...
      renderer.domElement.addEventListener("pointermove", (event) => {
        ...
        if (coreToggle.checked) {
          ...
          if (activeCutClipPlane && icDepth) {
            ...
            if (clipIntersected) {
              const localHit = earthGroup.worldToLocal(coreHit.clone());
              const rScene = localHit.length();
              const GLOBE_RADIUS_SCENE = 3.2;
              const EARTH_RADIUS_KM = 3389.5;
      ...
      function updateMoonOrbits(moonEntries, featureEntries, t) {
        ...
        for (const entry of moonEntries) {
          ...
          const omega = _periodDays
            ? (2 * Math.PI * _EARTH_MOON_SPEED_FACTOR) / (_periodDays * 86400000)
      ...
      function render() {
        ...
        if (!activeMoonViewerFeature) {
          ...
          if (moonControlContext) {
            const moonCenterWorld = earthGroup.localToWorld(moonControlContext.centerLocal.clone());
        ...
        if (!coreToggle.checked) {
          ...
          globe.rotation.y = Math.PI + _spinT * (2 * Math.PI / _EARTH_DISPLAY_PERIOD_MS);
        ...
        if (activeGeoPopupLocalPos && activeGeoPopupFeature) {
          try {
            const worldPos = earthGroup.localToWorld(
              activeGeoPopupLocalPos.clone().applyEuler(new THREE.Euler(0, globe.rotation.y - Math.PI, 0)),
            );
            const planetCenterWorld = earthGroup.localToWorld(new THREE.Vector3(0, 0, 0));
