...
import { ROUTE, SUMMIT, TIME_SCALE, MOVE, RENDER, IMAGERY, ELEVATION, PHYS } from "./config.js?v=4becb87-599a0ad6";
import { llToLocal, localToLL, haversine, bearing, compassPoint } from "./geo.js?v=4becb87-599a0ad6";
import { Heightfield } from "./dem.js?v=4becb87-599a0ad6";
import { Imagery } from "./imagery.js?v=4becb87-599a0ad6";
import { Terrain } from "./terrain.js?v=4becb87-599a0ad6";
import { Sky, NEPAL_UTC_OFFSET_H } from "./sky.js?v=4becb87-599a0ad6";
import { Weather, Precipitation, Spindrift } from "./weather.js?v=4becb87-599a0ad6";
import { Glacier } from "./glacier.js?v=4becb87-599a0ad6";
import { TerrainShadows } from "./shadows.js?v=4becb87-599a0ad6";
import { PostFX, QUALITY } from "./postfx.js?v=4becb87-599a0ad6";
import { estimateCaptureSun } from "./delight.js?v=4becb87-599a0ad6";
import { SnowField } from "./snowfield.js?v=4becb87-599a0ad6";
import { Photoclinometry } from "./photoclino.js?v=4becb87-599a0ad6";
import { World } from "./world.js?v=4becb87-599a0ad6";
import { Survival, pressureKPa, inspiredO2 } from "./survival.js?v=4becb87-599a0ad6";
import { Player, STATE } from "./player.js?v=4becb87-599a0ad6";
import { Director, Climbers } from "./director.js?v=4becb87-599a0ad6";
import { Hud } from "./hud.js?v=4becb87-599a0ad6";
import { Audio } from "./audio.js?v=4becb87-599a0ad6";
import { install as installDiag } from "./diag.js?v=4becb87-599a0ad6";
import * as tiles from "./tiles.js?v=4becb87-599a0ad6";
...
export class Game {
  ...
  async boot(onProgress) {
       ...
       once: layered rounded stones in the greys of the surrounding rock. */
    {
      ...
      /* Broken rock for flat dark ground above the valley — the shader
         gates it by slope, so faces never take it. */
  ...
  onKey(e) {
    ...
    if (c === "KeyE") {
      /* A perched step-ladder takes priority: E at its base boosts you
         onto the top — three metres of aluminium, four of your breath. */
      const sl = this.glacier.stepLadderNear(this.player.pos.x, this.player.pos.z, 3.5);
      if (sl && this.player.pos.y < sl.topY - 1.0 && this.player.state === STATE.WALKING) {
        this.player.placeAt(sl.topX, sl.topZ, this.player.yaw);
        this.player.pos.y = sl.topY;
        this.survival.energy = Math.max(0, this.survival.energy - 2.5);
        this.audio.cue("chime");
        this.hud.notify(`Up the ladder — ${sl.rise.toFixed(0)} m gained.`, "good");
        return;
      }
  ...
  step(dtReal, dtSim) {
    ...
    if (this._routeRebuildAt && performance.now() > this._routeRebuildAt) {
      ...
      this.world.buildRoute();
      this.world.routeGroup.visible = this.showRoute;
      /* Everything seated on the heightfield re-seats with it: the tents
         and the boulders were placed against boot-time coarse heights and
         floated once finer elevation lowered the ground under them. */
      this.world.buildBoulders();
      this.world.snapToGround();
    }
    ...
    /* Day fades through civil twilight: full night below -7 degrees of
       solar altitude, full day above +7. */
    u.dayLight.value = Math.min(1, Math.max(0, (this.sky.sun.altitude + 7) / 14));
  ...
  promptFor() {
    ...
    const sl = this.glacier.stepLadderNear(P.pos.x, P.pos.z, 3.5);
    if (sl && P.pos.y < sl.topY - 1.0) return `<kbd>E</kbd> up the ladder — ${sl.rise.toFixed(0)} m`;
