    ...
    class CTXCanvasLayer {
      ...
      _refreshFocus(bbox, focusLevel, meta = {}) {
        ...
        if (this._focusQueue.length > 1) {
          ...
          // FLIGHT-SIM: fill from the SHIP outward, not from the window centre.
          // The flight window is deliberately led forward, so a centre-out sort
          // fetches the mid-distance first and leaves the ground under the nose
          // for LAST. With 6 connections a wide round takes ~27 s to drain, by
          // which time a 9.6 km/s ship has moved ~260 km and the window has been
          // retired — so the near field was never reached. Symptom exactly as
          // reported: only the centre of the line of sight is mapped, with the
          // immediate foreground left coarse.
              ...
              if (window.__flightSim?.active) {
                ...
                if (shipGround) {
                  ...
                  if (this._flightInputMode === "near") {
                    ...
                    // Straddle the ship rather than parking ahead of it. The
                    // look-at point at altitude is near the horizon, and
                    // centring on it (plus a further lead) left the ship 92 km
                    // from the trailing edge with the window centre 167 km out
                    // in front — measured. The surround owns the long view; this
                    // window's job is regional detail around the aircraft, so
                    // the forward offset is capped at 20% of the span.
                    let offLon = baseAnchor.lon - shipGround.lon;
                    if (offLon > 180) offLon -= 360; else if (offLon < -180) offLon += 360;
                    let offLat = baseAnchor.lat - shipGround.lat;
                    const maxOff = spanFar * 0.20;
                    const clampOff = (v) => Math.max(-maxOff, Math.min(maxOff, v * 0.45));
                    const anchor = {
                      lon: shipGround.lon + clampOff(offLon),
                      lat: shipGround.lat + clampOff(offLat),
                    };
