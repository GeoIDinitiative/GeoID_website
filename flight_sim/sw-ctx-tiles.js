...
const TILE_CACHE  = `geoid-flight-ctx-tiles-${CACHE_VERSION}`;
const ASSET_CACHE = `geoid-flight-assets-${CACHE_VERSION}`;
  ...
  event.waitUntil(
    caches.keys()
      .then(keys => Promise.all(
        keys
          .filter(k => (k.startsWith('geoid-flight-') && !k.endsWith(CACHE_VERSION)))
