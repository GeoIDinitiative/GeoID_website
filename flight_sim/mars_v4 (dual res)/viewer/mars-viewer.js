...
import { moonLatLonToVector3, makeLabelTexture, isVolcanicMoonFeature, isCraterMoonFeature, getMoonFeatureConnectorStart, buildLabelLayer, buildMoonLayer, buildMoonFeatureLabelLayer, getEntryConnectorStart, updateLabelAnchors, rebuildLabelTextures, updateMoonFeatureLabelVisibility, updateMoonVisibility, updateLabelVisibility } from "./label-layer.js";
    ...
    class CTXCanvasLayer {
      ...
      _updateSurround(viewBbox) {
        ...
        // Active whenever the focus window can under-cover the view — that
        // starts at the L7 rung (the mid-zoom framings where the base mosaic
        // showed through around the focus window). Below L7 the ladder's own
        // level already blankets the screen.
        ...
        let sLevel = Math.max(4, Math.min(8, fs.level - 2));
        let range = null;
        for (; sLevel >= 4; sLevel -= 1) {
          const r = this._getFocusTileRange(sLevel, bb);
          r.cMin = Math.max(0, r.cMin);
          r.cMax = Math.min(r.nc - 1, r.cMax);
          if ((r.rMax - r.rMin + 1) * (r.cMax - r.cMin + 1) <= 140) { range = r; break; }
        }
        if (!range) { this._surroundDisplayActive = false; return; }
        const key = `${sLevel}:${range.rMin}:${range.rMax}:${range.cMin}:${range.cMax}`;
        ...
        const nr = range.nr;
        this._surroundBbox = {
          lonMin: (range.cMin / nc) * 360 - 180,
          lonMax: ((range.cMax + 1) / nc) * 360 - 180,
          latMax: 90 - (range.rMin / nr) * 180,
          latMin: 90 - ((range.rMax + 1) / nr) * 180,
        };
      ...
      _drainSurround() {
        ...
        while (this._surroundInflight < 2 && this._surroundQueue.length > 0 && this._focusInflight <= 4) {
