...
// against this script's own URL, so they are absolute here.
//
// RE-FORKED 2026-07-26 from upstream to discard an accumulated round of CTX
// streaming experiments. Flight now inherits the viewer's CTX streaming path
// (the focus-overlay `ctxStreamer`) UNCHANGED — `ctxDetailStreamer` stays dead
// code exactly as upstream leaves it. Every remaining local change is tagged
// "FLIGHT-SIM"; grep for it to re-apply after the next upstream re-copy.
// The tags are, in file order:
//   1. absolute module + label-data imports   (this block, and ~line 177)
//   2. window.MARS_RADIUS_METERS              (render-loop stability)
//   3. FOCUS_MAX_LEVEL clamp to 12            (CTX's real LOD ceiling)
//   3b. _ctxWorkingMaxLevel()                 (the clamp that actually holds —
//       workingMaxLevel preferred __ctxDebug over FOCUS_MAX_LEVEL and bypassed it)
//   4. 1:1 relief override                    (TRUE_SCALE_TERRAIN_RELIEF)
//   5. label distance attenuation + horizon-POI system
//   6. window.__flightSimHooks export
//   7. render-loop handoff to window.__flightSim.update(camera)
//   8. __fsSafeRender wrapper                 (render-loop stability)
// Do NOT add streaming patches here without a live measurement.
...
import { moonLatLonToVector3, makeLabelTexture, isVolcanicMoonFeature, isCraterMoonFeature, getMoonFeatureConnectorStart, buildLabelLayer, buildMoonLayer, buildMoonFeatureLabelLayer, getEntryConnectorStart, updateLabelAnchors, rebuildLabelTextures, updateMoonFeatureLabelVisibility, updateMoonVisibility, updateLabelVisibility } from "/planet_explorer/mars/viewer/label-layer.js";
    ...
    // global in its mosaic close-zoom branch (scale bar ≤ 2 km). Without this,
    // updateLabelVisibility throws a ReferenceError at low altitude, which
    // aborts render() before it schedules the next frame — the render loop dies
    // and the sim freezes. Publishing the constant fixes the reference with the
    // correct value.
    ...
    class CTXCanvasLayer {
      ...
      constructor(baseTexture, serviceConfig = {}, canvasWidth = 8192, canvasHeight = 4096) {
        ...
        // FLIGHT-SIM: clamp to 12 — CTX's real native LOD ceiling.
        //
        // The service's tileInfo advertises LODs 0-17 and the startup probe
        // reports workingMaxLevel 17, but that is a lie for actual tiles.
        // Measured live 2026-07-26 at Jezero (18.4°N 77.5°E):
        //   L10 → 200, L11 → 200, L12 → 200, L13 → 400, L14 → 400.
        // L12 is 5.09 m/px, exactly CTX's native imaging resolution, so the
        // deeper LODs were simply never built.
        //
        // This matters for throughput, not just correctness. The stock default
        // of 15 is harmless in orbit (you never zoom that deep), but at flight
        // proximity `_chooseLevel`'s altitude kickers ask for 13-15 — so every
        // request would walk a doomed fallback chain (15→14→13, all 400) before
        // reaching real imagery, burning the ~6 per-host HTTP/1.1 connections on
        // tiles that cannot exist. Capping at the true ceiling is what makes it
        // safe to run the stock focus-overlay streamer during flight.
        this.FOCUS_MAX_LEVEL = Math.min(
          Number.isFinite(serviceConfig.focusMaxLevel) ? serviceConfig.focusMaxLevel : 15,
          12,
        );
      ...
      // FLIGHT-SIM: the effective LOD ceiling, honouring CTX's real maximum.
      //
      // The level actually requested is derived from `workingMaxLevel`, which
      // preferred `window.__ctxDebug.workingMaxLevel` over this.FOCUS_MAX_LEVEL
      // — so clamping FOCUS_MAX_LEVEL alone did NOT hold. __ctxDebug is written
      // by the startup service probe, and `discoverCtxWorkingMaxLevel` only
      // level-probes PROXY candidates; for a direct ArcGIS candidate it takes
      // the advertised maximum wholesale, i.e. 17. So whenever the /ctx-proxy/
      // route is unavailable and the direct candidate wins, workingMaxLevel
      // became 17 and low altitude (tierLevel forced to ≥15) requested L13-15.
      // Those LODs do not exist — they 400, and because ArcGIS omits the CORS
      // header on error responses they surface as opaque CORS failures — so
      // nothing streamed close to the surface while higher altitudes (which ask
      // for levels that do exist) kept working. That is the exact reported
      // symptom. Clamping here makes the ceiling hold regardless of which
      // candidate won, what the probe reported, or what the 24 h localStorage
      // config cache pinned.
      _ctxWorkingMaxLevel() {
        const reported = Number.isFinite(this.FORCE_MAX_LEVEL)
        ...
        return Math.min(reported, this.FOCUS_MAX_LEVEL);
      }
        ...
        if (this.focusOverlayEnabled) {
          ...
          if (baseLayerSelect.value === "ctx-mosaic" || baseLayerSelect.value === "ctx-mosaic-color") {
            ...
            if ((nowTick - this._lastCamMoveAt) >= settleMs) {
              ...
              // FLIGHT-SIM: clamp this ladder to CTX's real ceiling too.
              //
              // The rungs above go to 13 and 14, and this target is handed to
              // _refreshFocus UNCLAMPED — the workingMaxLevel clamp guards the
              // other (tier) branch only. Worse, for targetLevel < 15 the meta
              // sets `minLevel`/`fetchMinLevel` = targetLevel, so the fallback
              // chain has nowhere to descend to: every tile fails, and the
              // "all tiles failed" handler then parks streaming for 30 s
              // (_focusEnqueuePausedUntil).
              //
              // In ORBIT this is unreachable — CTX_MOSAIC_MIN_SCALEBAR_METERS
              // (10 000) snaps the camera back, pinning the scale bar around
              // 7.2 km / level 10. FLIGHT skips that snap-back, so it is the
              // only mode that reaches these rungs. Measured on descent:
              // 10 km → scale bar 1515 → L13; 6.4 km → 967 → L14; neither
              // exists, which is precisely why tiles stopped streaming close
              // to the surface while higher altitudes kept working.
              if (Number.isFinite(targetLevel)) {
                targetLevel = Math.min(targetLevel, this._ctxWorkingMaxLevel());
              }
              if (!Number.isFinite(targetLevel)) {
          ...
          if (
            ...
            window.__ctxPatchDebug = {
              ...
              workingMaxLevel: this._ctxWorkingMaxLevel(),
            };
            this._drain(true);
            return;
          }
          ...
          const workingMaxLevel = this._ctxWorkingMaxLevel();
    ...
    async function init() {
      ...
      // (≈ 0.02774). flightsim.js reads this via the hooks and sets the stock
      // "Terrain relief" slider to it on engage.
      ...
      // including the CTX mosaics (the stock viewer force-flattens those to
      // zero). Flight needs real terrain under the ship on a CTX basemap, and
      // dragging the slider then applies vertical exaggeration on top.
    ...
    function isObjectActuallyVisible(object) {
      ...
      function updateMeasureVisualScale() {
        ...
        for (const visual of measureVisuals) {
          ...
          // FLIGHT-SIM: labels are normally constant-pixel-size (never shrink with
          // distance) — correct in orbit, but in flight it makes far-horizon labels
          // look oversized. Attenuate marker + label with distance so distant ones
          // recede naturally; ~48 km stays full size, falling to 16% far away.
      ...
      function applyPlanetDisplayState() {
        ...
        // FLIGHT-SIM: snapshot each label's intended (enabled) visibility so the
        // per-frame horizon pass in render() has a stable source of truth to
        // restore from — it toggles sprite/line/marker itself for over-horizon
        // features, so it must not read the value it previously overwrote.
        if (window.__flightSim?.active) {
      ...
      function syncTerrainReliefState() {
        ...
        if (terrainScale) {
          // FLIGHT-SIM: the slider stays live in CTX modes (stock viewer disables
          // it), matching the relief override above.
      ...
      window.__flightSimHooks = {
        ...
        renderFrame: () => render(),
        // FLIGHT-SIM: label layers exposed for the flight horizon-POI system's
        // diagnostics (getters — baseSiteLayer is reassigned on GIS rebuilds).
...
${error && error.message ? error.message : error}`;
      ...
      function render() {
        ...
        _freeTextureImages();
        // FLIGHT-SIM: the loop reschedules itself at the END of render(), so an
        // uncaught per-frame throw used to kill the loop permanently (a frozen
        // screen that read as a "crash"). __fsSafeRender catches, logs at most
        // once a second, and always reschedules ITSELF — even a persistent error
        // degrades to a throttled retry instead of a dead simulator.
