            ...
            if (window.__flightSim?.active || (nowTick - this._lastCamMoveAt) >= settleMs) {
              ...
              if (window.__flightSim?.active) {
                ...
                if (shipGround) {
                  ...
                  // Bands loosened (user: don't cap) — low flight reaches
                  // CTX native L12 (the only ceiling above is the server's:
                  // L13+ returns HTTP 400 planet-wide). At cruise speeds the
                  // speed-scaled window keeps demand bounded; flying low AND
                  // slow gets full native resolution sustainably.
                  const byAlt = altKm <= 10 ? 12 : altKm <= 25 ? 11 : altKm <= 50 ? 10 : altKm <= 110 ? 9 : 8;
                  // FLIGHT-SIM v2 of the choke fix: keep the SHARP altitude
                  // level and shrink the WINDOW with speed instead of dulling
                  // the level (the speed-cap made scale-bar 5-10 km bands read
                  // blurry). Sustainability condition: a round must complete
                  // before the ship crosses it — so size the round in TILES by
                  // speed (48 at reference cruise 0.16 deg/s, more when slow,
                  // floor 24 when fast) and derive the window span from the
                  // level's tile size. Sharp directly under/around the ship,
                  // panorama from the surround, queues drain at cruise.
                  const spd = Number(window.__flightSim.shipSpeedDegPerSec) || 0;
                  const budgetTiles = Math.max(24, Math.min(90, 60 * (0.16 / Math.max(spd, 0.05))));
                  const tileDeg = 360 / (1 << (byAlt + 1));
                  // sqrt(budget) MINUS 3: _getFocusTileRange adds a ±1-tile
                  // pad plus floor/ceil growth (~3 tiles per axis), which
                  // inflated a 48-tile budget to a measured 121-tile round —
                  // right at the supply limit, starving the surround.
                  const span = Math.max(0.3, Math.min(8, tileDeg * Math.max(2, Math.sqrt(budgetTiles) - 3)));
