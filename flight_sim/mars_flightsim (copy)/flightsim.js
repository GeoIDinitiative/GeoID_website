...
(function () {
  ...
  const fs = {
    ...
    maxDetailLevel: 9, // CTX tile-level ceiling (Light, 40 m/px); adaptive by altitude + speed
    update: () => {},
    engage: () => {},
    disengage: () => {},
  };
  ...
  function applyShipScale() {
    if (!ship) return;
    ship.scale.setScalar(shipDisplayLengthUnits() / SHIP_NOMINAL_LENGTH);
  }
  ...
  function engage() {
    ...
    if (hooks.terrainScale) {
      ...
      hooks.terrainScale.value = String(hooks.TRUE_SCALE_TERRAIN_RELIEF ?? 0.0277);
  ...
  flightToggle?.addEventListener("change", () => {
    if (flightToggle.checked) engage(); else disengage();
  });
  ...
  function setCacheStatus(text) {
    if (cacheStatusEl) cacheStatusEl.textContent = "Global CTX base cache: " + text;
  }
  ...
  async function warmGlobalCtxCache() {
    ...
    // The warmer fully suspends during flight (below), so when it IS running the
    // ship isn't using connections — use the full ~6 per-host budget to finish
    // the larger L6 set faster.
    const CONCURRENCY = 6;
    while (done < total) {
