// FLIGHT-SIM FORK of /planet_explorer/mars/viewer/mars-viewer.js.
// This copy is loaded by /flight_sim/mars_flightsim/index.html (which sets
// <base href="/planet_explorer/mars/viewer/"> so every relative asset/data
// fetch still resolves into the real viewer directory). Module imports resolve
// against this script's own URL, so they are absolute here. All flight-sim
// changes are marked with "FLIGHT-SIM" comments; everything else is untouched.
import * as THREE from "/planet_explorer/mars/viewer/vendor/three.module.js";
    import { OrbitControls } from "/planet_explorer/mars/viewer/vendor/OrbitControls.js";
import { moonLatLonToVector3, makeLabelTexture, isVolcanicMoonFeature, isCraterMoonFeature, getMoonFeatureConnectorStart, buildLabelLayer, buildMoonLayer, buildMoonFeatureLabelLayer, getEntryConnectorStart, updateLabelAnchors, rebuildLabelTextures, updateMoonFeatureLabelVisibility, updateMoonVisibility, updateLabelVisibility } from "/planet_explorer/mars/viewer/label-layer.js";
    ...
    // FLIGHT-SIM: import.meta.url points at this fork's directory — resolve
    // label data against the real viewer directory instead.
    const labelData = await fetch('/planet_explorer/mars/viewer/label-data.json').then(r => r.json());
        ...
        if (this.focusOverlayEnabled) {
          ...
          if (baseLayerSelect.value === "ctx-mosaic" || baseLayerSelect.value === "ctx-mosaic-color") {
            ...
            if (Number.isFinite(rawScaleBarMeters)) {
              ...
              // FLIGHT-SIM: skip the zoom-out overlay clear while flying —
              // normal altitude oscillation would wipe streamed tiles mid-flight.
              if (!window.__flightSim?.active && Number.isFinite(prevScaleBar) && rawScaleBarMeters > prevScaleBar * 1.35) {
            ...
            // FLIGHT-SIM: the ship never stops, so the settle gate would block
            // all high-res streaming during flight. Stream continuously instead
            // (tile fetches are de-duplicated and served from the SW cache).
            if (window.__flightSim?.active || (nowTick - this._lastCamMoveAt) >= settleMs) {
              ...
              // FLIGHT-SIM: the chase camera aims at the horizon, so the
              // screen-center scale estimate reads far coarser than the terrain
              // beneath the ship. Floor the level from true camera altitude
              // (scene units — 0.001 ≈ 1.06 km) so low passes actually sharpen.
              // Capped at 12: CTX has no tiles above that (see CTX_NATIVE_MAX_LEVEL).
              if (window.__flightSim?.active) {
                const _fsFloor = altitude <= 0.03 ? 12
                  : altitude <= 0.07 ? 11
                  : altitude <= 0.15 ? 9
                  : null;
                if (_fsFloor !== null) {
                  targetLevel = Math.max(Number.isFinite(targetLevel) ? targetLevel : 0, _fsFloor);
                }
              }
              if (!Number.isFinite(targetLevel)) {
    ...
    class CTXDetailPatchStreamer {
      constructor(marsGroup, tileBase, getTilePayload, getTileLevelCap, getViewBbox, getFocusTarget, elevationSampler, getTerrainRelief, anisotropy = 1) {
        ...
        // FLIGHT-SIM: the lift that floats the draped tiles above the base
        // globe scales with relief so that under vertical exaggeration the
        // tiles clear the base globe's coarse 128→256-segment approximation.
        // At 0.01 the lift (3.5 km at ×9 exaggeration) was smaller than the
        // globe's per-segment deviation, so the low-res base poked THROUGH the
        // sharp tiles. 0.045 lifts them ~13 km at full exaggeration — clear of
        // the poke — while staying ~2 km at true scale. The ship references the
        // lifted tile surface, so this doesn't affect flight altitude.
        this.surfaceLiftReliefFactor = 0.045;
        this.minLevel = 3;
        this.maxLevel = 17;
        // FLIGHT-SIM: every tile comes from one host (astro.arcgis.com), and
        // browsers cap concurrent connections per host at ~6. The stock 48 (and
        // even 16) does not buy parallelism — the surplus just sits in the
        // browser's own queue, so the streamer believes 16 fetches are active
        // while only 6 are moving, its queue never drains, and newly requested
        // tiles wait behind stale ones. It also exhausted the socket pool
        // (ERR_INSUFFICIENT_RESOURCES surfacing as "Failed to fetch"). Matching
        // the real limit keeps the queue honest and priority-ordered.
        this.maxInflight = 6;
        this.maxMeshes = 700;
      ...
      // FLIGHT-SIM: this method existed only on the focus-overlay streamer;
      // _buildBgTileGrid below referenced it but the detail streamer was dead
      // code in the stock viewer so the gap never surfaced. Same pure math.
      _getFocusTileRange(level, bbox) {
        const nc = 1 << (level + 1);
        const nr = 1 << level;
        const pad = level >= 11 ? 0 : 1;
      ...
      // FLIGHT-SIM: the level for the tiles DIRECTLY under/ahead of the ship
      // (the immediate field of view). This is the FINEST level of the
      // hierarchical pyramid — coarser rings grade outward from it (see the
      // ring builder in update()). It scales with altitude because the ground
      // is farther away when you're high, and is clamped to the user's chosen
      // ceiling (window.__flightSim.maxDetailLevel). altitude is in scene units
      // (1 unit ≈ 1 061 km). Requests resolve via fallback, so where the finest
      // level 404/500s in a region the tile lands on the finest that works.
      _flightCenterLevel(altitudeUnits) {
        const cap = Math.max(6, Math.min(this.maxLevel, Number(window.__flightSim?.maxDetailLevel) || this.maxLevel));
        const km = altitudeUnits * (this.metersPerUnit || 1061309) / 1000;
        let byAlt;
        if (km <= 6) byAlt = 12;         // skim — CTX native 5 m/px
        else if (km <= 15) byAlt = 11;   // low pass — 10 m/px
        else if (km <= 35) byAlt = 10;   // cruise — 20 m/px
        else if (km <= 80) byAlt = 9;    // high cruise
        else if (km <= 180) byAlt = 8;   // very high
        else byAlt = 7;                  // near orbit
        return Math.min(cap, byAlt);
      }
      // Back-compat alias.
      _flightTargetLevel(a) { return this._flightCenterLevel(a); }
      ...
      _getStage(camera, bbox, viewBbox = bbox) {
        ...
        // FLIGHT-SIM: the ship never settles — stream continuously in flight.
        const isSettled = Boolean(window.__flightSim?.active) || (now - this._lastMotionAt) >= this._settleDelayMs;
        ...
        // FLIGHT-SIM: hysteresis on downward level changes.
        //
        // The stock rule drops _smoothedStageLevel to desiredLevel *immediately*
        // on any decrease, then climbs back one step at a time. In flight the
        // desired level jitters constantly (altitude wobble, and view.bbox comes
        // from a raycast against moving terrain), so the level was snapping down
        // and crawling back up over and over — that is the resolution flicker.
        // Require a sustained drop before following it down, so a transient dip
        // never discards the fine tiles that are already on screen.
        const flightMode = Boolean(window.__flightSim?.active);
        let allowDrop = true;
        if (flightMode && Number.isFinite(this._smoothedStageLevel) && this._smoothedStageLevel > desiredLevel) {
          if (this._pendingDropLevel !== desiredLevel) {
            this._pendingDropLevel = desiredLevel;
            this._pendingDropSince = now;
          }
          const sustainedMs = now - this._pendingDropSince;
          // A 2+ level gap is a real altitude change — follow it quickly.
          // A single-level dip must persist before it is believed.
          allowDrop = (this._smoothedStageLevel - desiredLevel >= 2)
            ? sustainedMs >= 250
            : sustainedMs >= 900;
        } else {
          this._pendingDropLevel = null;
        }
        if (!Number.isFinite(this._smoothedStageLevel) || (this._smoothedStageLevel > desiredLevel && allowDrop)) {
      ...
      _createGeometry(bounds, overlap = 0) {
        ...
        // FLIGHT-SIM: segment density must follow the DEM closely enough that
        // coarse tiles don't read as flat slabs beside the sharp near patch
        // under vertical exaggeration. Cap 20 / divisor 0.1° left level-7
        // medium tiles at ~14 segments (0.1°/seg) which flattened out at high
        // relief. When exaggeration is on, use a finer divisor (0.05°) and a
        // higher cap so a level-7 tile gets ~28 segments and follows the
        // terrain like the surrounding fine tiles. Low relief keeps the cheaper
        // 20/0.1° budget since flat tiles need few segments there.
        const highRelief = window.__flightSim?.active && relief > 0.05;
        const segCap = highRelief ? 48 : (window.__flightSim?.active ? 20 : 36);
        const segDiv = highRelief ? 0.05 : 0.1;
        const segLon = flatMode
          ? Math.max(2, Math.min(10, Math.ceil(lonSpan / 0.35)))
          : Math.max(10, Math.min(segCap, Math.ceil(lonSpan / segDiv)));
        const segLat = flatMode
          ? Math.max(2, Math.min(10, Math.ceil(latSpan / 0.35)))
          : Math.max(10, Math.min(segCap, Math.ceil(latSpan / segDiv)));
      ...
      _createMesh(level, row, col, image) {
        ...
        texture.needsUpdate = true;
        // FLIGHT-SIM: depthTest is ON in this fork (stock viewer used false).
        // Only flight mode uses these meshes, and the ship must occlude
        // correctly against the terrain; the drape lift keeps tiles above the
        // displaced globe so they still win the depth test against it.
        ...
        // FLIGHT-SIM: Viking colorization is done on the GPU here rather than by
        // compositing canvases per tile on the CPU (2 canvas allocations and 3
        // filtered draws per tile was a visible frame hitch while streaming).
        // uColorUv maps this tile's local uv into the global equirect mosaic.
        const colorMap = this.colorMapProvider ? this.colorMapProvider() : null;
        const colorUvTransform = new THREE.Vector4(
          (bounds.lonMin + 180) / 360,
          (90 - bounds.latMin) / 180,
          (bounds.lonMax - bounds.lonMin) / 360,
          -(bounds.latMax - bounds.latMin) / 180,
        );
        material.customProgramCacheKey = () => 'ctx-detail-tile';
        material.onBeforeCompile = (shader) => {
          material.userData.ctxContourShader = shader;
          shader.uniforms.uCtxColorMap = { value: colorMap };
          shader.uniforms.uCtxColorMix = { value: colorMap ? 1 : 0 };
          shader.uniforms.uCtxColorUv = { value: colorUvTransform };
          shader.uniforms.uContourMap = { value: this.contourTexture };
          shader.uniforms.uContourOpacity = { value: this.contourOpacity };
          shader.uniforms.uContourEnabled = { value: this.contourEnabled ? 1 : 0 };
          ...
          shader.fragmentShader = shader.fragmentShader
            .replace(
              "#include <common>",
              "#include <common>\nuniform sampler2D uContourMap;\nuniform float uContourOpacity;\nuniform float uContourEnabled;\nuniform vec2 uContourTexel;\nuniform float uContourThickness;\nuniform float uContourInterval;\nuniform float uContourMinMeters;\nuniform float uContourReliefMeters;\nvarying vec2 vContourUv;\nuniform sampler2D uCtxColorMap;\nuniform float uCtxColorMix;\nuniform vec4 uCtxColorUv;",
            )
            .replace(
              "#include <map_fragment>",
              `#include <map_fragment>
              if (uCtxColorMix > 0.5) {
                // Grayscale CTX detail × Viking global mosaic, with a lift to
                // offset the darkening that multiplying two mid-tones causes.
                vec2 ctxColorUv = uCtxColorUv.xy + (vMapUv * uCtxColorUv.zw);
                vec3 ctxTint = texture2D(uCtxColorMap, ctxColorUv).rgb;
                diffuseColor.rgb = clamp(diffuseColor.rgb * ctxTint * 1.9, 0.0, 1.0);
              }
      ...
      async _fetch({ level, row, col, key, epoch, priority = 0 }) {
        ...
        try {
          // FLIGHT-SIM: two fixes over the stock `max(activationMinLevel, level-3)`.
          //
          // 1. The floor must never exceed the requested level. Background
          //    tiles are requested at coarse levels (5-8), below the level-9
          //    activation floor — the fallback loop `while (level >= minLevel)`
          //    then exited immediately and returned null without ever issuing
          //    a fetch, so background coverage never rendered at all.
          // 2. CTX regional coverage varies enormously (some areas top out at
          //    level 9, others reach 17). Descending all the way to the floor
          //    instead of only 3 levels lets sparse regions land on their best
          //    available imagery rather than resolving to nothing.
          const fallbackMinLevel = Math.min(level, Math.max(this.minLevel, this.activationMinLevel));
          const payload = await this.getTilePayload(level, row, col, fallbackMinLevel, controller.signal);
          if (controller.signal.aborted) {
            return;
          }
          if (!payload) {
            // FLIGHT-SIM: only a genuine network outage should trigger the
            // global backoff. Region-specific server errors (some CTX areas
            // return HTTP 500 for a whole level — e.g. level 8 near 90°E — while
            // finer levels there work fine) used to count toward the streak and
            // fire an 8 s backoff that stalled ALL loading, including the levels
            // that do resolve. Gate the backoff on navigator.onLine so those
            // gaps just blacklist the dead tile and move on.
            this._nullPayloadStreak = (this._nullPayloadStreak || 0) + 1;
            if (this._nullPayloadStreak >= 40 && navigator.onLine === false) {
              this._knownBlank.clear();
              this.queue = [];
              this.queued.clear();
              this._backoffUntil = performance.now() + 8000;
              this._nullPayloadStreak = 0;
            } else {
              // Mark this tile blank so _enqueue never re-queues it.
      ...
      _flushStagingQueue() {
        ...
        // FLIGHT-SIM: _cameraSettled is forced true while flying (the ship never
        // stops), which would take the 8/frame branch — 8 geometry builds plus
        // 8 texture uploads in one frame is a visible hitch at 60 fps. Cap it
        // at 3 during flight: still ~180 tiles/sec, but spread smoothly.
        const maxPerFrame = window.__flightSim?.active ? 5 : (this._cameraSettled ? 8 : 2);
      ...
      _processDisposalQueue() {
        // Release GL resources per frame without a synchronous WebGL stall.
        // During flight the pool churns fast (small resident disc + constant
        // motion), so free more aggressively to keep GPU memory bounded; the
        // service-worker disk cache makes any re-fetch of an evicted tile cheap.
        const batch = window.__flightSim?.active ? 16 : 8;
      ...
      rebuild() {
        // FLIGHT-SIM: rebuilding every tile's geometry in one synchronous pass
        // stalls for ~1 s once a few hundred tiles are loaded, and the terrain
        // relief slider fires this on every input event while being dragged.
        // Queue the work instead and spread it across frames in update().
        if (window.__flightSim?.active) {
          this._geometryRebuildQueue = [...this.meshes.keys()];
          return;
        }
        for (const mesh of this.meshes.values()) {
      ...
      // Rebuild a few queued tile geometries per frame (see rebuild()).
      _processGeometryRebuildQueue(maxPerFrame = 4) {
        if (!this._geometryRebuildQueue || this._geometryRebuildQueue.length === 0) return;
        let count = 0;
        while (count < maxPerFrame && this._geometryRebuildQueue.length > 0) {
          const key = this._geometryRebuildQueue.shift();
          const mesh = this.meshes.get(key);
          if (!mesh || !mesh.userData.bounds) continue;
          mesh.geometry.dispose();
          mesh.geometry = this._createGeometry(mesh.userData.bounds, 0);
          count += 1;
        }
      }
      ...
      update(camera) {
        ...
        // Stage 1b: spread queued relief-driven geometry rebuilds across frames.
        this._processGeometryRebuildQueue();
        ...
        // FLIGHT-SIM: network-outage backoff (see the null-payload streak
        // handling in _fetch) — pause all tile work until the window expires.
        if (this._backoffUntil && performance.now() < this._backoffUntil) {
          return;
        }

        // FLIGHT-SIM: use the streamer's native camera-raycast focus (the point
        // the camera looks at), same as orbit mode. The per-tile quadtree LOD
        // below refines by projected size, so near tiles get fine detail and far
        // tiles stay coarse across the whole grazing view — no single-focus ring.
        ...
        // FLIGHT-SIM: in flight the concentric rings (below) own LOD entirely,
        // so `level` is just the ring centre level — the DFS detail layer is
        // disabled for flight and this only sets bookkeeping (currentLevel,
        // render order). The old clamp forced BOTH the DFS and the near patch to
        // one altitude-derived level, flattening the hierarchy → uniformly low
        // resolution. Now the rings give the finest level in the centre.
        // FLIGHT-SIM: use the native stage level (from _chooseLevel's altitude
        // kickers + span). The per-tile DFS refines from here.
        ...
        let backgroundTiles = [];
        let bgBbox = view.bbox;
        if (window.__flightSim?.active) {
          // FLIGHT-SIM: gap-free coverage. Cover the ENTIRE visible view at the
          // FINEST level whose full grid fits the budget — a complete flat grid,
          // so there are never holes showing the coarse base. The level adapts
          // to how much ground is visible: on a low pass the view spans less, so
          // the whole surface is fine; higher up it spans more, so it steps a
          // level coarser but stays fully covered. Counting DOWN from the stage
          // level (its altitude kickers give the fine target) lands on the
          // finest gap-free level automatically. Fallback resolves any level
          // that 404/500s in a region onto the finest that works.
          // Budget must stay well under maxMeshes so there's always headroom for
          // eviction churn and the instant base — a grid bigger than the pool
          // can never fully load, leaving permanent gaps (and thrashing memory).
          const FLIGHT_BUDGET = 550;
          // SPEED-ADAPTIVE DETAIL CEILING. At cruise the ship crosses a fresh
          // tile-disc far faster than 6 connections can load it (~200 fine tiles
          // = ~30 s to fetch vs a disc crossed in a second or two), so every fine
          // tile is stale before it arrives: the ground never maps and the
          // endless fetch→build→dispose churn is what thrashes GPU memory into a
          // context-loss crash. So cap the requested level to what can actually
          // finish loading at the current speed — coarse levels are few, big, and
          // mostly in the warm cache, so they paint the base INSTANTLY with almost
          // no churn. Detail sharpens only as the ship slows toward a hover.
          // Thresholds from "can the disc finish loading before the ship crosses
          // it?": an 8° disc at level L is ~(8·2^(L+1)/360)² tiles, loading at
          // ~4 effective tiles/s (allowing for latency + fallback chains), while
          // the ship crosses it in 8/spd seconds. Solving load<cross gives the
          // spd ceilings below. Coarse discs are tiny (L7 ≈ 9 tiles) so cruise
          // (~0.4°/s at ×10) still sustains L8; only a near-hover earns L11-12.
          const spd = Number(window.__flightSim?.shipSpeedDegPerSec) || 0;
          const IC = this.INSTANT_CACHED_LEVEL ?? 5;
          // SPEED CEILING — yields ONLY reliable CTX levels (7 & 8 are broken
          // across large stretches; see the disc note below). Fast ⇒ a coarse
          // reliable level that keeps up; slow ⇒ fine. There is no 7/8 bucket, so
          // cruise steps straight from L9 down to L6 rather than through the
          // broken middle that made the ground go blank.
          let speedCeil;
          if (spd < 0.01)      speedCeil = this.maxLevel; // hover        → full detail
          else if (spd < 0.05) speedCeil = 10;            // ×1 cruise    → L10 (20 m/px)
          else if (spd < 0.20) speedCeil = 9;             // slow         → L9  (40 m/px)
          else                 speedCeil = IC + 1;        // cruise/boost → L6  (325 m/px, reliable, keeps up)
          const cap = Math.max(3, Math.min(this.maxLevel, Number(window.__flightSim?.maxDetailLevel) || this.maxLevel, speedCeil));
          // The disc's FINE ceiling comes from the ship's ALTITUDE, not `level`
          // (= stage.level). stage.level is driven by the horizon-spanning grazing
          // view so it's always coarse (~7); using it as startLevel capped the
          // disc at ~7 forever and a hover could never sharpen. _flightCenterLevel
          // maps altitude→appropriate fine level (32 km → L10); the budget fit and
          // the speed cap then reduce it as needed.
          let altLevel = this._flightCenterLevel(Number(window.__flightSim?.shipAltUnits) || 0);
          // The altitude curve can land on the BROKEN L7/L8 (e.g. 120 km → L8).
          // Snap that UP to the reliable, finer L9 — NOT down to L6 — so a slow
          // high pass still gets 40 m/px. L9 is reliable everywhere tested and
          // fits the disc budget at slow-speed disc sizes; when the ship is fast
          // the speed ceiling (cap) below pulls it back to L6 anyway.
          if (altLevel === 7 || altLevel === 8) altLevel = 9;
          const startLevel = Math.min(altLevel, cap);
          // Cover a DISC centred on the ship (not just the forward view) so the
          // ground behind the ship stays mapped and a U-turn shows detail without
          // a full rebuild. Radius follows the view reach but is CAPPED small so
          // the tile count — and GPU memory — stays bounded. Beyond the disc the
          // instant level-5 base (from cache) fills in; the disc is what carries
          // the higher-res mapping.
          const shipLL = window.__flightSim?.shipLatLon;
          const viewHalf = Math.max((view.bbox.lonMax - view.bbox.lonMin) / 2, (view.bbox.latMax - view.bbox.latMin) / 2, 0.05);
          // SPEED-ADAPTIVE DISC RADIUS. When moving fast you must pre-map the
          // ground ahead, so the disc is wide; when slow/hovering you only need
          // what's around you, so it TIGHTENS — and a tighter disc lets a finer
          // level fit the same tile budget (L10 fits a 3° disc but not an 8° one).
          // This is what lets a hover sharpen to L10 while cruise stays at L8.
          const discRadius = Math.max(1.5, Math.min(4, 1.5 + spd * 3));
          const half = Math.min(viewHalf, discRadius);
          const coverBbox = shipLL ? {
            lonMin: shipLL.lon - half, lonMax: shipLL.lon + half,
            latMin: Math.max(-90, shipLL.lat - half), latMax: Math.min(90, shipLL.lat + half),
          } : view.bbox;
          // Finest RELIABLE level whose full disc grid fits the budget. Iterate
          // reliable levels only (skip the broken 7/8) so if L9 doesn't fit it
          // drops straight to L6 rather than the broken L8.
          const RELIABLE_DESC = [12, 11, 10, 9, 6, 5, 4, 3];
          let fitLevel = 3;
          for (const tryLevel of RELIABLE_DESC) {
            if (tryLevel > startLevel) continue;
            if (this._getFocusTileRange(tryLevel, coverBbox).tileCount <= FLIGHT_BUDGET) { fitLevel = tryLevel; break; }
          }
          // SKIP THE BROKEN MIDDLE (levels 7 & 8). Both return HTTP 500 for the
          // WHOLE level across large stretches of the planet (measured: L8 at
          // ≈8°E; L7 AND L8 at ≈33°E, 89°E, 137°E, 230°E …). The disc requests a
          // SINGLE level, so landing on a broken one there makes every tile 500 →
          // blank → nothing renders, and the 6 host connections choke on slow
          // 500→fallback chains ("no tiles, then it hangs"). L5,L6,L9,L10 are
          // reliable everywhere tested. Snap the budget-fit DOWN to the nearest
          // reliable level (7/8 → 6: L9 isn't affordable when the fit is only
          // 7/8). The RELIABLE ladder treats 6 and 9 as adjacent so the level can
          // actually traverse the gap instead of getting pinned at 6.
          const RELIABLE = [3, 4, 5, 6, 9, 10, 11, 12];
          const snapDown = (L) => { let r = RELIABLE[0]; for (const x of RELIABLE) if (x <= L) r = x; return r; };
          const fitR = Math.max(3, Math.min(cap, snapDown(fitLevel)));
          // TEMPORAL HYSTERESIS on the reliable ladder: COARSEN immediately (never
          // hold a level finer than fits/affords — that would gap or thrash), but
          // only SHARPEN after the finer level has been wanted for several
          // consecutive frames, so a brief speed dip doesn't flip the whole disc.
          // This replaces the old ±1 level hysteresis, which — combined with the
          // 7/8 skip — dead-locked the disc at L6 (every step-up landed on the
          // snapped-back 7). Coarsening-fast / sharpening-slow is the standard LOD
          // rule and prevents the 6↔9 flicker.
          const last = this._flightBgLevel;
          let chosen = fitR;
          if (Number.isFinite(last) && fitR > last) {
            this._flightSharpenStreak = (this._flightSharpenStreak || 0) + 1;
            if (this._flightSharpenStreak < 6) chosen = last; // hold coarser until sustained
          } else {
            this._flightSharpenStreak = 0;
          }
          // Build cap == the fit budget: at chosen ≤ fitLevel the full disc grid
          // fits, so nothing is truncated and the pool stays within maxMeshes.
          let grid = this._buildBgTileGrid(coverBbox, chosen, FLIGHT_BUDGET, focusTarget);
          if (grid.length === 0) { chosen = fitLevel; grid = this._buildBgTileGrid(coverBbox, chosen, FLIGHT_BUDGET, focusTarget); }
          this._flightBgLevel = chosen;
          backgroundTiles = grid;
          bgLevel = chosen;
          bgBbox = coverBbox;
        } else if (backgroundTiles.length === 0 && bgLevel < level) {
          backgroundTiles = this._buildBgTileGrid(bgBbox, bgLevel, 480, focusTarget);
          while (backgroundTiles.length === 0 && bgLevel > 3) {
            bgLevel -= 1;
            backgroundTiles = this._buildBgTileGrid(bgBbox, bgLevel, 480, focusTarget);
          }
        }
        // FLIGHT-SIM: Layer 0 — instant coverage from the pre-warmed cache.
        // The background level above is chosen by view area and typically lands
        // on 7-9, which is NOT pre-cached, so every tile costs a ~300 ms round
        // trip and the ground stays blank on arrival at new terrain. Levels
        // 0-INSTANT_CACHED_LEVEL are warmed globally at page load (flightsim.js)
        // and come back from the service worker in ~5 ms, so enqueueing them
        // first paints the whole viewport almost immediately; the finer layers
        // then refine on top.
        let instantTiles = [];
        let mediumTiles = [];
        if (window.__flightSim?.active) {
          // Instant backdrop = the globally pre-warmed L6 (325 m/px), covering the
          // WHOLE visible view straight from cache in ~5 ms. Build it whenever the
          // disc is at L6 OR finer (instantLevel <= bgLevel), so even at cruise —
          // where the disc itself is L6 — the wide horizon is still painted at
          // 325 m/px instead of dropping to the low-res global Viking base. Capped
          // at 300 tiles: it's a big area (each L6 tile ≈ 2.8°) and every tile is
          // eviction-PROTECTED, so an unbounded grid would crowd the mesh pool.
          const instantLevel = this.INSTANT_CACHED_LEVEL ?? 6;
          if (instantLevel >= this.minLevel && instantLevel <= bgLevel) {
            instantTiles = this._buildBgTileGrid(view.bbox, instantLevel, 300, focusTarget);
          }
          // FLIGHT-SIM: COARSE SEED over the disc. A fresh (uncached) region has
          // to stream its whole fine grid live (~240 tiles at L9 ≈ 40 s over 6
          // connections), so "right off the bat" the ground shows only the soft
          // global base until a big fraction lands. This seed is a HANDFUL of
          // mid-level tiles (bgLevel−2) blanketing the same disc, enqueued AHEAD
          // of the fine grid (priority −340 vs −150 below): ~25 tiles load in a
          // few seconds and paint the whole region at recognisable detail while
          // the fine tiles refine center-out on top. They're ancestors of the
          // fine tiles, so _syncMeshVisibility hides them once the children land —
          // no wasted draws. Level is floored at INSTANT+1 so it never collides
          // with the L5 instant layer, and skipped when the disc is already
          // coarse (bgLevel ≤ INSTANT+2). bgBbox/bgLevel are the disc set above.
          // No separate coarse seed: the instant layer is now L6 (INSTANT_CACHED
          // _LEVEL, globally pre-warmed at 325 m/px), so it already blankets the
          // whole view from cache in ~5 ms. A seed at INSTANT+1 (=L7) would only
          // hit the broken flaky level and re-introduce the connection-starvation
          // stall, so it stays disabled.
          mediumTiles = [];
        }
        // Layer 2 — Detail DFS (per-tile quadtree LOD). Runs in flight too now:
        // it splits tiles by projected screen size, so near tiles get fine
        // detail and far tiles stay coarse across the whole view.
        const tileDegLon = 360 / (1 << (level + 1));
        const tileDegLat = 180 / (1 << level);
        const detailTileBudget = 480;
        // FLIGHT-SIM: the gap-free full-view grid (backgroundTiles) is the sole
        // detail layer in flight, so skip the DFS.
        let broadTiles = [];
        if (!window.__flightSim?.active) {
          broadTiles = this._buildBgTileGrid(view.bbox, level, detailTileBudget, focusTarget);
        ...
        // FLIGHT-SIM: the gap-free grid (backgroundTiles) is the detail source,
        // so the deepest/current level comes from it (desiredTiles is empty).
        const deepestSource = window.__flightSim?.active ? backgroundTiles : desiredTiles;
        const deepestLevel = deepestSource.reduce((maxLevel, tile) => Math.max(maxLevel, tile.level), this.minLevel);
        ...
        const instantKeys = new Set(instantTiles.map((t) => t.key));
        const mediumKeys = new Set(mediumTiles.map((t) => t.key));
        ...
        const allKeys = new Set([...desiredKeys, ...bgKeys, ...instantKeys, ...mediumKeys]);
        if (window.__flightSim?.active) {
          // FLIGHT-SIM: keep only coarser tiles that still intersect the
          // viewport. The stock rule protected EVERY tile coarser than the
          // stage level, so as the ship flew across the surface, level-10 tiles
          // from hundreds of km back were never evicted (all "level < 12") —
          // the mesh set grew past 2 300 against a 700 budget, and the flood of
          // stale tiles starved the near-field fetches (only ~6 slots, all busy
          // re-confirming tiles already on screen). Tiles behind the ship are
          // now eligible for eviction.
          // Protect coarse tiles against the tight near patch (bgBbox), NOT
          // view.bbox — at cruise altitude the raycast view.bbox spans the whole
          // horizon, so protecting against it kept every level-10 tile ever
          // loaded (1000+) and the leak returned. The instant layer (level 5)
          // covers the far field, so only near-field coarse tiles need keeping.
          const keepBbox = {
            lonMin: bgBbox.lonMin - 1, lonMax: bgBbox.lonMax + 1,
            latMin: bgBbox.latMin - 1, latMax: bgBbox.latMax + 1,
          };
          for (const key of this.meshes.keys()) {
            const keyLevel = Number(key.split('/')[0]);
            if (keyLevel >= level) continue;
            if (keyLevel <= this.INSTANT_CACHED_LEVEL) { allKeys.add(key); continue; }
            const b = this.meshes.get(key)?.userData?.bounds;
            if (b && this._tileIntersectsBbox(b, keepBbox)) allKeys.add(key);
          }
        } else {
          for (const key of this.meshes.keys()) {
            const keyLevel = Number(key.split('/')[0]);
        ...
        if (largeAreaChange) {
          ...
          this._stagingQueue = [];
          this._staged.clear();
          // FLIGHT-SIM: a moving ship crosses the "large area change" threshold
          // regularly. Wiping in-flight fetches and resetting the stage level
          // restarts the coarse→fine climb from 3 levels down every time, which
          // reads as the whole surface dropping to mush and re-sharpening.
          // Keep both while flying — the tiles being fetched are still ahead of us.
          if (!window.__flightSim?.active) {
            this._abortInflight();
            this.inflight.clear();
            this._requestEpoch += 1;
        ...
        // FLIGHT-SIM: never abort in-flight fetches while flying. The desired
        // set shifts continuously with the moving ship, and 404-fallback chains
        // in sparse-coverage regions take >1 s — aborting on every state change
        // meant no fetch ever completed. Tiles just behind the ship are still
        // useful, and LRU eviction reclaims them under memory pressure.
        if (!window.__flightSim?.active) {
        ...
        const nowMs = performance.now();
        // FLIGHT-SIM: cached instant tiles go first (priority below every other
        // layer) so the viewport is never blank while real detail loads.
        for (const tile of instantTiles) {
          if (!desiredKeys.has(tile.key) && !bgKeys.has(tile.key) && !mediumKeys.has(tile.key)) {
            this._enqueue(tile.level, tile.row, tile.col, (tile.priority?.dist2 || 0) * 0.01 - 400);
          }
          const instantMesh = this.meshes.get(tile.key);
          if (instantMesh) instantMesh.userData.lastUsed = nowMs;
        }
        // FLIGHT-SIM: mid-field ring — between the instant base and the fine
        // near patch, so the flanks of the sharp strip aren't bare level-5.
        for (const tile of mediumTiles) {
          if (!desiredKeys.has(tile.key) && !bgKeys.has(tile.key)) {
            // Load the medium ring right alongside the near patch (−340 vs the
            // near −350), not far behind it. On a slow server the old −260
            // left the ring starving, so gaps around the sharp patch showed the
            // very coarse level-5 base as flat slabs. Filling the ring with the
            // medium level promptly keeps coverage consistent while the centre
            // sharpens.
            this._enqueue(tile.level, tile.row, tile.col, (tile.priority?.dist2 || 0) * 0.01 - 340);
          }
          const medMesh = this.meshes.get(tile.key);
          if (medMesh) medMesh.userData.lastUsed = nowMs;
        }
    ...
    async function init() {
      ...
      function handleSurfaceWheelZoom(event) {
        ...
        // FLIGHT-SIM: wheel zoom is disabled while flying (throttle owns speed).
        if (window.__flightSim?.active) {
          event.preventDefault?.();
          return;
        }
      ...
      // FLIGHT-SIM: true-scale relief (1:1 DEM, no vertical exaggeration).
      // relief_m spans the full elevation range; the sphere radius (3.2 units)
      // maps to MARS_RADIUS_METERS, so 1:1 displacement is a fixed ratio.
      const TRUE_SCALE_TERRAIN_RELIEF = 3.2 * (Number(manifest.elevation?.relief_m ?? 29442) / MARS_RADIUS_METERS);
      // FLIGHT-SIM: in flight the stock "Terrain relief" slider (Basemap and
      // Relief section) drives relief directly, exactly like the normal viewer
      // — including in CTX modes, which the stock viewer flattens to zero.
      // flightsim.js sets the slider to true scale (1:1) on engage; dragging
      // it applies vertical exaggeration.
      // FLIGHT-SIM: the "Terrain relief" slider in Basemap and Relief drives
      // relief in every mode, including the CTX mosaics (the stock viewer
      // force-flattens those to zero and disables the slider). Both the base
      // globe and the draped CTX detail tiles follow it, so vertical
      // exaggeration is adjustable while CTX imagery is active.
      const getRequestedTerrainRelief = () => {
        if (!elevationMap) return 0;
        return Number(terrainScale.value);
      };
      const getEffectiveTerrainRelief = () => getRequestedTerrainRelief();
      ...
      // FLIGHT-SIM: 256 segments (was 128) so the base globe follows the DEM
      // more closely under vertical exaggeration — a coarse globe poked its
      // low-res base texture through the sharp draped tiles.
      const sphereGeometry = new THREE.SphereGeometry(3.2, 256, 256);
...
uniform float uViewportWidth;`,
      ...
      // FLIGHT-SIM: CTX_NATIVE_MAX_LEVEL is the real imagery ceiling.
      //
      // The service's tileInfo advertises LODs 0-17 and the startup probe
      // reports workingMaxLevel 17, but that is a lie for actual tiles: every
      // request at level 13+ returns HTTP 400 anywhere on the planet. Level 12
      // is 5.09 m/px, which is exactly CTX's native imaging resolution — there
      // is no more detail to serve, so the deeper LODs were never built.
      //
      // This matters for throughput, not just correctness. _chooseLevel's
      // altitude kickers ask for 14-17 at flight altitude, so with a higher cap
      // EVERY tile request walked a doomed fallback chain (17→16→15→14→13, all
      // 400) before reaching real imagery — four wasted round trips per tile
      // against 16 concurrent slots, which is why the ground stayed soft.
      // Capping at the true ceiling means the first request already targets the
      // sharpest tile that exists.
      //
      // Coverage at 12 is patchy (present at e.g. Jezero and Gale, absent over
      // much of the planet); the fallback resolves those to 11 (10 m/px) or 10
      // (20 m/px), and _getTileLevelCap remembers the region's real depth.
      const CTX_NATIVE_MAX_LEVEL = 12;
      ctxDetailStreamer.maxLevel = Math.min(ctxStreamer.FOCUS_MAX_LEVEL, CTX_NATIVE_MAX_LEVEL);
      ctxDetailStreamer.activationMinLevel = Math.min(ctxDetailStreamer.activationMinLevel, ctxDetailStreamer.maxLevel);
      // FLIGHT-SIM: Viking mosaic used to tint grayscale CTX detail tiles on the
      // "CTX Mosaic (Color)" basemap, and the globally pre-warmed cache level
      // that provides instant coverage (must match WARM_MAX_LEVEL in flightsim.js).
      ctxDetailStreamer.colorMapProvider = () => (
        baseLayerSelect.value === "ctx-mosaic-color"
          ? (layerTextures.get("viking-color") || null)
          : null
      );
      ctxDetailStreamer.INSTANT_CACHED_LEVEL = 6; // must match WARM_MAX_LEVEL (flightsim.js)
      // FLIGHT-SIM: hold the full flight working set — near-field grid (~360) +
      // detail layer (~440) + instant/coarse — without evicting tiles that are
      // still on screen. The eviction fix above keeps this from growing
      // unbounded as the ship moves.
      // BOUNDED mesh pool. Each live tile holds a GPU texture (256²≈0.34 MB with
      // mipmaps), so an unbounded pool blows out GPU memory and crashes the
      // context. The ceiling must sit ABOVE the flight protected set — disc grid
      // (≤550) + instant level-5 backdrop (≤300) + near coarse (~100) ≈ 950 —
      // with headroom, or _evict can never drop below it and the pool grows
      // unbounded anyway (the real crash). 1400 → ~450 eviction headroom, ≈480 MB
      // GPU, safe. Agility comes from fast eviction + the service-worker disk
      // cache (revisited tiles reload in ~5 ms), NOT from hoarding meshes.
      ctxDetailStreamer.maxMeshes = 1400;
      ctxDetailStreamer.metersPerUnit = MARS_RADIUS_METERS / 3.2;
    ...
    function isObjectActuallyVisible(object) {
      ...
      let _reliefReanchorTimer = null; // FLIGHT-SIM: see syncTerrainReliefState
      function syncTerrainReliefState() {
        ...
        // Re-anchoring labels, contours and geology linework re-samples the DEM
        // for every entry — ~250 ms with labels enabled. Material displacement
        // above is a uniform update and is already live, so during flight this
        // tail is debounced to the end of a slider drag instead of running on
        // every input event.
        const _reanchorReliefDependents = () => {
        ...
        if (window.__flightSim?.active) {
          clearTimeout(_reliefReanchorTimer);
          _reliefReanchorTimer = setTimeout(_reanchorReliefDependents, 260);
        } else {
          _reanchorReliefDependents();
        }
        if (terrainScale) {
          // FLIGHT-SIM: the slider stays live in CTX modes (stock viewer disables it).
      ...
      // flightsim.js waits for the "flightsim:hooks-ready" event (or polls
      // window.__flightSimHooks) before wiring the flight-mode UI.
      window.__flightSimHooks = {
        ...
        // Debug/testing: drive one full render-loop frame on demand (note:
        // render() self-schedules a rAF, so use sparingly outside tests).
        renderFrame: () => render(),
      };
...
${error && error.message ? error.message : error}`;
      ...
      function render() {
        ...
        // ─────────────────────────────────────────────────────────────────────
        // FLIGHT-SIM: while the flight simulator is engaged it owns the camera.
        // OrbitControls updates, surface-barrier clamps and the CTX zoom-floor
        // snap-back are all skipped; flight physics enforces its own floor.
        ...
        // FLIGHT-SIM: in flight the draped detail-tile streamer is the terrain
        // renderer — real per-tile textures with quadtree LOD (fine near the
        // ship, coarse in the distance), draped over the 1:1 DEM. Orbit mode
        // keeps the stock focus-overlay-only behaviour.
        if (_flightActive && ctxMaxZoomStreaming) {
          if (!ctxDetailStreamer.active) ctxDetailStreamer.activate();
        } else if (ctxDetailStreamer.active) {
          ctxDetailStreamer.deactivate();
        }
        ...
        // Detail streamer runs only in flight mode (see activation above).
