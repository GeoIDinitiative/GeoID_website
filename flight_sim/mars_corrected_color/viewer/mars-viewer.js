            ...
            if (window.__flightSim?.active || (nowTick - this._lastCamMoveAt) >= settleMs) {
              ...
              if (window.__flightSim?.active) {
                ...
                if (shipGround) {
                  ...
                  if (this._flightInputMode === "near") {
                    const byAlt = altKm <= 10 ? 12 : altKm <= 25 ? 11 : altKm <= 50 ? 10 : 9;
                    const spd = Number(window.__flightSim.shipSpeedDegPerSec) || 0;
                    const baseB = byAlt >= 12 ? 100 : byAlt >= 11 ? 130 : 170;
                    const maxB = byAlt >= 12 ? 200 : byAlt >= 11 ? 260 : 340;
                    const budgetTiles = Math.max(24, Math.min(maxB, baseB * (0.16 / Math.max(spd, 0.05))));
                    const tileDeg = 360 / (1 << (byAlt + 1));
                    const span = Math.max(0.3, Math.min(14, tileDeg * Math.max(2, Math.sqrt(budgetTiles) - 3)));
                    // EXPANDED FORWARD EXTENT: window enlarged ×1.35 and led
                    // along the velocity (up to 0.55×base span, altitude-bounded
                    // at ~3 altitudes of ground), then GROWN along the track so
                    // its leading edge reaches FWD_MIN_KM further toward the
                    // horizon. Growing rather than shifting is the whole point:
                    // a shift buys ground ahead by giving up the ground under
                    // the ship, which is exactly what stripped the fine tiles
                    // off the surface at skim altitude last time.
                    const KM_PER_DEG = 59.3;
                    const altLeadDeg = (altKm / 59.3) * 3;
                    const lead = fdir ? Math.min(span * 0.55, altLeadDeg) : 0;
                    let effSpan = span * 1.35;
                    // At least 10 km of extra reach near the surface, more as
                    // altitude (and so the visible horizon) opens up.
                    let extraKm = fdir ? Math.max(10, altKm * 1.2) : 0;
                    // Track direction in KM space — away from the equator a
                    // degree of longitude is shorter than a degree of latitude,
                    // so a degree-space direction would skew the reach.
                    const cosLat = Math.max(0.2, Math.cos(shipGround.lat * Math.PI / 180));
                    const vLonKm = fv.lon * KM_PER_DEG * cosLat;
                    const vLatKm = fv.lat * KM_PER_DEG;
                    const vmKm = Math.hypot(vLonKm, vLatKm);
                    const ux = vmKm > 1e-6 ? vLonKm / vmKm : 0;
                    const uy = vmKm > 1e-6 ? vLatKm / vmKm : 0;
                    const buildWin = (extra, core) => {
                      const dLon = (ux * extra) / (KM_PER_DEG * cosLat);
                      const dLat = (uy * extra) / KM_PER_DEG;
                      const lonSpan = core + Math.abs(dLon);
                      const latSpan = core + Math.abs(dLat);
                      // Half the growth goes into the centre offset so the box
                      // extends forward only — the trailing edge does not move.
                      return {
                        lonSpan,
                        bbox: this._buildFocusBboxAroundTarget({
                          lon: shipGround.lon + (fdir ? fdir.lon * lead : 0) + dLon / 2,
                          lat: shipGround.lat + (fdir ? fdir.lat * lead : 0) + dLat / 2,
                        }, lonSpan, latSpan),
                      };
                    };
                    // Budget check against the REAL range function — its ±1
                    // tile pad is the trap that has caught every estimate in
                    // this file. The ladder authorises maxFocusTiles as
                    // min(ourCount, levelCap), so staying inside levelCap is
                    // what stops the budget loop stepping the level down.
                    const levelCap = byAlt === 9 ? 900 : (byAlt === 11 || byAlt === 12) ? 512 : 400;
                    let win = buildWin(extraKm, effSpan);
                    for (let guard = 0; guard < 40; guard += 1) {
                      if (this._getFocusTileRange(byAlt, win.bbox).tileCount <= levelCap) break;
                      // Surrender the ×1.35 enlargement before the forward
                      // reach: the reach is the thing that was asked for.
                      if (effSpan > span) effSpan = Math.max(span, effSpan * 0.94);
                      else if (extraKm > 0) extraKm = Math.max(0, extraKm - 2);
                      else break;
                      win = buildWin(extraKm, effSpan);
                    }
                    approxViewBbox = win.bbox;
                    focusTarget = {
                      lon: (win.bbox.lonMin + win.bbox.lonMax) / 2,
                      lat: (win.bbox.latMin + win.bbox.latMax) / 2,
                    };
                    this._flightFocusSpan = win.lonSpan;
                    this._flightFwdKm = extraKm;
                    this._flightCoreSpan = effSpan;
                    const flightTarget = byAlt;
                    if (!Number.isFinite(this._flightLevelHold)) this._flightLevelHold = flightTarget;
                    if (flightTarget !== this._flightLevelHold) {
                      this._flightLevelStreak = (this._flightLevelStreak || 0) + 1;
                      if (this._flightLevelStreak >= 12) {
                        this._flightLevelHold = flightTarget;
                        this._flightLevelStreak = 0;
                      }
                    } else {
                      this._flightLevelStreak = 0;
                    }
                    targetLevel = this._flightLevelHold;
                  } else {
                    // FAR mode: ladder level stands; regional bbox anchored on
                    // the look-at point, EXPANDED (budget 500→700) and led
                    // forward 0.35×span along the ground velocity so the
                    // regional map reaches ahead of the flight path.
                    this._flightLevelHold = undefined;
                    this._flightLevelStreak = 0;
                    const baseAnchor = focusTarget || shipGround;
                    const L = Number.isFinite(targetLevel) ? targetLevel : 8;
                    const tileDegFar = 360 / (1 << (L + 1));
                    const spanFar = Math.max(2, Math.min(24, tileDegFar * (Math.sqrt(700) - 3)));
                    const anchor = fdir ? {
                      lon: baseAnchor.lon + fdir.lon * spanFar * 0.35,
                      lat: baseAnchor.lat + fdir.lat * spanFar * 0.35,
                    } : baseAnchor;
                    approxViewBbox = this._buildFocusBboxAroundTarget(anchor, spanFar, spanFar);
                    focusTarget = anchor;
                  }
                }
                            } else {
              ...
              if (Number.isFinite(targetLevel)) {
                ...
                if (focusKey !== this._lastFocusKey && (nowTick - this._lastFocusUpdateAt) >= this.FOCUS_UPDATE_INTERVAL_MS) {
                  ...
                  const fetchMinLevel = Math.max(this.MIN_LEVEL, targetLevel - 4);
...
uniform float uViewportWidth;`,
      ...
      const ctxSurroundGlobe = new THREE.Mesh(
        new THREE.SphereGeometry(3.2006, 128, 128),
        ctxSurroundMaterial,
      );
        ...
        if (window.__flightSim?.active) {
          ...
          for (const entry of labelLayer.entries) {
            ...
            if (!out) { // promote: the standard viewer label — shown only while the
              ...
              if (entry.sprite.material) entry.sprite.material.depthTest = false;
          ...
          const MAX_SHOWN = 14, BIN_RAD = 6 * Math.PI / 180;
          ...
          for (const c of candidates) {
            ...
            if (shown.length >= MAX_SHOWN) { hideProxies(c.entry); continue; }
            const bin = Math.round(Math.atan2(tang.dot(Vb), tang.dot(Ub)) / BIN_RAD);
            if (usedBins.has(bin)) { hideProxies(c.entry); continue; }
            usedBins.add(bin);
            c.tang = tang;
            shown.push(c);
          }
          ...
          for (const c of shown) {
            ...
            const triPx = 16 - t * 5;                     // 16 px → 11 px
            const labPx = 18 - t * 4;                     // 18 px → 14 px (thin text needs a higher floor)
