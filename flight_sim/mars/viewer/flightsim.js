...
(function () {
  ...
  function setThrustVisuals(level, boosting) {
    ...
    for (const engine of thrustCones) {
      ...
      for (const dia of engine.diamonds) {
        ...
        const spacing = (engine.diaSpacing ?? 1.25) + (t * 0.9);
        dia.mesh.position.z = engine.origin.z + (engine.diaStart ?? 3.3) + (dia.index * spacing);
  ...
  function repairBaseTexture() {
    ...
    const path = (window.__marsViewerManifest?.texture?.path) || "https://data.geoidinitiative.com/assets/planet-mars/mars_color.jpg?v=aff5dbff30d7";
